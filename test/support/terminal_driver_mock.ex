defmodule Raxol.Terminal.DriverMock do
  ...
  def handle_info({:register_dispatcher, pid}, state) when is_pid(pid) do
    Raxol.Core.Runtime.Log.info("DriverMock: Registering dispatcher PID: #{inspect(pid)}")
  ...
  defp parse_test_input(input_data) do
    Raxol.Core.Runtime.Log.warning("DriverMock: Unknown test input: #{inspect(input_data)}")
    %Event{
