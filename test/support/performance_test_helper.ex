defmodule Raxol.Test.PerformanceTestHelper do
  ...
  def measure_average_time(operation, iterations \\ 1000) do
    {time, _} = Raxol.Test.PerformanceHelper.measure_time(fn ->
  ...
  def assert_performance(operation, name, threshold \\ 0.001, iterations \\ 1000) do
    ...
    assert avg_time < threshold, "Average time for #{name} operation (#{avg_time}ms) exceeds #{threshold}ms threshold"
  end
  ...
  def assert_concurrent_performance(operations, name, threshold \\ 0.002, iterations \\ 1000) do
    ...
    assert avg_time < threshold, "Average time for #{name} operations (#{avg_time}ms) exceeds #{threshold}ms threshold"
  end
end
