defmodule Raxol.UI.Components.Display.ProgressTest do
  ...
  describe "update/2" do
    ...
    test "updates animation state when animated" do
      ...
      initial_frame = 3
      old_timestamp = System.monotonic_time(:millisecond) - 200  # Older than animation speed
      progress = %{progress | state: %{animation_frame: initial_frame, last_update: old_timestamp}}
  ...
  describe "render/2" do
    test "renders basic progress bar" do
      ...
      assert String.length(text.text) == 8  # Total internal width
      assert String.trim_trailing(text.text) |> String.length() == 4  # Filled part
    end

    test "renders percentage text when enabled" do
      progress = Progress.create(%{progress: 0.75, width: 20, show_percentage: true})
      elements = Progress.render(progress, %{})
      ...
      percentage = Enum.find(elements, fn e ->
    ...
    test "generates correct bar content for different progress values" do
      ...
      empty_fill = Enum.find(empty_elements, fn e ->
      ...
      half_fill = Enum.find(half_elements, fn e ->
        e.type == :text && e.x == 1 && e.y == 0
      end)
      assert half_fill.text == String.duplicate("█", 4) <> String.duplicate(" ", 4)

      # Test completely filled bar
      full = Progress.create(%{progress: 1.0, width: 10})
      full_elements = Progress.render(full, %{})
      full_fill = Enum.find(full_elements, fn e ->
        e.type == :text && e.x == 1 && e.y == 0
      end)
    ...
    test "renders animation character when animated" do
      ...
      frame = 3  # Choose a specific frame for predictable test
      progress = Progress.create(%{
        progress: 0.5,
        width: 10,
        animated: true
      ...
      fill = Enum.find(elements, fn e ->
        e.type == :text && e.x == 1 && e.y == 0
      end)

      # Should have animation character at position 4 (after 4 filled blocks)
      animation_char = Enum.at(Progress.module_info(:attributes)[:animation_chars], frame)
      expected = String.duplicate("█", 4) <> animation_char <> String.duplicate(" ", 3)
