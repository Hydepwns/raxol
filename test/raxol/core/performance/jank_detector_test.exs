defmodule Raxol.Core.Performance.JankDetectorTest do
  ...
  describe "Jank Detector" do
    ...
    test "maintains window size limit" do
      ...
      detector = Enum.reduce(1..5, detector, fn _, acc ->
    ...
    test "calculates average frame time correctly" do
      ...
      # Record some frames
      detector = Enum.reduce([16, 20, 24], detector, fn time, acc ->
    ...
    test "tracks jank count in window" do
      ...
      detector = Enum.reduce([16, 20, 16, 24, 16], detector, fn time, acc ->
        JankDetector.record_frame(acc, time)
      end)
      
      assert detector.jank_count == 2  # 20ms and 24ms frames
    end
    ...
    test "updates jank count when window changes" do
      ...
      # Record janky frames
      detector = Enum.reduce([20, 20, 20], detector, fn _, acc ->
