defmodule Raxol.Core.Renderer.Views.PerformanceTest do
  ...
  # View helpers might still be used
  alias Raxol.Core.Renderer.View
  alias Raxol.Core.Runtime.ComponentManager
  # Alias for the module itself
  alias Raxol.Core.Renderer.Manager, as: RendererManagerMod
  # Added alias
  ...
  # seconds
  @threshold_initial_render 0.2
  # seconds (placeholder, as resize is not fully tested yet)
  @threshold_resize 0.3
  # 50MB (very generous)
  @threshold_memory_increase_bytes 50_000_000
  # 50ms (for 20 FPS target)
  @threshold_animation_frame_time 0.050
  # 100ms
  @threshold_animation_max_frame_time 0.100
  # 50ms
  @threshold_scroll_time 0.050
  # 100ms
  @threshold_max_scroll_time 0.100
  # 50ms
  @threshold_data_update_time 0.050
  # 100ms
  @threshold_max_data_update_time 0.100
  # 100ms
  @threshold_incremental_load_time 0.100
  # 200ms
  @threshold_max_incremental_load_time 0.200
  # 1 second
  @threshold_large_table_render 1.0
  # 0.5 seconds
  @threshold_complex_layout_render 0.5
  # 100ms
  @threshold_dynamic_update_complex 0.1
  ...
  setup do
    ...
    :ok =
      RendererManagerMod.initialize(
        fps: 60,
        # Enable debug mode for better test feedback
        debug_mode: true
      )

    # Subscribe to component render events
    {:ok, _ref} =
      Raxol.Core.Events.Manager.subscribe([:component_queued_for_render])
    ...
    {{:ok, host_component_id}, _initial_render_time} =
      measure_action_and_render_time(action_fun)
  ...
  defp measure_action_and_render_time(action_fun) do
    ...
    after
      ...
      15_000 ->
        Logger.error(
          "Timeout waiting for :render_cycle_complete in performance test measure helper."
        )
  ...
  defp mount_component_and_measure_initial_render(module, props) do
    ...
    if elem(mount_result, 0) == :error do
      Logger.error(
        "Failed to mount component in mount_component_and_measure_initial_render: #{inspect(mount_result)}"
      )
    end
  ...
  defp update_test_host_view_and_measure_render(host_component_id, view_map) do
    ...
    get_action_result_fun = fn -> :ok end
  ...
  describe "large table performance" do
    test "renders large table efficiently" do
      ...
      {mount_result, mount_time_sync} =
        measure_sync(fn -> ComponentManager.mount(Table, table_props) end)

      assert elem(mount_result, 0) == :ok,
             "Table component mount failed: #{inspect(mount_result)}"

      # _table_component_id_sync = elem(mount_result, 1) # If needed

      assert mount_time_sync < 0.1,
             "Table mounting (synchronous part) should be fast: got #{mount_time_sync}s"
      ...
      {render_mount_result, first_render_time} =
        mount_component_and_measure_initial_render(Table, table_props)

      assert elem(render_mount_result, 0) == :ok,
             "Table component mount for render measurement failed: #{inspect(render_mount_result)}"

      # {:ok, _table_component_id_for_render} = render_mount_result

      assert first_render_time < @threshold_large_table_render,
             "Large table initial render time #{first_render_time}s exceeded threshold #{@threshold_large_table_render}s"
    end
    ...
    test "handles dynamic updates efficiently" do
      ...
      {{:ok, table_component_id}, initial_time} =
        mount_component_and_measure_initial_render(Table, initial_table_props)

      # Define new props for the update
      updated_table_props = %{
        initial_table_props
        | # Start with old props
          # Change border
          border: :double,
          # Change striped
          striped: false
      ...
      assert elem(update_result, 0) == :ok,
             "Table component update failed: #{inspect(update_result)}"
  ...
  describe "complex layout performance" do
    test "handles deeply nested views efficiently", %{
      host_component_id: host_component_id
    } do
      # Create a deeply nested structure with alternating flex/grid containers
      {view_map, creation_time} =
        measure_sync(fn ->
          # This helper returns a view map
          PerformanceViewGenerators.create_nested_structure(10)
        end)
      ...
      render_time =
        update_test_host_view_and_measure_render(host_component_id, view_map)
      ...
      assert render_time < @threshold_initial_render,
             "Deeply nested view render time exceeded threshold"

      # Verify we have many nested views
      assert PerformanceViewGenerators.count_nested_views(view_map) > 100
    end

    test "manages multiple charts efficiently", %{
      host_component_id: host_component_id
    } do
      # Create a grid of charts
      # 4x4 grid
      # Renamed from 'charts' to avoid confusion with rendered view elements
      charts_data =
        for i <- 1..16 do
          Chart.new(
      ...
      {view_map, creation_time} =
        measure_sync(fn -> View.grid([columns: 4], do: charts_data) end)

      assert creation_time < 0.2,
             "Chart grid view map creation time should be less than 0.2s, got #{creation_time}s"
      ...
      Logger.info(
        "Created grid view with #{length(charts_data)} charts. Creation_time: #{creation_time}s, Render_time: #{render_time}s"
      )

      # Add a threshold for rendering multiple charts, e.g., @threshold_complex_layout_render
      assert render_time < @threshold_complex_layout_render,
             "Chart grid render time #{render_time}s exceeded threshold #{@threshold_complex_layout_render}s"
    end

    test "handles dynamic resizing efficiently", %{
      host_component_id: host_component_id
    } do
      # Create a complex layout view map
      view_map =
        PerformanceViewGenerators.create_configurable_test_layout(
          include_top_header: true,
          table_rows: 100,
          # From old create_complex_layout
          table_panel_width: 30,
          num_charts: 4,
          # 2x2 grid for 4 charts
          chart_grid_columns: 2,
      ...
      receive do
        :render_cycle_complete ->
          ...
          :ok
      after
        # Use the same timeout as measure_action_and_render_time for consistency
        15_000 ->
          flunk(
            "Timeout waiting for sync-up render after resize dispatch in 'handles dynamic resizing efficiently' test."
          )
      end
      ...
      {_action_result, post_resize_render_time} =
        measure_action_and_render_time(fn -> :ok end)

      # Assertions
      assert initial_time < @threshold_initial_render
      # Repurpose @threshold_resize
      assert post_resize_render_time < @threshold_resize
    ...
    test "maintains performance with z-index sorting", %{
      host_component_id: host_component_id
    } do
      # Create overlapping views with various z-indices
      # Renamed from 'views' to avoid confusion
      views_data =
        for i <- 1..100 do
          View.box(
      ...
      assert creation_time < 0.1,
             "Z-index view map creation time should be < 0.1s, got #{creation_time}s"
      ...
      Logger.info(
        "Z-index view: Creation_time: #{creation_time}s, Render_time: #{render_time}s"
      )

      # Add a threshold for rendering this, could be @threshold_initial_render or a new one.
      assert render_time < @threshold_initial_render,
             "Z-index view render time #{render_time}s exceeded threshold #{@threshold_initial_render}s"
    end
  end

  describe "memory usage" do
    test "maintains reasonable memory usage with large layouts", %{
      host_component_id: host_component_id
    } do
      # Create a large complex layout view map
      view_map =
        PerformanceViewGenerators.create_configurable_test_layout(
          # Uses full @large_data by default if table_data_source is not changed
          table_rows: 1000,
          # From old create_large_complex_layout
          table_panel_width: 40,
          num_charts: 9,
          # 3x3 grid for 9 charts
          chart_grid_columns: 3,
      ...
      for i <- 1..10 do
        ...
        after
          ...
          15_000 ->
            flunk(
              "Timeout waiting for render_cycle_complete in memory test loop, frame #{i}"
            )
        end
      end
  ...
  describe "animation performance" do
    ...
    setup do
      ...
      # Assuming ComponentManager is globally available or passed
      %{component_manager: ComponentManager}
    end

    test "handles smooth progress bar animation", %{
      host_component_id: host_component_id
    } do
      # List of view maps
      frames = PerformanceViewGenerators.create_progress_bar_frames(50)

      frame_render_times =
        Enum.map(frames, fn frame_view_map ->
          update_test_host_view_and_measure_render(
            host_component_id,
            frame_view_map
          )
        end)
      ...
      avg_frame_time =
        Enum.sum(frame_render_times) / Enum.count(frame_render_times)
    ...
    test "handles spinner animation efficiently", %{
      host_component_id: host_component_id
    } do
      # List of view maps
      frames = PerformanceViewGenerators.create_spinner_frames(50)
    ...
    test "handles chart animation smoothly", %{
      host_component_id: host_component_id
    } do
      # List of view maps
      frames = PerformanceViewGenerators.create_chart_animation_frames(50)
  ...
  describe "scrolling performance" do
    ...
    setup %{host_component_id: host_component_id} do
      ...
      # Pass host_component_id from top-level setup
      %{host_component_id: host_component_id}
    end

    test "handles smooth vertical scrolling", %{
      host_component_id: host_component_id
    } do
      ...
      table_props_for_view = %{
        ...
        # 100 rows
        data: PerformanceTestData.generate_data(100, 5),
        border: :single
      }
      ...
      initial_render_time =
        update_test_host_view_and_measure_render(
          host_component_id,
          Table.new(table_props_for_view)
        )

      Logger.info(
        "Vertical scrolling: initial full table render_time=#{initial_render_time}s"
      )
      ...
      scroll_render_times =
        for i <- 1..scroll_steps do
          ...
          scrolled_data =
            Enum.slice(table_props_for_view.data, i, viewport_height)

          scrolled_table_view =
            Table.new(Map.put(table_props_for_view, :data, scrolled_data))
      ...
      avg_scroll_time =
        Enum.sum(scroll_render_times) / Enum.count(scroll_render_times)
    ...
    test "handles horizontal scrolling efficiently", %{
      host_component_id: host_component_id
    } do
      # Similar caveats as vertical scrolling test.
      table_props_for_view = %{
        # Many columns
        columns: PerformanceTestData.generate_columns(20),
        # 10 rows
        data: PerformanceTestData.generate_data(10, 20),
        border: :single
      }
      ...
      Logger.info(
        "Horizontal scrolling: initial full table render_time=#{initial_render_time}s"
      )
      ...
      scroll_render_times =
        for i <- 1..scroll_steps do
          ...
          # Show 5 columns at a time
          scrolled_columns = Enum.slice(table_props_for_view.columns, i, 5)

          scrolled_table_view =
            Table.new(Map.put(table_props_for_view, :columns, scrolled_columns))
  ...
  describe "dynamic content performance" do
    ...
    test "handles real-time data updates efficiently", %{
      host_component_id: host_component_id
    } do
      ...
      # Initial view render
      initial_view_map =
        Table.new(columns: columns, data: initial_data, border: :single)
      ...
      update_render_times =
        for i <- 1..20 do
          ...
          updated_view_map =
            Table.new(columns: columns, data: new_data, border: :single)
      ...
      avg_update_time =
        Enum.sum(update_render_times) / Enum.count(update_render_times)
    ...
    test "handles incremental content loading", %{
      host_component_id: host_component_id
    } do
      columns = PerformanceTestData.generate_columns(10)
      # Start with 10 rows
      initial_data = PerformanceTestData.generate_data(10, 10)
      ...
      # Load up to 1000 rows in chunks
      load_render_times =
        for chunk_start <- 0..950//chunk_size do
          # Adjust start_offset
          new_rows =
            PerformanceTestData.generate_data(
              chunk_size,
              10,
              chunk_start + length(initial_data)
            )

          current_data = current_data ++ new_rows

          updated_view_map =
            Table.new(columns: columns, data: current_data, border: :single)
      ...
      avg_load_time =
        Enum.sum(load_render_times) / Enum.count(load_render_times)
