  ...
  describe "plugin lifecycle events" do
    test "handles plugin termination errors", %{command_registry_table: table} do
      Helper.with_running_manager([command_registry_table: table], fn manager_pid ->
        ...
        assert {:error, :termination_failed} = Manager.unload_plugin(manager_pid, :failing_termination_plugin)
      end)
    end

    test "handles plugin state transition errors", %{command_registry_table: table} do
      Helper.with_running_manager([command_registry_table: table], fn manager_pid ->
    ...
    test "handles plugin event handling errors", %{command_registry_table: table} do
      Helper.with_running_manager([command_registry_table: table], fn manager_pid ->
        ...
        assert {:error, :event_handling_failed} = Manager.handle_event(manager_pid, :failing_event_handler_plugin, event)
      end)
    end
    ...
    test "handles plugin cleanup errors", %{command_registry_table: table} do
      Helper.with_running_manager([command_registry_table: table], fn manager_pid ->
        ...
        # Attempt to unload the plugin
        assert {:error, :cleanup_failed} = Manager.unload_plugin(manager_pid, :failing_cleanup_plugin)
      end)
    end
  end
end
