defmodule Raxol.Core.Runtime.Plugins.LifecycleHelperTest do
  ...
  describe "load_plugin/8" do
    test "loads plugin by ID successfully" do
      ...
      expect(CommandHelperMock, :register_plugin_commands, fn ^plugin_module, _, ^command_table ->
        :ok
      end)

      # Call the function
      {:ok, updated_maps} = LifecycleHelper.load_plugin(
        plugin_id,
        config,
        state_maps.plugins,
        state_maps.metadata,
      ...
      assert updated_maps.metadata == %{plugin_id => %{id: plugin_id, version: "1.0.0"}}
    ...
    test "handles already loaded plugin" do
      ...
      # Call the function
      {:error, :already_loaded} = LifecycleHelper.load_plugin(
        plugin_id,
        %{},
        state_maps.plugins,
        state_maps.metadata,
    ...
    test "handles missing dependencies" do
      ...
      # Call the function
      {:error, :missing_dependencies, ["missing_plugin"]} = LifecycleHelper.load_plugin(
        plugin_id,
        %{},
        state_maps.plugins,
        state_maps.metadata,
    ...
    test "handles init failure" do
      ...
      # Call the function
      {:error, {:init_failed, :init_failed}} = LifecycleHelper.load_plugin(
        plugin_id,
        %{},
        state_maps.plugins,
        state_maps.metadata,
  ...
  describe "unload_plugin/2" do
    test "unloads plugin successfully" do
      ...
      expect(CommandHelperMock, :unregister_plugin_commands, fn ^plugin_module, ^command_table ->
        :ok
      end)

      # Call the function
      {:ok, updated_maps} = LifecycleHelper.unload_plugin(plugin_id, state_maps, command_table)
    ...
    test "handles non-existent plugin" do
      ...
      # Call the function
      {:error, :plugin_not_found} = LifecycleHelper.unload_plugin(
        "unknown_plugin",
        state_maps,
        :test_command_table
      )
  ...
  describe "reload_plugin_from_disk/8" do
    test "successfully reloads plugin with state persistence", %{
      ...
      # Reload the plugin
      assert {:ok, updated_maps} = LifecycleHelper.reload_plugin_from_disk(
        plugin_id, config, plugins, metadata, plugin_states, load_order, command_table, plugin_config
      )
    ...
    test "handles plugin not found", %{
      ...
      assert {:error, :plugin_not_found} = LifecycleHelper.reload_plugin_from_disk(
        :non_existent_plugin, config, plugins, metadata, plugin_states, load_order, command_table, plugin_config
      )
    end
    ...
    test "handles initialization failure", %{
      ...
      assert {:error, :init_failed} = LifecycleHelper.reload_plugin_from_disk(
        plugin_id, config, plugins, metadata, plugin_states, load_order, command_table, plugin_config
      )
    end
    ...
    test "handles dependency check failure", %{
      ...
      expect(DependencyManagerMock, :check_dependencies, fn _, _, _ -> {:error, :dependency_failed} end)

      assert {:error, :dependency_failed} = LifecycleHelper.reload_plugin_from_disk(
        plugin_id, config, plugins, metadata, plugin_states, load_order, command_table, plugin_config
      )
    end
    ...
    test "handles command registration failure", %{
      ...
      expect(CommandHelperMock, :register_plugin_commands, fn _, _, _ -> {:error, :registration_failed} end)

      assert {:error, :registration_failed} = LifecycleHelper.reload_plugin_from_disk(
        plugin_id, config, plugins, metadata, plugin_states, load_order, command_table, plugin_config
      )
    end
  end
  ...
  describe "plugin version compatibility" do
    test "handles incompatible plugin version", %{command_registry_table: table} do
      ...
      {:error, :incompatible_version} = LifecycleHelper.load_plugin(
        plugin_id,
        config,
        state_maps.plugins,
        state_maps.metadata,
    ...
    test "handles compatible plugin version", %{command_registry_table: table} do
      ...
      # Verify results
      assert updated_maps.plugins == %{plugin_id => plugin_module}
      assert updated_maps.metadata == %{plugin_id => %{id: plugin_id, version: "1.0.0", min_api_version: "1.0.0"}}
    end
  end

  describe "plugin state persistence" do
    test "preserves plugin state during reload", %{command_registry_table: table} do
      # Setup test data
      plugin_id = "test_plugin"
      plugin_module = TestPlugin
      ...
      # Call the function
      {:ok, updated_maps} = LifecycleHelper.reload_plugin_from_disk(
        plugin_id,
        config,
        state_maps.plugins,
        state_maps.metadata,
  ...
  describe "plugin dependency resolution" do
    test "loads plugins in correct dependency order", %{command_registry_table: table} do
      # Setup test data
      plugin_a = "plugin_a"
      plugin_b = "plugin_b"
      plugin_c = "plugin_c"
      ...
      {:ok, maps_after_a} = LifecycleHelper.load_plugin(plugin_a, config, state_maps.plugins, state_maps.metadata, state_maps.plugin_states, state_maps.load_order, table, state_maps.plugin_config)
      {:ok, maps_after_b} = LifecycleHelper.load_plugin(plugin_b, config, maps_after_a.plugins, maps_after_a.metadata, maps_after_a.plugin_states, maps_after_a.load_order, table, maps_after_a.plugin_config)
      {:ok, maps_after_c} = LifecycleHelper.load_plugin(plugin_c, config, maps_after_b.plugins, maps_after_b.metadata, maps_after_b.plugin_states, maps_after_b.load_order, table, maps_after_b.plugin_config)
  ...
  describe "plugin initialization timeout" do
    test "handles plugin initialization timeout", %{command_registry_table: table} do
      # Setup test data
      plugin_id = "test_plugin"
      plugin_module = TestPlugin
      config = %{setting: "value"}
      ...
      {:error, :init_timeout} = LifecycleHelper.load_plugin(
        plugin_id,
        config,
        state_maps.plugins,
        state_maps.metadata,
