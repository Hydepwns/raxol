defmodule Raxol.Core.Runtime.Plugins.CommandRegistryTest do
  ...
  describe "register_plugin_commands/3" do
    ...
    test "handles invalid command name", %{
      ...
      assert {:error, :invalid_command_name_format} =
               CommandRegistry.register_plugin_commands(
                 plugin_module,
                 plugin_state,
                 command_table
    ...
    test "handles invalid metadata", %{
      ...
      assert {:error, :invalid_metadata_fields} =
               CommandRegistry.register_plugin_commands(
                 plugin_module,
                 plugin_state,
                 command_table
    ...
    test "handles command conflicts", %{
      ...
      command_table =
        Map.put(command_table, :other_plugin, [
          {"test_command", fn args, _ctx -> {:ok, args} end, %{}}
        ])

      assert {:error, {:command_exists, "test_command"}} =
               CommandRegistry.register_plugin_commands(
                 plugin_module,
                 plugin_state,
                 command_table
  ...
  describe "execute_command/3" do
    ...
    test "handles execution error", %{
      ...
      assert {:error, {:execution_failed, "Test error"}} =
               CommandRegistry.execute_command(
                 "error_command",
                 %{},
                 command_table
  ...
  describe "unregister_plugin_commands/2" do
    test "successfully unregisters plugin commands", %{
      ...
      assert {:ok, updated_table} =
               CommandRegistry.unregister_plugin_commands(
                 plugin_module,
                 command_table
               )
