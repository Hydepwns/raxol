defmodule Raxol.Core.Runtime.Plugins.MemoryManagementTest do
  ...
  describe "memory management" do
    ...
    test "handles memory cleanup during repeated operations" do
      ...
      {_, memory_diff} = DependencyManagerTestHelper.measure_memory_usage(fn ->
        Enum.each(1..100, fn _ ->
          {:ok, _} = DependencyManager.resolve_load_order(plugins)
          :ok = DependencyManager.check_dependencies("plugin_a", plugins["plugin_a"], plugins)
        end)
      end)
    ...
    test "handles memory cleanup with rapid plugin updates" do
      ...
      {_, memory_diff} = DependencyManagerTestHelper.measure_memory_usage(fn ->
        Enum.each(1..100, fn i ->
          ...
          :ok = DependencyManager.check_dependencies("plugin_a", plugins["plugin_a"], updated_plugins)
        end)
      end)
    ...
    test "handles memory cleanup with mixed operations" do
      ...
      {_, memory_diff} = DependencyManagerTestHelper.measure_memory_usage(fn ->
        Enum.each(1..50, fn i ->
          ...
          # Update and check
          updated_plugins = Map.update!(plugins, "plugin_c", fn plugin ->
            Map.put(plugin, :version, "#{i}.0.0")
          end)
          :ok = DependencyManager.check_dependencies("plugin_a", plugins["plugin_a"], updated_plugins)
        end)
