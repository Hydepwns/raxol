defmodule Raxol.Core.Runtime.Events.HandlersTest do
  use ExUnit.Case, async: false
  import ExUnit.CaptureLog

  alias Raxol.Core.Runtime.Events.Handlers
  ...
  describe "register_handler/4" do
    test "registers a handler with default options" do
      ...
      assert {:ok, :test_handler} = Handlers.register_handler(:test_handler, [:test_event], handler_fun)
    end
    ...
    test "registers a handler with custom priority and filter" do
      ...
      assert {:ok, :custom_handler} = Handlers.register_handler(
        :custom_handler,
        [:test_event],
        handler_fun,
        [priority: 50, filter: filter_fun]
      )
    end
  end
  ...
  describe "unregister_handler/1" do
    test "unregisters an existing handler" do
      ...
      handler_fun = fn _event, state -> {:ok, _event, state} end
      {:ok, :temp_handler} = Handlers.register_handler(:temp_handler, [:test_event], handler_fun)
    ...
    test "returns error when unregistering non-existent handler" do
      assert {:error, :not_found} = Handlers.unregister_handler(:non_existent_handler)
    end
  end
  ...
  describe "execute_handlers/2" do
    test "executes handlers in priority order", %{event: event, state: state} do
      ...
      Handlers.register_handler(:handler1, [:test_event], fn e, s ->
        current = Process.get(:execution_order)
        Process.put(:execution_order, current ++ [:handler1])
        {:ok, e, s}
      end, priority: 30)

      Handlers.register_handler(:handler2, [:test_event], fn e, s ->
        current = Process.get(:execution_order)
        Process.put(:execution_order, current ++ [:handler2])
        {:ok, e, s}
      end, priority: 10)

      Handlers.register_handler(:handler3, [:test_event], fn e, s ->
        current = Process.get(:execution_order)
    ...
    test "filters events based on filter function", %{event: event, state: state} do
      # Register a handler that only processes events with specific data
      Handlers.register_handler(:filtered_handler, [:test_event], fn e, s ->
        # Update state to mark that the handler was executed
        {:ok, e, %{s | count: s.count + 1}}
      end, filter: fn e -> e.data == "test data" end)

      # This event should be processed
      {:ok, _updated_event, updated_state} = Handlers.execute_handlers(event, state)
      ...
      {:ok, _updated_event, updated_state} = Handlers.execute_handlers(filtered_event, state)
      assert updated_state.count == 0
    end

    test "handler can transform event for next handler", %{event: event, state: state} do
      # First handler transforms the event
      Handlers.register_handler(:transformer, [:test_event], fn e, s ->
        {:ok, %{e | data: "transformed"}, s}
      end, priority: 10)

      # Second handler checks the transformed data
      Handlers.register_handler(:checker, [:test_event], fn e, s ->
      ...
      {:ok, updated_event, updated_state} = Handlers.execute_handlers(event, state)
    ...
    test "handler can stop event propagation", %{event: event, state: state} do
      ...
      Handlers.register_handler(:stopper, [:test_event], fn e, s ->
        {:stop, %{e | data: "stopped"}, s}
      end, priority: 10)

      # Second handler should not be executed
      Handlers.register_handler(:never_run, [:test_event], fn _e, s ->
        {:ok, _e, %{s | count: 999}}  # This would make count 999 if executed
      end, priority: 20)
    ...
    test "handles errors in handlers", %{event: event, state: state} do
      ...
      # Execute handler and capture the log
      log = capture_log(fn ->
        assert {:error, {:handler_error, _}, ^state} = Handlers.execute_handlers(event, state)
      end)
    ...
    test "handles handler returning an error", %{event: event, state: state} do
      ...
      Handlers.register_handler(:error_return_handler, [:test_event], fn _e, _s ->
        {:error, :test_error}
      end)

      # Execute handler and capture the log
      log = capture_log(fn ->
        assert {:error, :test_error, ^state} = Handlers.execute_handlers(event, state)
      end)
