defmodule Raxol.Core.Buffer.BufferServerConcurrentTest do
  ...
  setup do
    {:ok, context} = setup_unique_buffer("BufferServer")
    {:ok, context}
  end
  ...
  describe "Concurrent Read/Write Operations" do
    test "handles concurrent reads and writes", %{buffer_pid: pid} do
      ...
      readers = create_concurrent_readers(pid, 2, 10)
      writers = create_concurrent_writers(pid, 2, regions: true, iterations: 10)

      # Wait for all processes to complete using helper
      assert :ok = await_and_verify_tasks(readers ++ writers, 10_000)
    end
  end
