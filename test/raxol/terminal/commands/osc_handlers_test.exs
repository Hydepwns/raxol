defmodule Raxol.Terminal.Commands.OSCHandlerTest do
  ...
  describe "window title and icon" do
    ...
    test "sets icon title", %{emulator: emulator} do
      result = OSCHandler.handle_icon_title(emulator, "Test Icon Title")
      assert {:ok, updated_emulator} = result
    ...
    test "handles empty titles", %{emulator: emulator} do
      result = OSCHandler.handle_window_title(emulator, "")
      assert {:ok, updated_emulator} = result
      assert updated_emulator.window_title == ""
    end

    test "handles nil titles", %{emulator: emulator} do
      result = OSCHandler.handle_window_title(emulator, nil)
      assert {:ok, updated_emulator} = result
    ...
    test "handles very long titles", %{emulator: emulator} do
      ...
      result = OSCHandler.handle_window_title(emulator, long_title)
      assert {:ok, updated_emulator} = result
  ...
  describe "color settings" do
    test "sets color index", %{emulator: emulator} do
      result = OSCHandler.handle_osc4_color(emulator, 1, "#FF0000")
      assert result == {:ok, emulator}
    end

    test "sets foreground color", %{emulator: emulator} do
      result = OSCHandler.handle_foreground_color(emulator, "#FF0000")
      assert result == {:ok, emulator}
    end

    test "sets background color", %{emulator: emulator} do
      result = OSCHandler.handle_background_color(emulator, "#0000FF")
      assert result == {:ok, emulator}
    end

    test "sets cursor color", %{emulator: emulator} do
      result = OSCHandler.handle_cursor_color(emulator, "#00FF00")
      assert result == {:ok, emulator}
    end

    test "sets mouse foreground color", %{emulator: emulator} do
      result = OSCHandler.handle_mouse_foreground_color(emulator, "#FF00FF")
      assert result == {:ok, emulator}
    end

    test "sets mouse background color", %{emulator: emulator} do
      result = OSCHandler.handle_mouse_background_color(emulator, "#00FFFF")
      assert result == {:ok, emulator}
    end

    test "sets highlight foreground color", %{emulator: emulator} do
      result =
        OSCHandler.handle_highlight_foreground_color(emulator, "#FFFFFF")
    ...
    test "sets highlight background color", %{emulator: emulator} do
      result =
        OSCHandler.handle_highlight_background_color(emulator, "#000000")

      assert result == {:ok, emulator}
    end

    test "handles invalid color formats", %{emulator: emulator} do
      result = OSCHandler.handle_foreground_color(emulator, "invalid")
      assert result == {:ok, emulator}
    end

    test "handles invalid color indices", %{emulator: emulator} do
      result = OSCHandler.handle_osc4_color(emulator, 999, "#FF0000")
      assert result == {:ok, emulator}
    end

    test "handles nil colors", %{emulator: emulator} do
      result = OSCHandler.handle_foreground_color(emulator, nil)
      assert result == {:ok, emulator}
    end
  end
  ...
  describe "font settings" do
    test "sets font", %{emulator: emulator} do
      result = OSCHandler.handle_font(emulator, "Monospace")
      assert result == {:ok, emulator}
    end

    test "handles empty font", %{emulator: emulator} do
      result = OSCHandler.handle_font(emulator, "")
      assert result == {:ok, emulator}
    end

    test "handles nil font", %{emulator: emulator} do
      result = OSCHandler.handle_font(emulator, nil)
      assert result == {:ok, emulator}
    end

    test "handles very long font names", %{emulator: emulator} do
      long_font = String.duplicate("a", 1000)
      result = OSCHandler.handle_font(emulator, long_font)
      assert result == {:ok, emulator}
    end
  end
  ...
  describe "clipboard operations" do
    test "sets clipboard content", %{emulator: emulator} do
      result = OSCHandler.handle_clipboard_set(emulator, "Test Content")
      assert result == {:ok, emulator}
    end

    test "gets clipboard content", %{emulator: emulator} do
      result = OSCHandler.handle_clipboard_get(emulator)
      assert result == {:ok, emulator}
    end

    test "handles empty clipboard", %{emulator: emulator} do
      result = OSCHandler.handle_clipboard_set(emulator, "")
      assert result == {:ok, emulator}
    end

    test "handles nil clipboard", %{emulator: emulator} do
      result = OSCHandler.handle_clipboard_set(emulator, nil)
      assert result == {:ok, emulator}
    end

    test "handles very large clipboard content", %{emulator: emulator} do
      large_content = String.duplicate("a", 10_000)
      result = OSCHandler.handle_clipboard_set(emulator, large_content)
      assert result == {:ok, emulator}
    end

    test "handles invalid base64 clipboard data", %{emulator: emulator} do
      result = OSCHandler.handle_clipboard_set(emulator, "invalid base64")
      assert result == {:ok, emulator}
    end
  end
  ...
  describe "cursor shape" do
    test "sets cursor shape", %{emulator: emulator} do
      result = OSCHandler.handle_cursor_shape(emulator, "block")
      assert result == {:ok, emulator}
    end

    test "handles invalid cursor shapes", %{emulator: emulator} do
      result = OSCHandler.handle_cursor_shape(emulator, "invalid")
      assert result == {:ok, emulator}
    end

    test "handles empty cursor shape", %{emulator: emulator} do
      result = OSCHandler.handle_cursor_shape(emulator, "")
      assert result == {:ok, emulator}
    end

    test "handles nil cursor shape", %{emulator: emulator} do
      result = OSCHandler.handle_cursor_shape(emulator, nil)
      assert result == {:ok, emulator}
    end
  end
  ...
  describe "handle_4/2" do
    ...
    test "handles multiple color queries", %{emulator: emulator} do
      ...
      assert result == {:ok, emulator}
      # Green
      result = OSCHandler.handle_4(emulator, "2;rgb:0000/FFFF/0000")
      assert result == {:ok, emulator}
      # Blue
      result = OSCHandler.handle_4(emulator, "3;rgb:0000/0000/FFFF")
      assert result == {:ok, emulator}

      # Query each color
      result = OSCHandler.handle_4(emulator, "1;?")
      assert result == {:ok, emulator}
      result = OSCHandler.handle_4(emulator, "2;?")
      assert result == {:ok, emulator}
      result = OSCHandler.handle_4(emulator, "3;?")
      assert result == {:ok, emulator}
    end
  end
end
