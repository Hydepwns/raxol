defmodule Raxol.Terminal.Plugin.UnifiedPluginTest do
  ...
  setup do
    {:ok, _pid} = UnifiedPlugin.start_link(
  ...
  describe "basic operations" do
    test "loads and unloads plugins" do
      # Load theme plugin
      assert {:ok, theme_id} = UnifiedPlugin.load_plugin(
      ...
      # Load script plugin
      assert {:ok, script_id} = UnifiedPlugin.load_plugin(
      ...
      assert {:error, :plugin_not_found} = UnifiedPlugin.get_plugin_state(theme_id)
      assert {:error, :plugin_not_found} = UnifiedPlugin.get_plugin_state(script_id)
    end
    ...
    test "handles plugin configuration" do
      ...
      assert {:ok, plugin_id} = UnifiedPlugin.load_plugin(
        "test/fixtures/plugins/theme",
        :theme,
        name: "Test Theme",
        version: "1.0.0",
  ...
  describe "plugin types" do
    test "handles theme plugins" do
      ...
      assert {:ok, result} = UnifiedPlugin.execute_plugin_function(
        plugin_id,
        :apply_theme,
  ...
  describe "plugin management" do
    ...
    test "handles plugin dependencies" do
      ...
      assert {:ok, dependent_id} = UnifiedPlugin.load_plugin(
      ...
      # Load base plugin
      assert {:ok, base_id} = UnifiedPlugin.load_plugin(
  ...
  describe "error handling" do
    test "handles invalid plugin types" do
      assert {:error, :invalid_plugin_type} = UnifiedPlugin.load_plugin(
    ...
    test "handles invalid plugin formats" do
      assert {:error, :invalid_plugin_format} = UnifiedPlugin.load_plugin(
        "test/fixtures/plugins/invalid",
        :theme,
        name: "Invalid Plugin"
    ...
    test "handles non-existent plugins" do
      assert {:error, :plugin_not_found} = UnifiedPlugin.get_plugin_state("non_existent")
      assert {:error, :plugin_not_found} = UnifiedPlugin.unload_plugin("non_existent")
      assert {:error, :plugin_not_found} = UnifiedPlugin.update_plugin_config("non_existent", %{})
    end
    ...
    test "handles invalid configurations" do
      ...
      assert {:error, :invalid_config_format} = UnifiedPlugin.update_plugin_config(plugin_id, "invalid_config")
    end
  end
