defmodule Raxol.Terminal.Input.ProcessorTest do
  ...
  alias Raxol.Terminal.Input.{Processor, Event, Event.MouseEvent, Event.KeyEvent}

  describe "input processing" do
    test "processes regular character input" do
      assert {:ok, %KeyEvent{key: "a", modifiers: []}} = Processor.process_input("a")
    end
  ...
  describe "mouse events" do
    ...
    test "handles invalid mouse events" do
      assert {:error, :invalid_mouse_event} = Processor.parse_mouse_event("invalid")
      assert {:error, :invalid_mouse_sequence} = Processor.parse_mouse_event("\e[invalid")
    end
  end

  describe "key events" do
    test "parses regular key press" do
      assert {:ok, %KeyEvent{key: "a", modifiers: []}} = Processor.parse_key_event("a")
    end

    test "parses function keys" do
      assert {:ok, %KeyEvent{key: "A", modifiers: []}} = Processor.parse_key_event("\e[A")
    end
    ...
    test "handles invalid key events" do
      ...
      assert {:error, :invalid_key_sequence} = Processor.parse_key_event("\e[invalid")
    end
  end
