defmodule Raxol.Terminal.Input.ProcessorTest do
  ...
  describe "mouse events" do
    ...
    test ~c"parses mouse release event" do
    ...
    test ~c"parses mouse drag event" do
    ...
    test ~c"parses mouse move event" do
    ...
    test ~c"parses mouse events with modifiers" do
    ...
    test ~c"handles invalid mouse events" do
  ...
  describe "key events" do
    test ~c"parses regular key press" do
      assert {:ok, %KeyEvent{key: "a", modifiers: []}} =
               Processor.parse_key_event("a")
    end

    test ~c"parses function keys" do
      assert {:ok, %KeyEvent{key: "A", modifiers: []}} =
               Processor.parse_key_event("\e[A")
    end

    test ~c"parses keys with modifiers" do
      assert {:ok, %KeyEvent{key: "A", modifiers: [:shift, :ctrl]}} =
               Processor.parse_key_event("\e[2;5A")
    end

    test ~c"handles invalid key events" do
  ...
  describe "event formatting" do
    test ~c"formats mouse events" do
      event = %MouseEvent{
        button: :left,
        action: :press,
        x: 10,
    ...
    test ~c"formats regular key events without escape sequence" do
      event = %KeyEvent{
        key: "a",
        modifiers: []
      }
