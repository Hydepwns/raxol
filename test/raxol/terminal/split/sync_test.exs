defmodule Raxol.Terminal.Split.SyncTest do
  ...
  describe "event broadcasting" do
    test "broadcasts events to subscribers", %{pid: pid} do
      ...
      # Broadcast event
      :ok =
        GenServer.cast(pid, {:broadcast_event, split_id, event_type, payload})
  ...
  describe "shared state management" do
    test "updates and retrieves shared state", %{pid: pid} do
      ...
      # Update state
      updated_state =
        GenServer.call(pid, {:update_shared_state, split_id, initial_state})

      assert updated_state == initial_state

      # Update state again
      final_state =
        GenServer.call(pid, {:update_shared_state, split_id, state_updates})
  ...
  describe "subscription management" do
    test "unsubscribes from events", %{pid: pid} do
      ...
      :ok =
        GenServer.cast(
          pid,
          {:broadcast_event, split_id, :test_event, %{data: "test"}}
        )
