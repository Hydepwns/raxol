defmodule Raxol.Terminal.Cache.UnifiedCacheTest do
  ...
  describe "namespaces" do
    ...
    test "non-existent namespace" do
      assert {:error, :namespace_not_found} == UnifiedCache.get("key1", namespace: :nonexistent)
    end
  end
  ...
  describe "TTL" do
    ...
    test "non-expired entry" do
      ...
      Process.sleep(1000)  # Wait less than TTL
      assert {:ok, "value1"} == UnifiedCache.get("key1")
    end
  end
  ...
  describe "eviction policies" do
    test "LRU eviction" do
      {:ok, _pid} = UnifiedCache.start_link(max_size: 100, eviction_policy: :lru)
      UnifiedCache.put("key1", String.duplicate("a", 50))
      UnifiedCache.put("key2", String.duplicate("b", 50))
      UnifiedCache.put("key3", String.duplicate("c", 50))  # Should evict key1
      assert {:error, :not_found} == UnifiedCache.get("key1")
      assert {:ok, _} == UnifiedCache.get("key2")
      assert {:ok, _} == UnifiedCache.get("key3")
    end

    test "LFU eviction" do
      {:ok, _pid} = UnifiedCache.start_link(max_size: 100, eviction_policy: :lfu)
      UnifiedCache.put("key1", String.duplicate("a", 50))
      UnifiedCache.put("key2", String.duplicate("b", 50))
      UnifiedCache.get("key1")  # Access key1 twice
      UnifiedCache.get("key1")
      UnifiedCache.put("key3", String.duplicate("c", 50))  # Should evict key2
      assert {:ok, _} == UnifiedCache.get("key1")
      assert {:error, :not_found} == UnifiedCache.get("key2")
      assert {:ok, _} == UnifiedCache.get("key3")
    end

    test "FIFO eviction" do
      {:ok, _pid} = UnifiedCache.start_link(max_size: 100, eviction_policy: :fifo)
      UnifiedCache.put("key1", String.duplicate("a", 50))
      UnifiedCache.put("key2", String.duplicate("b", 50))
      UnifiedCache.put("key3", String.duplicate("c", 50))  # Should evict key1
      assert {:error, :not_found} == UnifiedCache.get("key1")
  ...
  describe "statistics" do
    test "hit and miss counts" do
      UnifiedCache.put("key1", "value1")
      UnifiedCache.get("key1")  # Hit
      UnifiedCache.get("key1")  # Hit
      UnifiedCache.get("nonexistent")  # Miss
