defmodule Raxol.Terminal.Cache.SystemTest do
  ...
  setup do
    ...
    {:ok, _pid} = System.start_link(
      ...
      namespace_configs: %{
        buffer: %{max_size: 512 * 1024},  # 512KB
        animation: %{max_size: 256 * 1024},  # 256KB
        scroll: %{max_size: 128 * 1024},  # 128KB
        clipboard: %{max_size: 64 * 1024},  # 64KB
        general: %{max_size: 64 * 1024}  # 64KB
      }
    )
  ...
  describe "basic operations" do
    ...
    test "clear" do
      ...
      assert {:error, :not_found} == System.get("test_key1", namespace: :general)
      assert {:error, :not_found} == System.get("test_key2", namespace: :general)
    end
  end
  ...
  describe "namespace operations" do
    test "different namespaces" do
      ...
      assert {:ok, "animation_value"} == System.get("test_key", namespace: :animation)
      assert {:ok, "scroll_value"} == System.get("test_key", namespace: :scroll)
      assert {:ok, "clipboard_value"} == System.get("test_key", namespace: :clipboard)
      assert {:ok, "general_value"} == System.get("test_key", namespace: :general)
    end

    test "namespace not found" do
      assert {:error, :namespace_not_found} == System.get("test_key", namespace: :invalid)
    end
  end
  ...
  describe "TTL operations" do
    test "expired entry" do
      ...
      :timer.sleep(1100)  # Wait for entry to expire
    ...
    test "non-expired entry" do
      ...
      :timer.sleep(1000)  # Wait less than TTL
      assert {:ok, "test_value"} == System.get("test_key", namespace: :general)
    end
  end
  ...
  describe "eviction policies" do
    test "LRU eviction" do
      ...
      for i <- 1..10 do
        System.put("key#{i}", String.duplicate("value", 100), namespace: :general)
      end
  ...
  describe "statistics" do
    test "cache stats" do
      ...
      assert stats.hit_ratio > 0.7  # 3 hits out of 4 total requests
    end
  end
  ...
  describe "metadata" do
    test "metadata storage and retrieval" do
      ...
      System.put("test_key", "test_value", namespace: :general, metadata: metadata)
