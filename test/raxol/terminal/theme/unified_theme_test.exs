defmodule Raxol.Terminal.Theme.UnifiedThemeTest do
  use ExUnit.Case, async: false
  alias Raxol.Terminal.Theme.UnifiedTheme
  ...
  describe "load_theme/2" do
    test "loads a valid theme from file", %{pid: pid} do
      ...
      assert {:ok, theme_state} = UnifiedTheme.get_theme_state(theme_id)
    ...
    test "loads theme with custom options", %{pid: pid} do
      ...
      assert {:ok, theme_id} =
               UnifiedTheme.load_theme("test/fixtures/themes/dark.json", opts)
    ...
    test "returns error for invalid JSON", %{pid: pid} do
      ...
      assert {:error, {:invalid_theme_format, _}} =
               UnifiedTheme.load_theme(temp_file)
  ...
  describe "unload_theme/1" do
    test "unloads an existing theme", %{pid: pid} do
      ...
      assert {:ok, _theme_state} = UnifiedTheme.get_theme_state(theme_id)

      # Then unload it
      assert :ok = UnifiedTheme.unload_theme(theme_id)
  ...
  describe "get_themes/1" do
    test "returns all loaded themes", %{pid: pid} do
      ...
      {:ok, light_theme_id} =
        UnifiedTheme.load_theme("test/fixtures/themes/light.json")
    ...
    test "filters themes by status", %{pid: pid} do
      ...
      assert {:ok, themes} = UnifiedTheme.get_themes(status: :active)
      assert map_size(themes) == 1
      assert Map.has_key?(themes, dark_theme_id)

      assert {:ok, themes} = UnifiedTheme.get_themes(status: :inactive)
  ...
  describe "update_theme_config/2" do
    test "updates theme configuration", %{pid: pid} do
      ...
      assert :ok = UnifiedTheme.update_theme_config(theme_id, new_config)
    ...
    test "returns error for invalid config format", %{pid: pid} do
      ...
      assert {:error, :invalid_config_format} =
               UnifiedTheme.update_theme_config(theme_id, "not_a_map")
    end
  end
  ...
  describe "apply_theme/1" do
    test "applies an active theme", %{pid: pid} do
      ...
      assert :ok = UnifiedTheme.apply_theme(theme_id)
    ...
    test "returns error for inactive theme", %{pid: pid} do
      ...
      assert {:error, :theme_inactive} = UnifiedTheme.apply_theme(theme_id)
    end
    ...
    test "returns error for theme with error status", %{pid: pid} do
      ...
      assert {:error, :theme_error} = UnifiedTheme.apply_theme(theme_id)
    end
  end
  ...
  describe "preview_theme/1" do
    test "previews an active theme", %{pid: pid} do
      ...
      assert :ok = UnifiedTheme.preview_theme(theme_id)
    ...
    test "returns error for inactive theme", %{pid: pid} do
      ...
      assert {:error, :theme_inactive} = UnifiedTheme.preview_theme(theme_id)
    end
  end
  ...
  describe "export_theme/2" do
    test "exports theme to file", %{pid: pid} do
      ...
      assert :ok = UnifiedTheme.export_theme(theme_id, export_path)
    ...
    test "returns error for invalid export path", %{pid: pid} do
      ...
      assert {:error, {:file_write_error, :eisdir}} =
               UnifiedTheme.export_theme(theme_id, invalid_path)
    end
  end
  ...
  describe "import_theme/1" do
    ...
    test "returns error for invalid JSON", %{pid: pid} do
      ...
      assert {:error, {:invalid_theme_format, _}} =
               UnifiedTheme.import_theme(temp_file)
  ...
  describe "theme ID generation" do
    test "generates consistent IDs for same path", %{pid: pid} do
      ...
      {:ok, theme_id1} = UnifiedTheme.load_theme(path)
      {:ok, theme_id2} = UnifiedTheme.load_theme(path)
