defmodule Raxol.Terminal.Extension.UnifiedExtensionTest do
  ...
  setup do
    {:ok, _pid} = UnifiedExtension.start_link(
  ...
  describe "basic operations" do
    test "loads and unloads extensions" do
      # Load extension
      assert {:ok, extension_id} = UnifiedExtension.load_extension(
      ...
      # Get extension state
      assert {:ok, extension_state} = UnifiedExtension.get_extension_state(extension_id)
      ...
      assert {:error, :extension_not_found} = UnifiedExtension.get_extension_state(extension_id)
    end
    ...
    test "handles extension configuration" do
      ...
      assert :ok = UnifiedExtension.update_extension_config(extension_id, new_config)
  ...
  describe "extension activation" do
    ...
    test "handles invalid activation states" do
      ...
      assert {:error, :invalid_extension_state} = UnifiedExtension.deactivate_extension(extension_id)

      # Activate extension
      assert :ok = UnifiedExtension.activate_extension(extension_id)

      # Try to activate active extension
      assert {:error, :invalid_extension_state} = UnifiedExtension.activate_extension(extension_id)
    end
  end
  ...
  describe "extension commands" do
    test "executes extension commands" do
      ...
      # Execute command
      assert {:ok, result} = UnifiedExtension.execute_command(extension_id, "run", ["arg1", "arg2"])
      assert result =~ "Command 'run' executed with args:"

      # Try to execute non-existent command
      assert {:error, :command_not_found} = UnifiedExtension.execute_command(extension_id, "invalid")
    end
  end
  ...
  describe "extension management" do
    test "lists extensions with filters" do
      ...
      assert {:ok, theme_id} = UnifiedExtension.load_extension(
        "test/fixtures/extensions/theme",
        :theme,
        name: "Test Theme"
      )

      assert {:ok, script_id} = UnifiedExtension.load_extension(
        "test/fixtures/extensions/script",
        :script,
        name: "Test Script"
      )
      ...
      # Filter by type
      assert {:ok, theme_extensions} = UnifiedExtension.get_extensions(type: :theme)
      assert map_size(theme_extensions) == 1

      # Filter by status
      assert {:ok, idle_extensions} = UnifiedExtension.get_extensions(status: :idle)
    ...
    test "exports and imports extensions" do
      ...
      assert {:ok, original_state} = UnifiedExtension.get_extension_state(extension_id)
      assert {:ok, imported_state} = UnifiedExtension.get_extension_state(imported_id)
  ...
  describe "extension hooks" do
    test "registers and unregisters hooks" do
      ...
      assert :ok = UnifiedExtension.register_hook(extension_id, "init", callback)
    ...
    test "handles hook errors" do
      ...
      assert :ok = UnifiedExtension.register_hook(extension_id, "error", callback)
  ...
  describe "error handling" do
    test "handles invalid extension types" do
      assert {:error, :invalid_extension_type} = UnifiedExtension.load_extension(
    ...
    test "handles invalid configurations" do
      ...
      assert {:error, :invalid_extension_config} = UnifiedExtension.update_extension_config(
        extension_id,
        "invalid_config"
      )
    end
    ...
    test "handles invalid dependencies" do
      assert {:error, :invalid_extension_dependencies} = UnifiedExtension.load_extension(
        "test/fixtures/extensions/theme",
        :theme,
        name: "Test Theme",
    ...
    test "handles non-existent extensions" do
      assert {:error, :extension_not_found} = UnifiedExtension.get_extension_state("non_existent")
      assert {:error, :extension_not_found} = UnifiedExtension.unload_extension("non_existent")
      assert {:error, :extension_not_found} = UnifiedExtension.update_extension_config("non_existent", %{})
    end
    ...
    test "handles invalid hook names" do
      ...
      callback = fn args -> {:ok, args} end
      assert {:error, :hook_not_found} = UnifiedExtension.register_hook(extension_id, "invalid", callback)
    end
  end
  ...
  describe "extension types" do
    test "handles different extension types" do
      ...
      # Plugin extension
      assert {:ok, plugin_id} = UnifiedExtension.load_extension(
        "test/fixtures/extensions/plugin",
        :plugin,
        name: "Test Plugin"
      )
      ...
      # Custom extension
      assert {:ok, custom_id} = UnifiedExtension.load_extension(
      ...
      assert {:ok, script_state} = UnifiedExtension.get_extension_state(script_id)
      assert {:ok, plugin_state} = UnifiedExtension.get_extension_state(plugin_id)
      assert {:ok, custom_state} = UnifiedExtension.get_extension_state(custom_id)
