defmodule Raxol.System.DeltaUpdaterTest do
    ...
    def request(:get, {url, _headers}, _, _) do
      cond do
        String.contains?(url, "releases/tags/v1.2.0") ->
          ...
          """
          {:ok, {{:http, 200, 'OK'}, [{'content-type', 'application/json'}], body}}
        ...
        String.contains?(url, "releases/download") ->
          # Simulate download
          {:ok, {{:http, 200, 'OK'}, [{'content-type', 'application/octet-stream'}], "BINARY_DATA"}}
          
        true ->
          # Return error for any other URL
          {:error, {:failed_connect, [{:to_address, {~c"example.com", 80}}, {:inet, [:inet], :timeout}]}}
      end
    end
  end
  ...
  describe "check_delta_availability/1" do
    test "returns delta info when delta is available" do
      ...
      :meck.expect(:httpc, :request, fn method, url, opts1, opts2 -> MockHTTP.request(method, url, opts1, opts2) end)
