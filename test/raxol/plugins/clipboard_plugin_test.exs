defmodule Raxol.Plugins.ClipboardPluginTest do
  ...
  describe "clipboard plugin" do
    test "initializes with default configuration" do
      ...
      assert plugin.description == "Provides clipboard functionality for copying and pasting text"
    ...
    test "parses copy command correctly" do
      ...
      assert {:ok, "block", {10, 20}, {30, 40}} = 
        ClipboardPlugin.parse_copy_command("/copy block 10,20 30,40", "copy")
      ...
      assert {:error, _} = 
        ClipboardPlugin.parse_copy_command("/copy line 10,20 invalid", "copy")
    end
    ...
    test "handles selection start and end" do
      ...
      # Start selection
      {:ok, updated_plugin} = ClipboardPlugin.start_selection(plugin, "line", {10, 20})
      ...
      # Update selection
      {:ok, updated_plugin} = ClipboardPlugin.update_selection(updated_plugin, {30, 40})
      assert updated_plugin.selection_end == {30, 40}
      
      # End selection
      {:ok, updated_plugin} = ClipboardPlugin.end_selection(updated_plugin, {50, 60})
    ...
    test "handles mouse events for selection" do
      ...
      # Mouse down
      {:ok, updated_plugin} = ClipboardPlugin.handle_mouse(plugin, {:mouse_down, :left, 10, 20})
      assert updated_plugin.selection_start == {10, 20}
      assert updated_plugin.selection_end == {10, 20}
      assert updated_plugin.is_selecting == true
      
      # Mouse move
      {:ok, updated_plugin} = ClipboardPlugin.handle_mouse(updated_plugin, {:mouse_move, 30, 40})
      assert updated_plugin.selection_end == {30, 40}
      
      # Mouse up
      {:ok, updated_plugin} = ClipboardPlugin.handle_mouse(updated_plugin, {:mouse_up, :left, 50, 60})
      assert updated_plugin.selection_end == {50, 60}
      assert updated_plugin.is_selecting == false
    end
  end
