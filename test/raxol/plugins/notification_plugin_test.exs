defmodule Raxol.Plugins.NotificationPluginTest do
  ...
  describe "handle_command" do
    ...
    test "calls notify-send on Linux (Success)", %{
      ...
      assert {:ok, _, :notification_sent_linux} = NotificationPlugin.handle_command(args, state)
    end
    ...
    test "handles notify-send not found on Linux", %{current_state: state, args: args} do
      ...
      assert {:error, {:command_not_found, :notify_send}, _} = NotificationPlugin.handle_command(args, state)
    end
    ...
    test "handles notify-send command failure on Linux", %{
      ...
      # Call handle_command/2
      assert {:error, {:command_failed, 1, "Error output"}, _} = NotificationPlugin.handle_command(args, state)
    end
    ...
    test "handles unsupported OS", %{current_state: state, args: args} do
      ...
      # Call handle_command/2
      assert {:ok, _, :notification_skipped_unsupported_os} = NotificationPlugin.handle_command(args, state)
    end
  end
end
