defmodule Raxol.Plugins.PluginSystemTest do
  use ExUnit.Case
  alias Raxol.Plugins.{PluginManager, HyperlinkPlugin, ImagePlugin, ThemePlugin, SearchPlugin}
  ...
  describe "Plugin Manager" do
    ...
    test "unloads a plugin" do
      manager = PluginManager.new()
      {:ok, manager_with_plugin} = PluginManager.load_plugin(manager, HyperlinkPlugin)
      {:ok, updated_manager} = PluginManager.unload_plugin(manager_with_plugin, "hyperlink")
    ...
    test "enables a plugin" do
      manager = PluginManager.new()
      {:ok, manager_with_plugin} = PluginManager.load_plugin(manager, HyperlinkPlugin)
      {:ok, updated_manager} = PluginManager.disable_plugin(manager_with_plugin, "hyperlink")
      assert updated_manager.plugins["hyperlink"].enabled == false
      
      {:ok, enabled_manager} = PluginManager.enable_plugin(updated_manager, "hyperlink")
    ...
    test "processes output through plugins" do
      ...
      {:ok, updated_manager, transformed_output} = PluginManager.process_output(manager_with_plugin, "Visit https://example.com")
      assert String.contains?(transformed_output, "\e]8;;https://example.com")
      
      # Test with no URL in the output
      {:ok, updated_manager, _} = PluginManager.process_output(updated_manager, "Hello, World!")
    end

    test "processes input through plugins" do
      manager = PluginManager.new()
      {:ok, manager_with_plugin} = PluginManager.load_plugin(manager, SearchPlugin)
      
      # Test with a search command
      {:ok, updated_manager} = PluginManager.process_input(manager_with_plugin, "/search example")
    ...
    test "processes mouse events through plugins" do
      ...
      {:ok, updated_manager} = PluginManager.process_mouse(manager_with_plugin, {:click, 1, 2, 1})
      assert updated_manager.plugins["hyperlink"].enabled == true
    end
  end
  ...
  describe "Hyperlink Plugin" do
    ...
    test "detects and transforms URLs" do
      ...
      # Test with a URL
      {:ok, updated_plugin, transformed_output} = HyperlinkPlugin.handle_output(plugin, "Visit https://example.com")
      assert String.contains?(transformed_output, "\e]8;;https://example.com")
      
      # Test with no URL
      {:ok, updated_plugin} = HyperlinkPlugin.handle_output(updated_plugin, "Hello, World!")
    end
  end
  ...
  describe "Image Plugin" do
    ...
    test "detects and transforms image markers" do
      ...
      {:ok, updated_plugin, transformed_output} = ImagePlugin.handle_output(plugin, image_marker)
      assert String.contains?(transformed_output, "\e]1337;File=inline=1")
      
      # Test with no image marker
      {:ok, updated_plugin} = ImagePlugin.handle_output(updated_plugin, "Hello, World!")
    end
  end
  ...
  describe "Search Plugin" do
    ...
    test "handles search commands" do
      ...
      {:ok, updated_plugin} = SearchPlugin.handle_input(plugin, "/search example")
      assert updated_plugin.search_term == "example"
      
      # Test navigating search results
      {:ok, plugin_with_results} = %{updated_plugin | search_results: ["result1", "result2"]}
      {:ok, next_plugin} = SearchPlugin.handle_input(plugin_with_results, "/next")
