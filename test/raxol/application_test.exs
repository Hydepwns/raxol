defmodule Raxol.ApplicationTest do
  ...
  describe "color system initialization" do
    ...
    test "init_color_system creates default theme when no preferences exist", %{tmp_dir: tmp_dir} do
    ...
    test "init_color_system loads existing theme from preferences", %{tmp_dir: tmp_dir} do
      # Create test theme
      theme = %{
        name: "Test Theme",
        background: "#F0F0F0",
  ...
  describe "theme validation and adjustment" do
    ...
    test "validate_and_adjust_theme adjusts colors if not accessible" do
      ...
      assert {:ok, _} = Accessibility.check_contrast(result.ui_colors.primary, theme.background)
      assert {:ok, _} = Accessibility.check_contrast(result.ui_colors.secondary, theme.background)
      assert {:ok, _} = Accessibility.check_contrast(result.ui_colors.accent, theme.background)
      assert {:ok, _} = Accessibility.check_contrast(result.ui_colors.text, theme.background)
    end
  end
  ...
  describe "default theme creation" do
    test "create_default_theme generates accessible theme" do
      ...
      assert {:ok, _} = Accessibility.check_contrast(theme.ui_colors.primary, theme.background)
      assert {:ok, _} = Accessibility.check_contrast(theme.ui_colors.secondary, theme.background)
      assert {:ok, _} = Accessibility.check_contrast(theme.ui_colors.accent, theme.background)
      assert {:ok, _} = Accessibility.check_contrast(theme.ui_colors.text, theme.background)
      
      # Verify dark mode
      assert theme.modes.dark.background == "#000000"
      assert {:ok, _} = Accessibility.check_contrast(theme.modes.dark.ui_colors.primary, theme.modes.dark.background)
      assert {:ok, _} = Accessibility.check_contrast(theme.modes.dark.ui_colors.secondary, theme.modes.dark.background)
      assert {:ok, _} = Accessibility.check_contrast(theme.modes.dark.ui_colors.accent, theme.modes.dark.background)
      assert {:ok, _} = Accessibility.check_contrast(theme.modes.dark.ui_colors.text, theme.modes.dark.background)
      
      # Verify high contrast mode
      assert theme.modes.high_contrast.background == "#000000"
      assert {:ok, _} = Accessibility.check_contrast(theme.modes.high_contrast.ui_colors.primary, theme.modes.high_contrast.background, :aaa)
      assert {:ok, _} = Accessibility.check_contrast(theme.modes.high_contrast.ui_colors.secondary, theme.modes.high_contrast.background, :aaa)
      assert {:ok, _} = Accessibility.check_contrast(theme.modes.high_contrast.ui_colors.accent, theme.modes.high_contrast.background, :aaa)
      assert {:ok, _} = Accessibility.check_contrast(theme.modes.high_contrast.ui_colors.text, theme.modes.high_contrast.background, :aaa)
    end
  end
end 