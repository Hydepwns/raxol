defmodule Raxol.Audit.LoggerTest do
  ...
  describe "authentication logging" do
    test "logs successful authentication" do
      assert :ok = Logger.log_authentication("testuser", :password, :success,
    ...
    test "logs failed authentication with reason" do
      assert :ok = Logger.log_authentication("testuser", :password, :failure,
    ...
    test "logs locked account" do
      assert :ok = Logger.log_authentication("testuser", :password, :locked,
  ...
  describe "authorization logging" do
    test "logs successful authorization" do
      ...
      assert :ok = Logger.log_authorization("user123", resource, :read, :granted,
    ...
    test "logs denied authorization" do
      ...
      assert :ok = Logger.log_authorization("user123", resource, :access, :denied,
  ...
  describe "data access logging" do
    test "logs data read operation" do
      assert :ok = Logger.log_data_access("user123", :read, "customer_records",
        records_count: 50,
    ...
    test "logs sensitive data export" do
      assert :ok = Logger.log_data_access("user123", :export, "financial_data",
        records_count: 1000,
    ...
    test "logs data deletion" do
      assert :ok = Logger.log_data_access("user123", :delete, "user_profiles",
        records_count: 1,
  ...
  describe "configuration change logging" do
    test "logs configuration creation" do
      assert :ok = Logger.log_configuration_change("admin123", "security", 
        "new_setting", nil, "enabled",
        approval_required: false
      )
    end

    test "logs configuration update with approval" do
      assert :ok = Logger.log_configuration_change("admin123", "auth",
    ...
    test "logs configuration deletion" do
      assert :ok = Logger.log_configuration_change("admin123", "features",
        "legacy_mode", true, nil,
  ...
  describe "security event logging" do
    test "logs critical security event" do
      assert :ok = Logger.log_security_event(:intrusion_detected, :critical,
    ...
    test "logs medium severity event" do
      assert :ok = Logger.log_security_event(:suspicious_activity, :medium,
  ...
  describe "terminal operation logging" do
    test "logs command execution" do
      assert :ok = Logger.log_terminal_operation("user123", "term001", 
        :command_executed,
        command: "ls -la",
        exit_code: 0,
        duration_ms: 25
  ...
  describe "compliance logging" do
    test "logs compliant audit" do
      assert :ok = Logger.log_compliance(:soc2, "CC6.1", :audit_performed, 
        :compliant,
    ...
    test "logs non-compliant finding" do
      assert :ok = Logger.log_compliance(:hipaa, "164.312", :audit_performed,
        :non_compliant,
  ...
  describe "privacy request logging" do
    test "logs GDPR access request" do
      assert :ok = Logger.log_privacy_request("subject123", :access_request,
        :completed,
    ...
    test "logs deletion request" do
      assert :ok = Logger.log_privacy_request("subject456", :deletion_request,
        :in_progress,
        data_categories: ["all"],
    ...
    test "logs portability request with cross-border transfer" do
      assert :ok = Logger.log_privacy_request("subject789", :portability_request,
        :completed,
  ...
  describe "buffering and flushing" do
    ...
    test "flushes immediately on critical events" do
      Logger.log_security_event(:critical_breach, :critical,
        "System compromised")
