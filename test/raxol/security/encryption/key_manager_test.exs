defmodule Raxol.Security.Encryption.KeyManagerTest do
  ...
  setup do
    ...
    {:ok, _pid} = KeyManager.start_link(config: %{
      rotation_days: 1,
      cache_ttl_ms: 100,
      use_hsm: false
    })
  ...
  describe "generate_dek/3" do
    test "generates a new data encryption key" do
      ...
      refute Map.has_key?(key, :key_material)  # Should be sanitized
      assert key.created_at != nil
      assert key.expires_at != nil
    end
    
    test "generates key with custom algorithm" do
      {:ok, key} = KeyManager.generate_dek("test", algorithm: :chacha20_poly1305)
  ...
  describe "get_key/3" do
    test "retrieves a key by ID" do
      ...
      assert Map.has_key?(retrieved, :key_material)  # Should have key material
    end
