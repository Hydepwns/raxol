defmodule Raxol.Animation.FrameworkTest do
  ...
  describe "Animation Framework" do
    ...
    test "creates animation with default settings" do
      animation = Framework.create_animation(:test_animation, %{
        type: :fade,
        from: 0,
        to: 1
      })
      ...
      assert animation.duration == 300  # Default duration
      assert animation.easing == :linear  # Default easing
      assert animation.from == 0
      assert animation.to == 1
    end
    
    test "creates animation with custom settings" do
      animation = Framework.create_animation(:custom_animation, %{
        type: :slide,
        duration: 500,
    ...
    test "applies animation values to state" do
      ...
      animation = Framework.create_animation(:fade_in, %{
        type: :fade,
        from: 0,
        to: 1
      })
    ...
    test "handles multiple animations on same element" do
      ...
      fade_animation = Framework.create_animation(:fade_in, %{
        type: :fade,
        from: 0,
        to: 1
      })
      
      slide_animation = Framework.create_animation(:slide_in, %{
        type: :slide,
        from: 0,
        to: 100
      })
