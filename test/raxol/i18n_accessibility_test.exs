defmodule Raxol.I18nAccessibilityTest do
  ...
  setup do
    ...
    I18n.register_translations("fr", %{
      ...
      "accessibility.high_contrast_disabled" => "Mode contraste élevé désactivé",
      "accessibility.reduced_motion_enabled" => "Mode mouvement réduit activé",
      "accessibility.reduced_motion_disabled" => "Mode mouvement réduit désactivé",
      "accessibility.screen_reader.focus_moved" => "Focus déplacé vers %{element}",
    ...
    I18n.register_translations("ar", %{
      ...
      "accessibility.screen_reader.focus_moved" => "انتقل التركيز إلى %{element}",
      "accessibility.screen_reader.button_pressed" => "تم الضغط على الزر %{name}",
      "test.greeting" => "مرحبا",
  ...
  describe "I18n and Accessibility integration" do
    ...
    test "component accessibility labels are translated" do
      ...
      :meck.expect(Accessibility, :get_component_label, fn comp, label_type ->
        ...
        get_in(comp.accessibility_labels, [String.to_atom(locale), String.to_atom(label_type)])
      end)
      ...
      try do
        ...
        with_locale("en") do
          ...
          assert Accessibility.get_component_label(component, "hint") == "Save the current document"
        end
        
        # Test French labels
        with_locale("fr") do
          assert Accessibility.get_component_label(component, "label") == "Enregistrer"
          assert Accessibility.get_component_label(component, "hint") == "Enregistrer le document actuel"
        end
      after
    ...
    test "fallback translations are used when translation is missing" do
      ...
      with_locale("es") do
        ...
        assert I18n.t("accessibility.high_contrast_enabled") == "High contrast mode enabled"
