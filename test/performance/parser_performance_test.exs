defmodule Raxol.Performance.ParserPerformanceTest do
  ...
  describe "parser performance benchmarks" do
    ...
    test "plain text parsing performance regression guard", %{
      emulator: emulator
    } do
      text = "Hello World"
    ...
    test "single character parsing should be under 50 microseconds", %{
      emulator: emulator
    } do
      {time_us, _} =
        :timer.tc(fn ->
      ...
      assert avg_us < 500,
             "Single char parsing took #{Float.round(avg_us, 2)} μs/op, expected < 500 μs"
    end

    test "empty string parsing should be essentially free", %{
      emulator: emulator
    } do
      {time_us, _} =
        :timer.tc(fn ->
    ...
    test "ANSI color sequence parsing should be under 1500 microseconds", %{
      emulator: emulator
    } do
    ...
    test "complex ANSI sequences should be under 2000 microseconds", %{
      emulator: emulator
    } do
