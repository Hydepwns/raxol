defmodule Raxol.Core.Runtime.Plugins.DependencyManager.VersionTest do
  ...
  describe "check_version/2" do
    ...
    test "handles exact version matches" do
      assert :ok = Version.check_version("1.0.0", "== 1.0.0")
      assert {:error, :version_mismatch} = Version.check_version("1.0.1", "== 1.0.0")
      assert {:error, :version_mismatch} = Version.check_version("1.1.0", "== 1.0.0")
    end
    ...
    test "handles version ranges" do
      ...
      assert {:error, :version_mismatch} = Version.check_version("2.0.0", "~> 1.0")
    end
    ...
    test "handles complex version constraints" do
      ...
      assert {:error, :version_mismatch} = Version.check_version("2.0.0", ">= 1.0.0 and < 2.0.0")
      assert {:error, :version_mismatch} = Version.check_version("0.9.0", ">= 1.0.0 and < 2.0.0")
    end
    ...
    test "handles invalid version formats" do
      assert {:error, :invalid_version} = Version.check_version("invalid", ">= 1.0.0")
      assert {:error, :invalid_version} = Version.check_version("1", ">= 1.0.0")
      assert {:error, :invalid_version} = Version.check_version("1.0", ">= 1.0.0")
    end

    test "handles invalid requirement formats" do
      assert {:error, :invalid_requirement} = Version.check_version("1.0.0", "invalid")
      assert {:error, :invalid_requirement} = Version.check_version("1.0.0", ">")
      assert {:error, :invalid_requirement} = Version.check_version("1.0.0", ">= ")
    end
  end
  ...
  describe "parse_version_requirement/1" do
    ...
    test "parses complex version requirements" do
      assert {:ok, _} = Version.parse_version_requirement(">= 1.0.0 and < 2.0.0")
      assert {:ok, _} = Version.parse_version_requirement(">= 1.0.0 || >= 2.0.0")
    end
