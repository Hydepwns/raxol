defmodule Raxol.Core.Runtime.Plugins.DependencyManager.GraphTest do
  ...
  describe "build_dependency_graph/1" do
    test ~c"builds graph from plugin metadata" do
      plugins = %{
        "plugin_a" => %{
    ...
    test ~c"handles simple plugin IDs" do
      plugins = %{
    ...
    test ~c"handles empty plugin map" do
  ...
  describe "build_dependency_chain/2" do
    test ~c"builds chain for circular dependencies" do
      graph = %{
    ...
    test ~c"handles single-node cycles" do
      graph = %{
  ...
  describe "get_all_dependencies/3" do
    test ~c"gets all dependencies for a plugin" do
      graph = %{
        "plugin_a" => [{"plugin_b", ">= 1.0.0", %{optional: false}}],
    ...
    test ~c"handles plugins not in the graph" do
      graph = %{
        "plugin_a" => []
      }
    ...
    test ~c"handles complex dependency chains" do
      graph = %{
        "plugin_a" => [
