defmodule Raxol.Property.UIComponentTest do
  ...
  describe "Button component properties" do
    property "button renders with any valid text" do
      check all text <- string(:printable, min_length: 1, max_length: 50),
                enabled <- boolean(),
                max_runs: 500 do
        button = Button.new(text: text, enabled: enabled)
        
        # Should have valid structure
        assert button.text == text
        assert button.enabled == enabled
    ...
    property "button click handling is deterministic" do
      check all clicks <- integer(0..100),
                max_runs: 200 do
    ...
    property "button styling properties are preserved" do
      check all color <- rgb_color_generator(),
  ...
  describe "TextInput component properties" do
    property "text input handles any valid input" do
      check all initial <- string(:printable, max_length: 100),
    ...
    property "text input respects max_length constraint" do
      check all max_length <- integer(1..100),
                text <- string(:printable, min_length: max_length + 1, max_length: 200),
                max_runs: 500 do
    ...
    property "text input validation is consistent" do
      check all text <- string(:alphanumeric, max_length: 50),
                max_runs: 500 do
        # Numeric validator
        validator = fn value -> Regex.match?(~r/^\d*$/, value) end
        
        input = TextInput.new(value: "", validator: validator)
        updated = TextInput.handle_input(input, text)
    ...
    property "cursor position stays within bounds" do
      check all text <- string(:printable, min_length: 1, max_length: 100),
                cursor_moves <- list_of(cursor_movement_generator(), max_length: 50),
                max_runs: 500 do
        input = TextInput.new(value: text)
        
        final = Enum.reduce(cursor_moves, input, fn move, acc ->
  ...
  describe "Flexbox layout properties" do
    property "flexbox maintains child order" do
      check all children <- list_of(integer(1..100), min_length: 1, max_length: 20),
                max_runs: 500 do
        layout = Flexbox.new(
          children: Enum.map(children, &create_test_component/1)
        )
    ...
    property "flexbox respects size constraints" do
      check all width <- integer(10..1000),
    ...
    property "flex properties distribute space correctly" do
      check all flex_values <- list_of(integer(0..10), min_length: 2, max_length: 5),
                container_size <- integer(100..1000),
                max_runs: 500 do
        children = Enum.map(flex_values, fn flex ->
          %{id: flex, flex: flex}
        end)
  ...
  describe "Grid layout properties" do
    property "grid places items in correct cells" do
      check all rows <- integer(1..10),
                cols <- integer(1..10),
                items <- list_of(grid_item_generator(rows, cols), max_length: rows * cols),
                max_runs: 500 do
        grid = Grid.new(
          rows: rows,
          columns: cols,
  ...
  describe "State Store properties" do
    property "store updates are atomic" do
      check all initial <- map_of(atom(:alphanumeric), integer()),
                updates <- list_of(store_update_generator(), max_length: 100),
                max_runs: 200 do
        {:ok, store} = Store.start_link(initial_state: initial)
    ...
    property "store subscriptions receive all updates" do
      check all updates <- list_of(store_update_generator(), min_length: 1, max_length: 50),
                max_runs: 200 do
    ...
    property "store handles concurrent updates safely" do
      check all update_count <- integer(10..100),
                max_runs: 100 do
        {:ok, store} = Store.start_link(initial_state: %{counter: 0})
        
        # Spawn concurrent updaters
        tasks = for i <- 1..update_count do
          Task.async(fn ->
  ...
  describe "Component composition properties" do
    ...
    property "component lifecycle hooks execute in order" do
      check all hook_count <- integer(1..10),
                max_runs: 200 do
        order = :ets.new(:order, [:ordered_set, :public])
        
        hooks = for i <- 1..hook_count do
  ...
  defp grid_item_generator(max_row, max_col) do
    gen all row <- integer(1..max_row),
            col <- integer(1..max_col) do
      %{row: row, column: col, content: "Item"}
    end
  end

  defp store_update_generator do
    gen all key <- atom(:alphanumeric),
            value <- one_of([integer(), string(:alphanumeric), boolean()]) do
      {key, value}
    end
  end
  ...
  defp calculate_tree_size(depth, breadth) do
    ...
    if breadth == 1 do
      ...
      div(:math.pow(breadth, depth) - 1, breadth - 1)
    end
  end
