defmodule Raxol.Terminal.ANSI.AutoRepeatTest do
  ...
  describe "auto-repeat mode" do
    test "switching auto-repeat mode on", %{terminal: terminal} do
      ...
      assert Raxol.Terminal.ANSI.ScreenModes.get_auto_repeat_mode(new_terminal.mode_state) == true
    end
    ...
    test "switching auto-repeat mode off", %{terminal: terminal} do
      ...
      assert Raxol.Terminal.ANSI.ScreenModes.get_auto_repeat_mode(new_terminal.mode_state) == false
    end

    test "auto-repeat mode is saved and restored when switching screen modes", %{terminal: terminal} do
      # First switch auto-repeat mode on
      terminal = ANSI.process_escape_sequence(terminal, "\x1b[?8h")
      ...
      assert Raxol.Terminal.ANSI.ScreenModes.get_auto_repeat_mode(terminal.mode_state) == true
