defmodule Raxol.Terminal.ANSI.InterlacingTest do
  ...
  describe "interlacing mode" do
    test "switching interlacing mode on", %{terminal: terminal} do
      ...
      assert Raxol.Terminal.ANSI.ScreenModes.get_interlacing_mode(new_terminal.mode_state) == true
    end
    ...
    test "switching interlacing mode off", %{terminal: terminal} do
      ...
      assert Raxol.Terminal.ANSI.ScreenModes.get_interlacing_mode(new_terminal.mode_state) == false
    end

    test "interlacing mode is saved and restored when switching screen modes", %{terminal: terminal} do
      # First switch interlacing mode on
      terminal = ANSI.process_escape_sequence(terminal, "\x1b[?9h")
      ...
      assert Raxol.Terminal.ANSI.ScreenModes.get_interlacing_mode(terminal.mode_state) == true
