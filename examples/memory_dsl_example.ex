defmodule Raxol.Examples.MemoryDSLExample do
  ...
  memory_benchmark "Terminal Operations Memory Test" do
    ...
    assert_memory_peak :small_terminal, less_than: 5_000_000      # 5MB
    assert_memory_peak :large_terminal, less_than: 150_000_000   # 150MB
    assert_memory_peak :buffer_operations, less_than: 50_000_000 # 50MB
    assert_memory_peak :ansi_processing, less_than: 10_000_000   # 10MB
    assert_memory_peak :memory_stress_test, less_than: 200_000_000 # 200MB

    # Sustained memory usage assertions (75th percentile)
    assert_memory_sustained :small_terminal, less_than: 3_000_000      # 3MB
    assert_memory_sustained :large_terminal, less_than: 100_000_000    # 100MB
    assert_memory_sustained :buffer_operations, less_than: 30_000_000  # 30MB
    assert_memory_sustained :ansi_processing, less_than: 8_000_000     # 8MB

    # Garbage collection pressure assertions
    assert_gc_pressure :small_terminal, less_than: 5       # Max 5 GC collections
    assert_gc_pressure :large_terminal, less_than: 20      # Max 20 GC collections
    assert_gc_pressure :buffer_operations, less_than: 15   # Max 15 GC collections
    assert_gc_pressure :memory_stress_test, less_than: 50  # Max 50 GC collections

    # Memory efficiency assertions (higher is better)
    assert_memory_efficiency :small_terminal, greater_than: 0.8     # 80% efficiency
    assert_memory_efficiency :large_terminal, greater_than: 0.6     # 60% efficiency
    assert_memory_efficiency :buffer_operations, greater_than: 0.7  # 70% efficiency
    assert_memory_efficiency :ansi_processing, greater_than: 0.9    # 90% efficiency

    # Memory regression detection compared to baseline
    assert_no_memory_regression baseline: "v1.4.0", threshold: 0.1  # 10% regression tolerance
  end
  ...
  defp perform_buffer_operations do
    ...
    updated_buffer = Enum.map(buffer, fn row ->
    ...
    scrolled_buffer = case updated_buffer do
      [] -> []
      [_first | rest] ->
        empty_row = for _col <- 1..80 do
    ...
    copied_sections = for _i <- 1..10 do
  ...
  defp process_ansi_sequences do
    ...
    sequences = [
      "\e[2J",                    # Clear screen
      "\e[1;1H",                  # Cursor to home
      "\e[31mRed text\e[0m",      # Red colored text
      "\e[1mBold text\e[0m",      # Bold text
      "\e[4mUnderlined\e[0m",     # Underlined text
      "\e[7mInverted\e[0m",       # Inverted colors
      "\e[?1049h",                # Enable alternative buffer
      "\e[?1049l",                # Disable alternative buffer
      "\e[38;5;196mBright red\e[0m", # 256-color mode
      "\e[48;5;21mBlue bg\e[0m",  # 256-color background
      "\e[38;2;255;128;0mRGB\e[0m" # True color mode
    ]

    # Process each sequence multiple times
    processed = for sequence <- sequences do
    ...
    result_buffer = for {sequence, index} <- Enum.with_index(sequences) do
      %{
        sequence: sequence,
        processed_count: 100,
  ...
  defp perform_memory_stress_test do
    ...
    large_data_chunks = for chunk_id <- 1..1000 do
      chunk_data = for _byte <- 1..1024 do
        :rand.uniform(255)
      end
    ...
    processed_chunks = large_data_chunks
    |> Enum.chunk_every(100)
    |> Enum.map(fn chunk_group ->
      # Process each group
      processed = Enum.map(chunk_group, fn chunk ->
      ...
      avg_checksum = Enum.sum_by(processed, fn chunk -> chunk.metadata.checksum end) / length(processed)
    ...
    %{
      ...
      avg_group_size: Enum.sum_by(processed_chunks, & &1.group_size) / length(processed_chunks)
    }
  end
  ...
  defp extract_color_info(sequence) do
    cond do
      String.contains?(sequence, "31m") -> {:fg_color, :red}
      String.contains?(sequence, "32m") -> {:fg_color, :green}
      String.contains?(sequence, "38;5;") -> {:fg_color_256, extract_256_color(sequence)}
      String.contains?(sequence, "38;2;") -> {:fg_color_rgb, extract_rgb_color(sequence)}
      String.contains?(sequence, "48;5;") -> {:bg_color_256, extract_256_color(sequence)}
      String.contains?(sequence, "48;2;") -> {:bg_color_rgb, extract_rgb_color(sequence)}
      String.contains?(sequence, "1m") -> {:style, :bold}
      String.contains?(sequence, "4m") -> {:style, :underline}
      String.contains?(sequence, "7m") -> {:style, :reverse}
      String.contains?(sequence, "0m") -> {:reset, :all}
      true -> {:unknown_color, sequence}
    end
  end
  ...
  defp extract_rgb_color(sequence) do
    ...
    case Regex.run(~r/38;2;(\d+);(\d+);(\d+)/, sequence) do
      [_, r_str, g_str, b_str] ->
        {String.to_integer(r_str), String.to_integer(g_str), String.to_integer(b_str)}
      _ ->
        {0, 0, 0}
    end
  end
  ...
  defp print_example_report(report) do
    ...
    IO.puts("  Fragmentation ratio: #{Float.round(analysis.fragmentation_ratio, 3)}")
