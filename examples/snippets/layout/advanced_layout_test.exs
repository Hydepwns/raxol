...
defmodule Raxol.Docs.Guides.Examples.Layout.AdvancedLayoutTest do
  ...
  def view(_model) do
    view do
      column(gap: 20, padding: 10, style: "border: 1px solid #ccc; width: 400px;") do
        # Example 1: Centered Row
        text(content: "Row with centered items:", style: "font-weight: bold;")
        row(gap: 10, justify: :center, align: :center, style: "border: 1px dashed blue; padding: 5px;") do
        ...
        text(content: "Row with space-between items:", style: "font-weight: bold;")
        row(gap: 10, justify: :space_between, style: "border: 1px dashed green; padding: 5px;") do
          text(content: "Left Item")
          text(content: "Right Item")
        end

        # Example 3: Nested Layout with Flex
        text(content: "Nested Column/Row with Flex:", style: "font-weight: bold;")
        row(gap: 15, align: :start, style: "border: 1px dashed red; padding: 5px;") do
          # Fixed width column
          column(gap: 5, style: "border: 1px solid orange; padding: 5px; width: 100px;") do
          ...
          column(gap: 10, flex: 1, style: "border: 1px solid purple; padding: 5px;") do
            text(content: "Main Content Area", style: "font-weight: bold; text-align: center;")
            row(gap: 5, justify: :space_around) do
              text_input(placeholder: "Input 1")
              text_input(placeholder: "Input 2")
            end
            panel(title: "Inner Panel") do
              text(content: "Content inside a panel within the flexible column.")
            end
          end
        end

        # Example 4: Column Alignment
        text(content: "Column with different alignments:", style: "font-weight: bold;")
        row(gap: 10, style: "border: 1px dashed teal; padding: 5px; height: 100px; align-items: stretch;") do
          column(align: :start, flex: 1, style: "border: 1px solid gray; padding: 3px;") do
            text(content: "Align Start")
            button(content: "S")
          end
          column(align: :center, flex: 1, style: "border: 1px solid gray; padding: 3px;") do
            text(content: "Align Center")
            button(content: "C")
          end
           column(align: :end, flex: 1, style: "border: 1px solid gray; padding: 3px;") do
