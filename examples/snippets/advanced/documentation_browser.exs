...
defmodule DocumentationBrowser do
  ...
  def init(_context) do
    ...
    modules = Code.available_modules()
    |> Enum.sort()

    model = %{
      content: "Loading...",
      content_lines: [], # Store content as lines for easier scrolling
      scroll_offset: 0, # Top visible line index
      module_cursor: 0,
      modules: modules,
      view_height: 20 # Default/Estimate, could be updated via resize events
    }
  ...
  def update(message,
    ...
    Logger.debug("DocumentationBrowser: update/2 received message: \#{inspect(message)}")
    case message do
      ...
      %Event{type: :key, data: %{key: :key_up}} ->
        ...
        new_model = %{model | module_cursor: new_cursor, scroll_offset: 0, content: "Loading..."}
      ...
      {:command_result, :content_updated, content} ->
        ...
        {:ok, %{model | content: content, content_lines: lines, scroll_offset: 0}, []}

      # TODO: Handle :resize event to update view_height for scrolling
  ...
  def view(%{view_height: view_height} = model) do
    ...
    view do
      ...
      column style: %{gap: 0} do # Use column for header + main row
        box style: %{width: :fill, padding: [0, 1]} do
            text(content: @header, style: %{color: :blue})
        end
        row style: %{height: :fill} do # Main content row
          box title: "Modules", style: [[:width, "30%"], [:height, :fill], [:border, :single]] do
            # Basic list rendering, could use a list component if available
            # Highlight selected module
            module_list_content = for {module, idx} <- Enum.with_index(model.modules) do
          ...
          box title: inspect(Enum.at(model.modules, model.module_cursor)), style: [[:width, "70%"], [:height, :fill], [:border, :single]] do
            # Implement scrolling by slicing lines
            visible_lines = Enum.slice(model.content_lines, model.scroll_offset, view_height)
  ...
  defp update_cmd(model) do
    Logger.debug("DocumentationBrowser: Creating content update command for module index \#{model.module_cursor}")
    # Pass only needed data to the command function
    module_index = model.module_cursor
    modules = model.modules
    Command.new(fn -> fetch_content(module_index, modules) end, :content_updated)
  end

  defp fetch_content(module_index, modules) do
    selected = Enum.at(modules, module_index)
    Logger.debug("DocumentationBrowser: Fetching docs for \#{inspect(selected)}")
    # Use Code.get_docs for beam chunks
    case Code.get_docs(selected, :docs) do
      {:docs_v1, _annotation, :elixir, _format, %{"en" => docs}, _meta} ->
         docs
      {:docs_v1, _annotation, :elixir, _format, docs_map, _meta} when is_map(docs_map) ->
    ...
    kind, reason ->
        IO.puts("Error fetching docs for \#{inspect(selected)}: \#{kind} - \#{inspect(reason)}")
        "(Error fetching documentation for \#{inspect(selected)})"
  end
end
