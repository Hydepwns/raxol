...
A full frame uses 13% of the 60fps budget, leaving plenty of room for application logic. The Unix/macOS backend uses a termbox2 NIF; Windows uses a pure Elixir driver (usable, not yet tuned). See the [benchmark suite](docs/bench/README.md) for methodology and platform details.
