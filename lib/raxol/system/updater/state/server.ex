defmodule Raxol.System.Updater.State.Server do
  ...
  def handle_call(:clear_update_history, _from, state) do
    history_file = Path.join(state.settings.download_path, "update_history.json")
  ...
  def handle_call(:get_update_log, _from, state) do
    ...
    result = case File.read(log_file) do
  ...
  def handle_call(:clear_update_log, _from, state) do
    ...
    result = case File.write(log_file, "") do
      :ok -> :ok
      {:error, reason} -> {:error, reason}
    end
  ...
  @impl true
  def handle_call({:set_auto_check, enabled}, _from, state) when is_boolean(enabled) do
