    ...
    case :os.type() do
      {:unix, :darwin} ->
        ...
        case System.cmd("pbcopy", [], input: text, stderr_to_stdout: true) do
          {_output, 0} ->
            :ok
          {output, exit_code} ->
            Logger.error("Failed to copy using pbcopy. Exit code: #{exit_code}, Output: #{output}")
      ...
      {:unix, _} ->
        ...
        case System.find_executable("xclip") do
          nil ->
            Logger.error("Clipboard error: `xclip` command not found. Please install it for clipboard support.")
          ...
          _ ->
            case System.cmd("xclip", ["-selection", "clipboard"], input: text, stderr_to_stdout: true) do
              {_output, 0} ->
                :ok
              {output, exit_code} ->
                Logger.error("Failed to copy using xclip. Exit code: #{exit_code}, Output: #{output}")
      ...
      {:win32, _} ->
        ...
        case System.cmd("clip", [], input: text, stderr_to_stdout: true) do
          {_output, 0} ->
            :ok
          {output, exit_code} ->
            Logger.error("Failed to copy using clip. Exit code: #{exit_code}, Output: #{output}")
    ...
    case :os.type() do
      {:unix, :darwin} ->
        ...
        case System.cmd("pbpaste", [], stderr_to_stdout: true) do
          {output, 0} ->
            {:ok, String.trim(output)} # pbpaste might add newline
          {output, exit_code} ->
            Logger.error("Failed to paste using pbpaste. Exit code: #{exit_code}, Output: #{output}")
        ...
        case System.find_executable("xclip") do
          ...
          _ ->
            case System.cmd("xclip", ["-selection", "clipboard", "-o"], stderr_to_stdout: true) do
              {output, 0} ->
                {:ok, output} # xclip -o usually includes a newline, handled by caller if needed
              {output, exit_code} ->
                # Exit code 1 can mean empty clipboard, which isn't an error for paste
                if exit_code == 1 and String.trim(output) == "" do
                  {:ok, ""}
                else
                  Logger.error("Failed to paste using xclip. Exit code: #{exit_code}, Output: #{output}")
                  {:error, {:xclip_failed, output}}
                end
            end
        end
      ...
      {:win32, _} ->
        ...
        case System.cmd("powershell", ["-noprofile", "-command", "Get-Clipboard"], stderr_to_stdout: true) do
          {output, 0} ->
...
")}
          {output, exit_code} ->
            ...
            if String.contains?(output, ["Cannot retrieve the Clipboard.", "Get-Clipboard: Failed to get clipboard content"]) do
               Logger.debug("Clipboard appears empty or inaccessible via PowerShell.")
               {:ok, ""} # Treat as empty clipboard
            else
              Logger.error("Failed to paste using PowerShell. Exit code: #{exit_code}, Output: #{output}")
