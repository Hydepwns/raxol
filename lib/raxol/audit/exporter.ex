defmodule Raxol.Audit.Exporter do
  ...
  """
  def generate_compliance_report(exporter \\ __MODULE__, framework, time_range, opts \\ []) do
    GenServer.call(exporter, {:generate_compliance_report, framework, time_range, opts}, 120_000)
  end
  ...
  @impl GenServer
  def handle_call({:generate_compliance_report, framework, time_range, opts}, _from, state) do
  ...
  defp calculate_syslog_priority(event) do
    facility = 16  # Local0
    severity = case Map.get(event, :severity, :info) do
      :critical -> 2  # Critical
      :high -> 3      # Error
      :medium -> 4    # Warning
      :low -> 5       # Notice
  ...
  defp record_export(format, filters, state) do
    ...
    %{state | export_history: [export_record | Enum.take(state.export_history, 99)]}
  end
  ...
  defp count_access_events(events) do
    Enum.count(events, &(Map.get(&1, :event_type) in [:authentication, :authorization]))
  end
