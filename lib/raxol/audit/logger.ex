defmodule Raxol.Audit.Logger do
  ...
  def log_data_access(user_id, operation, resource_type, opts \\ []) do
    ...
    severity = determine_data_severity(operation, Keyword.get(opts, :data_classification))
  ...
  """
  def log_configuration_change(user_id, component, setting, old_value, new_value, opts \\ []) do
  ...
  def verify_integrity(start_time, end_time) do
    GenServer.call(__MODULE__, {:verify_integrity, start_time, end_time}, 60_000)
  end
  ...
  def init(config) do
    ...
    if config.enabled do
      ...
      :timer.send_interval(3_600_000, :cleanup_old_logs)  # Every hour
      :timer.send_interval(86_400_000, :verify_integrity)  # Daily
    end
  ...
  defp process_event(event, category, severity, state) do
    ...
    signed_event = if state.config.sign_events do
    ...
    final_event = if state.config.encrypt_events do
    ...
    should_flush = severity == :critical or length(new_buffer) >= state.config.buffer_size
  ...
  defp sign_event(event, state) do
    signature = :crypto.mac(:hmac, :sha256, state.encryption_key, :erlang.term_to_binary(event))
    |> Base.encode64()
  ...
  defp encrypt_event(event, state) do
    ...
    encrypted_data = :crypto.crypto_one_time(:aes_256_gcm, state.encryption_key, 
                                             generate_iv(), :erlang.term_to_binary(event), true)
  ...
  defp flush_buffer_to_storage(state) do
    if Enum.empty?(state.buffer) do
      ...
      case EventStore.append_events(state.event_store, events_to_flush, stream_name) do
        {:ok, _event_ids} ->
  ...
  defp determine_terminal_severity(:command_executed, command) do
    dangerous_commands = ["rm -rf", "sudo", "chmod 777", "curl | sh", "wget | sh"]
  ...
  defp cleanup_expired_logs(state) do
    cutoff_time = System.system_time(:millisecond) - (state.retention_policy.retention_days * 86_400_000)
  ...
  defp verify_event_signatures(events, state) do
    invalid = Enum.filter(events, fn event ->
      case Map.get(event, :signature) do
        nil -> false  # Unsigned events are allowed if signing is disabled
        signature ->
          event_without_sig = Map.delete(event, :signature)
          expected_sig = :crypto.mac(:hmac, :sha256, state.encryption_key, 
  ...
  defp verify_daily_integrity(state) do
    ...
    start_time = end_time - 86_400_000  # 24 hours ago
  ...
  defp calculate_statistics(state, :last_24_hours) do
    %{
      ...
      uptime_hours: div(System.system_time(:millisecond) - state.metrics.start_time, 3_600_000),
