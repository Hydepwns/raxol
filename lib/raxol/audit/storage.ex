defmodule Raxol.Audit.Storage do
  ...
  def init(config) do
    ...
    :timer.send_interval(3_600_000, :rotate_file)  # Hourly
  ...
  defp store_events(events, state) do
    # Update indexes
    new_indexes = Enum.reduce(events, state.indexes, fn event, acc_indexes ->
  ...
  defp write_events_to_file(events, state) do
    ...
    # Serialize events
    lines = Enum.map(events, fn event ->
      Jason.encode!(event) <> "\n"
    end)
  ...
  defp execute_query(filters, opts, state) do
    try do
      ...
      base_events = if Map.has_key?(filters, :user_id) and Map.has_key?(state.indexes.user_id, filters.user_id) do
      ...
      # Apply sorting
      sorted = sort_events(filtered, Keyword.get(opts, :sort_by, :timestamp),
  ...
  defp filter_by_resource(events, %{resource_type: type, resource_id: id}) do
    Enum.filter(events, fn event ->
      Map.get(event, :resource_type) == type and Map.get(event, :resource_id) == id
    end)
  end
  ...
  defp delete_old_events(timestamp, state) do
    ...
    {to_delete, to_keep} = Enum.split_with(all_events, &(&1.timestamp < timestamp))
  ...
  defp update_index(indexes, field, event) do
    case Map.get(event, field) do
      ...
      value ->
        ...
        updated_field_index = Map.update(field_index, value, event_ids, fn existing ->
  ...
  defp rebuild_index(field, state) do
    ...
    new_index = Enum.reduce(all_events, %{}, fn event, acc ->
      case Map.get(event, field) do
        nil -> acc
        value ->
  ...
  defp update_search_index(events, search_index) do
    Enum.reduce(events, search_index, fn event, acc ->
      ...
      Enum.reduce(terms, acc, fn term, acc_index ->
        put_in(acc_index, [:terms, term], 
               MapSet.put(get_in(acc_index, [:terms, term]) || MapSet.new(), event.event_id))
      end)
    end)
  end
  ...
  defp init_rotation_config(config) do
    %{
      max_file_size: Map.get(config, :max_file_size, 100_000_000),  # 100MB
  ...
  defp calculate_statistics(state) do
    ...
    %{
      ...
      file_count: length(Path.wildcard(Path.join(state.storage_path, "*.log*"))),
