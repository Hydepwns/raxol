defmodule Raxol.Architecture.CQRS.CommandHandler do
    ...
    quote do
      ...
      def publish_event(event, context) do
        case EventStore.append_event(event, get_stream_name(event), context) do
          {:ok, _event_id} -> :ok
          {:error, reason} -> 
            Logger.error("Failed to publish event #{inspect(event.__struct__)}: #{inspect(reason)}")
      ...
      def publish_events(events, context) when is_list(events) do
        case EventStore.append_events(events, get_batch_stream_name(events), context) do
          {:ok, _event_ids} -> :ok
          {:error, reason} ->
      ...
      def execute_with_handling(command, context, handler_fn) do
        ...
        try do
          ...
          case result do
            {:ok, data} ->
              Logger.info("Command processed successfully: #{inspect(command_type)} in #{execution_time}μs")
              {:ok, data}
              
            {:error, reason} ->
              Logger.warning("Command failed: #{inspect(command_type)} - #{inspect(reason)}")
              {:error, reason}
          end
        rescue
          error ->
            execution_time = System.monotonic_time(:microsecond) - start_time
            Logger.error("Command handler crashed: #{inspect(command_type)} after #{execution_time}μs - #{inspect(error)}")
  ...
  """
  def validate_preconditions(command, preconditions) when is_list(preconditions) do
    failed_preconditions = Enum.filter(preconditions, fn {_name, check_fn} ->
  ...
  """
  def execute_compensation(compensations, context) when is_list(compensations) do
  ...
  def create_pipeline(steps) when is_list(steps) do
    fn command, context ->
      Enum.reduce_while(steps, {:ok, command, context}, fn step, {_status, cmd, ctx} ->
  ...
  defp execute_saga([step | remaining_steps], command, context, completed_steps) do
    case step.(command, context) do
      {:ok, result} ->
        execute_saga(remaining_steps, command, context, [result | completed_steps])
