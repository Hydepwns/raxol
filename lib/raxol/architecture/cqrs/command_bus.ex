defmodule Raxol.Architecture.CQRS.CommandBus do
  ...
  @default_config %{
    ...
    dead_letter_retention: 86_400_000,  # 24 hours
    enable_metrics: true,
  ...
  @impl GenServer
  def handle_call({:register_handler, command_type, handler_module}, _from, state) do
    ...
    case validate_command_handler(handler_module) do
      :ok ->
        ...
        Logger.info("Registered handler #{inspect(handler_module)} for #{inspect(command_type)}")
        {:reply, :ok, new_state}
  ...
  def handle_call({:add_middleware, middleware_module, opts}, _from, state) do
    ...
    new_middleware = [middleware_entry | state.middleware_stack]
    |> Enum.sort_by(& &1.priority)
    
    new_state = %{state | middleware_stack: new_middleware}
    
    Logger.info("Added middleware #{inspect(middleware_module)} with priority #{priority}")
    {:reply, :ok, new_state}
  end
  
  @impl GenServer
  def handle_call({:remove_middleware, middleware_module}, _from, state) do
    new_middleware = Enum.reject(state.middleware_stack, &(&1.module == middleware_module))
    new_state = %{state | middleware_stack: new_middleware}
  ...
  def handle_call(:get_metrics, _from, state) do
    metrics = if state.config.enable_metrics do
  ...
  def handle_call({:get_dead_letter_commands, limit}, _from, state) do
    commands = state.dead_letter_queue
    |> Map.values()
    |> Enum.take(limit)
  ...
  def handle_call({:retry_dead_letter_command, command_id}, _from, state) do
    case Map.get(state.dead_letter_queue, command_id) do
      ...
      dead_command ->
        ...
        Logger.info("Moved command #{command_id} from dead letter queue back to retry queue")
        {:reply, :ok, new_state}
    end
  end
  ...
  defp execute_command_internal(command, _opts, state) do
    ...
    if handler_module && is_circuit_breaker_open?(state.circuit_breakers, handler_module) do
      {:error, :circuit_breaker_open, state}
    else
      # Execute through middleware pipeline
      case execute_through_middleware(command, handler_module, state.middleware_stack, state) do
        {:ok, result} ->
        ...
        {:error, reason} ->
          ...
          execution_time = System.monotonic_time(:microsecond) - start_time
          new_state = handle_command_failure(state, command, reason, execution_time)
          {:error, reason, new_state}
      end
    end
  end
  
  defp execute_through_middleware(command, handler_module, middleware_stack, state) do
    ...
    case execute_middleware_chain(middleware_stack, context, state) do
      {:ok, updated_context} ->
        ...
        if handler_module do
          execute_command_handler(handler_module, updated_context.command, updated_context)
        else
  ...
  defp execute_command_handler(handler_module, command, context) do
    try do
      case apply(handler_module, :handle, [command, context]) do
        ...
        result -> {:ok, result}  # Assume success if not explicitly error
      end
    catch
      kind, reason ->
        Logger.error("Command handler #{inspect(handler_module)} failed: #{inspect(kind)} #{inspect(reason)}")
        {:error, {kind, reason}}
    end
  end
  ...
  defp record_command_success(state, command, execution_time) do
    # Update metrics
    new_state = if state.config.enable_metrics do
  ...
  defp handle_command_failure(state, command, reason, execution_time) do
    ...
    new_circuit_breakers = update_circuit_breaker(state.circuit_breakers, handler_module, :failure)
    ...
    final_state = if is_retriable_error?(reason) do
  ...
  defp update_circuit_breaker(circuit_breakers, handler_module, result) do
    current = Map.get(circuit_breakers, handler_module, %{failures: 0, status: :closed})
    
    case result do
      :success ->
        # Reset failure count on success
        Map.put(circuit_breakers, handler_module, %{failures: 0, status: :closed})
        
      :failure ->
        new_failures = current.failures + 1
        
        if new_failures >= 5 do  # Circuit breaker threshold
          ...
          Map.put(circuit_breakers, handler_module, %{current | failures: new_failures})
        end
    end
  end
  ...
  defp add_to_retry_queue(state, command, reason) do
    retry_entry = %{
      command: command,
      attempts: 0,
      next_retry_at: System.monotonic_time(:millisecond) + state.config.retry_backoff_base,
  ...
  defp add_to_dead_letter_queue(state, command, reason) do
    ...
    new_dead_letter_queue = Map.put(state.dead_letter_queue, command_id, dead_letter_entry)
  ...
  defp process_retry_queue(state) do
    ...
    {ready_commands, remaining_queue} = extract_ready_commands(state.retry_queue, now)
    
    # Process ready commands
    final_state = Enum.reduce(ready_commands, %{state | retry_queue: remaining_queue}, fn retry_entry, acc_state ->
      if retry_entry.attempts < acc_state.config.max_retries do
        # Retry the command
        case execute_command_internal(retry_entry.command, [], acc_state) do
          {:ok, _result, new_state} ->
            Logger.info("Command retry succeeded: #{inspect(retry_entry.command.__struct__)}")
            new_state
            
          {:error, reason, new_state} ->
            # Increment attempts and re-queue or move to dead letter
            updated_entry = %{retry_entry |
              attempts: retry_entry.attempts + 1,
              next_retry_at: calculate_next_retry_time(retry_entry.attempts + 1, acc_state.config.retry_backoff_base)
            }
            
            if updated_entry.attempts >= acc_state.config.max_retries do
              # Move to dead letter queue
              add_to_dead_letter_queue(new_state, retry_entry.command, reason)
            else
              # Re-queue for retry
        ...
        add_to_dead_letter_queue(acc_state, retry_entry.command, :max_retries_exceeded)
      end
    end)
  ...
  defp cleanup_expired_dead_letters(state) do
    ...
    new_dead_letter_queue = state.dead_letter_queue
    |> Enum.filter(fn {_id, entry} ->
      now - entry.failed_at < retention_ms
    end)
    |> Map.new()
    
    removed_count = map_size(state.dead_letter_queue) - map_size(new_dead_letter_queue)
  ...
  defp reset_expired_circuit_breakers(state) do
    ...
    new_circuit_breakers = state.circuit_breakers
  ...
  defp update_metrics(state, command, result, execution_time) do
    if state.metrics_collector.enabled do
      ...
      current_count = Map.get(state.metrics_collector.command_counts, command_type, %{success: 0, failure: 0})
      new_count = case result do
        :success -> %{current_count | success: current_count.success + 1}
        :failure -> %{current_count | failure: current_count.failure + 1}
      end
      
      new_command_counts = Map.put(state.metrics_collector.command_counts, command_type, new_count)
      
      # Update execution times (keep last 100)
      new_execution_times = [execution_time | Enum.take(state.metrics_collector.execution_times, 99)]
      
      new_metrics_collector = %{state.metrics_collector |
  ...
  defp get_metrics_from_collector(metrics_collector) do
    if metrics_collector.enabled do
      total_commands = metrics_collector.command_counts
      |> Map.values()
      |> Enum.reduce(0, fn counts, acc -> acc + counts.success + counts.failure end)
      
      total_successes = metrics_collector.command_counts
      |> Map.values()
      |> Enum.reduce(0, fn counts, acc -> acc + counts.success end)
      
      success_rate = if total_commands > 0 do
        total_successes / total_commands * 100
      else
        0.0
      end
      
      avg_execution_time = if length(metrics_collector.execution_times) > 0 do
        Enum.sum(metrics_collector.execution_times) / length(metrics_collector.execution_times)
      else
        0.0
      end
