defmodule Raxol.Architecture.CQRS.CommandDispatcher do
  ...
  def register_handler(dispatcher \\ __MODULE__, command_type, handler_module) do
    GenServer.call(dispatcher, {:register_handler, command_type, handler_module})
  end
  ...
  def init(config) do
    ...
    Logger.info("Command dispatcher initialized with config: #{inspect(config)}")
    {:ok, state}
  end
  ...
  def handle_call({:register_handler, command_type, handler_module}, _from, state) do
    case CommandHandler.validate_handler(handler_module) do
      :ok ->
        ...
        Logger.info("Registered handler #{inspect(handler_module)} for command #{inspect(command_type)}")
        {:reply, :ok, new_state}
        
      {:error, reason} ->
        Logger.warning("Failed to register handler #{inspect(handler_module)}: #{inspect(reason)}")
  ...
  def handle_call(:list_handlers, _from, state) do
    handlers = Enum.map(state.handlers, fn {command_type, handler} ->
  ...
  defp do_dispatch_command(command, opts, state) do
    ...
    with {:ok, handler} <- get_handler(command_type, state.handlers),
         ...
         {:ok, _} <- check_circuit_breaker(command_type, state.circuit_breakers),
         {:ok, result} <- execute_command_with_retries(command, handler, context, state) do
      
      # Update metrics
      new_state = update_metrics(state, :command_executed, %{
      ...
      {:error, :handler_not_found} ->
        Logger.warning("No handler registered for command #{inspect(command_type)}")
        error_state = update_metrics(state, :command_failed, %{
          command_type: command_type,
      ...
      {:error, :circuit_breaker_open} ->
        Logger.warning("Circuit breaker open for command #{inspect(command_type)}")
        error_state = update_metrics(state, :command_rejected, %{
          command_type: command_type,
      ...
      {:error, reason} ->
        Logger.error("Command dispatch failed for #{inspect(command_type)}: #{inspect(reason)}")
        
        # Update circuit breaker
        updated_state = update_circuit_breaker(command_type, state)
        
        # Add to dead letter queue if enabled
        final_state = if state.config.enable_dead_letter_queue do
        ...
        # Update metrics
        error_state = update_metrics(final_state, :command_failed, %{
          command_type: command_type,
          reason: reason
        })
  ...
  defp do_dispatch_batch(commands, opts, state) do
    if length(commands) > state.config.batch_size_limit do
      ...
      {results, final_state} = Enum.reduce(commands, {[], state}, fn command, {acc_results, acc_state} ->
      ...
      metrics_state = update_metrics(final_state, :batch_executed, %{
  ...
  defp execute_with_retry(command, handler, context, state, attempt, max_retries) do
    case apply_middleware_and_execute(command, handler, context, state) do
      ...
      {:error, _reason} when attempt < max_retries ->
        ...
        delay_ms = state.config.retry_base_delay_ms * :math.pow(2, attempt) |> round()
        
        Logger.info("Retrying command after #{delay_ms}ms (attempt #{attempt + 1}/#{max_retries})")
        :timer.sleep(delay_ms)
        
        execute_with_retry(command, handler, context, state, attempt + 1, max_retries)
        
      {:error, reason} ->
        Logger.error("Command failed after #{max_retries} retries: #{inspect(reason)}")
        {:error, reason}
    end
  end
  ...
  defp apply_middleware_and_execute(command, handler, context, state) do
    ...
    final_handler = build_middleware_pipeline(state.middleware, fn cmd, ctx ->
  ...
  defp check_circuit_breaker(command_type, circuit_breakers) do
    case Map.get(circuit_breakers, command_type) do
      ...
      %{state: :half_open} -> {:ok, :half_open}
    end
  end
  
  defp update_circuit_breaker(command_type, state) do
    current_breaker = Map.get(state.circuit_breakers, command_type, %{
      state: :closed,
      failure_count: 0,
    ...
    updated_breaker = if new_failure_count >= current_breaker.threshold do
      %{current_breaker |
        state: :open,
    ...
    new_circuit_breakers = Map.put(state.circuit_breakers, command_type, updated_breaker)
  ...
  defp reset_circuit_breaker(command_type, state) do
    case Map.get(state.circuit_breakers, command_type) do
      ...
      breaker ->
        ...
        new_circuit_breakers = Map.put(state.circuit_breakers, command_type, reset_breaker)
        %{state | circuit_breakers: new_circuit_breakers}
    end
  end
    ...
    new_metrics = case metric_type do
      :command_executed ->
        %{state.metrics_collector |
          ...
          successful_commands: state.metrics_collector.successful_commands + 1
        }
      :command_failed ->
        %{state.metrics_collector |
          commands_executed: state.metrics_collector.commands_executed + 1,
  ...
  defp register_default_handlers(state) do
    ...
    Logger.debug("Default handlers registration skipped - will be done via configuration")
    :ok
  end
end
...
defmodule Raxol.Architecture.CQRS.Middleware.LoggingMiddleware do
  ...
  def call(command, context, next) do
    ...
    Logger.info("Executing command: #{inspect(command_type)} [#{correlation_id}]")
    start_time = System.monotonic_time(:microsecond)
    
    case next.(command, context) do
      {:ok, result} ->
        execution_time = System.monotonic_time(:microsecond) - start_time
        Logger.info("Command completed: #{inspect(command_type)} in #{execution_time}μs [#{correlation_id}]")
        {:ok, result}
        
      {:error, reason} ->
        execution_time = System.monotonic_time(:microsecond) - start_time
        Logger.error("Command failed: #{inspect(command_type)} after #{execution_time}μs - #{inspect(reason)} [#{correlation_id}]")
        {:error, reason}
    end
  end
end