defmodule Raxol.Architecture.EventSourcing.EventStore do
  ...
  """
  def read_stream(store \\ __MODULE__, stream_name, start_position \\ 0, count \\ 100) do
  ...
  """
  def create_projection(store \\ __MODULE__, projection_name, projection_fn, opts \\ []) do
    GenServer.call(store, {:create_projection, projection_name, projection_fn, opts})
  end
  ...
  def handle_call({:get_stream_version, stream_name}, _from, state) do
    version = case Map.get(state.streams, stream_name) do
      nil -> 0
  ...
  @impl GenServer
  def handle_call({:save_snapshot, stream_name, version, snapshot_data}, _from, state) do
    snapshot = %Snapshot{
  ...
  def handle_call({:subscribe, subscriber_pid, opts}, _from, state) do
    ...
    Logger.debug("Added subscriber #{inspect(subscriber_pid)} for stream #{inspect(stream_filter)}")
    {:reply, :ok, new_state}
  end
  ...
  @impl GenServer
  def handle_call({:create_projection, projection_name, projection_fn, opts}, _from, state) do
    projection = %{
  ...
  defp append_single_event(event, stream_name, context, state) do
    with {:ok, enriched_event} <- enrich_event(event, stream_name, context, state),
         {:ok, event_id} <- store_event(enriched_event, stream_name, state.storage),
         {:ok, new_state} <- update_stream_state(stream_name, enriched_event, state) do
      
      # Notify subscribers
      notify_subscribers(enriched_event, stream_name, new_state.subscribers)
      
      # Update metrics
      updated_state = update_metrics(new_state, :event_appended, %{stream: stream_name})
  ...
  defp append_multiple_events(events, stream_name, context, state) do
    case events do
      ...
      _ ->
        ...
        enriched_events = Enum.map(events, &enrich_event(&1, stream_name, context, state))
          ...
          nil ->
            ...
            case store_events_batch(enriched_list, stream_name, state.storage) do
              {:ok, event_ids} ->
                ...
                new_state = Enum.reduce(enriched_list, state, fn event, acc_state ->
                  {:ok, updated} = update_stream_state(stream_name, event, acc_state)
                  updated
                end)
                ...
                # Update metrics
                final_state = update_metrics(new_state, :events_batch_appended, %{
                  stream: stream_name,
                  count: length(events)
                })
  ...
  defp update_stream_state(stream_name, event, state) do
    current_stream = Map.get(state.streams, stream_name, %EventStream{
      name: stream_name,
      version: 0,
      last_position: 0,
      created_at: System.system_time(:millisecond),
  ...
  defp read_stream_events(stream_name, start_position, count, state) do
    case EventStorage.read_stream(state.storage, stream_name, start_position, count) do
      {:ok, events} -> {:ok, events}
  ...
  defp notify_subscribers(event, stream_name, subscribers) do
    matching_subscribers = subscribers
  ...
  defp create_automatic_snapshots(state) do
    if state.config.enable_snapshots do
      ...
      streams_needing_snapshots = state.streams
      |> Enum.filter(fn {stream_name, stream} ->
        needs_snapshot?(stream_name, stream, frequency, state.snapshots)
      end)
      
      Enum.reduce(streams_needing_snapshots, state, fn {stream_name, _stream}, acc_state ->
        case create_stream_snapshot(stream_name, acc_state) do
          {:ok, new_state} -> new_state
          {:error, reason} ->
            Logger.warning("Failed to create snapshot for #{stream_name}: #{inspect(reason)}")
            acc_state
        end
      end)
    else
  ...
  defp replay_stream_events(stream_name, start_position, handler_fn, state) do
    case read_stream_events(stream_name, start_position, 1000, state) do
      {:ok, events} ->
        processed_count = Enum.reduce(events, 0, fn event, acc ->
          handler_fn.(event)
          acc + 1
        end)
  ...
  defp init_replication_nodes(config) do
    case config.replication_factor do
      1 -> []
      _ -> []  # Would initialize replication nodes
    end
  end
  ...
  defp update_metrics(state, metric_type, metadata) do
    new_metrics = case metric_type do
      :event_appended ->
        %{state.metrics_collector | events_appended: state.metrics_collector.events_appended + 1}
      :events_batch_appended ->
        count = Map.get(metadata, :count, 1)
        %{state.metrics_collector | events_appended: state.metrics_collector.events_appended + count}
      _ ->
  ...
  defp replicate_to_nodes(state) do
    ...
    if length(state.replication_nodes) > 0 do
      Logger.debug("Replicating events to #{length(state.replication_nodes)} nodes")
    end
