defmodule Raxol.Architecture.EventSourcing.Event do
  ...
  """
  def is_event_type?(%__MODULE__{event_type: type}, expected_type), do: type == expected_type
  def is_event_type?(%{__struct__: type}, expected_type), do: type == expected_type
  ...
  def from_map(map) when is_map(map) do
    ...
    event_data = case map.data do
      %{__struct__: _} = data -> data
  ...
  def validate_envelope(%__MODULE__{} = event) do
    required_fields = [:id, :stream_name, :event_type, :data, :position, :created_at]
  ...
  def correlate_events(parent_event, child_event) do
    correlation_id = case get_correlation_id(parent_event) do
      nil -> parent_event.id
    ...
    %{child_event |
      metadata: Map.merge(child_event.metadata, %{
  ...
  """
  def migrate_event(%__MODULE__{} = event, migration_fn) when is_function(migration_fn, 1) do
