defmodule Raxol.Terminal.Emulator.Buffer do
  ...
  """
  @spec switch_buffer(Core.t(), :main | :alternate) :: {:ok, Core.t()} | {:error, String.t()}
  ...
  Returns {:ok, updated_emulator} or {:error, reason}.
  """
  @spec scroll_up(Core.t(), non_neg_integer()) :: {:ok, Core.t()} | {:error, String.t()}
  ...
  Returns {:ok, updated_emulator} or {:error, reason}.
  """
  @spec scroll_down(Core.t(), non_neg_integer()) :: {:ok, Core.t()} | {:error, String.t()}
  ...
  Returns {:ok, updated_emulator} or {:error, reason}.
  """
  @spec clear_from_cursor_to_end(Core.t()) :: {:ok, Core.t()} | {:error, String.t()}
  def clear_from_cursor_to_end(%Core{} = emulator) do
    active_buffer = Core.get_active_buffer(emulator)
    case Manager.clear_from_cursor_to_end(active_buffer, emulator.cursor.position) do
      {:ok, updated_buffer} ->
        updated_emulator = update_active_buffer(emulator, updated_buffer)
        {:ok, updated_emulator}
      {:error, reason} ->
  ...
  Returns {:ok, updated_emulator} or {:error, reason}.
  """
  @spec clear_from_cursor_to_start(Core.t()) :: {:ok, Core.t()} | {:error, String.t()}
  def clear_from_cursor_to_start(%Core{} = emulator) do
    active_buffer = Core.get_active_buffer(emulator)
    case Manager.clear_from_cursor_to_start(active_buffer, emulator.cursor.position) do
      {:ok, updated_buffer} ->
        updated_emulator = update_active_buffer(emulator, updated_buffer)
        {:ok, updated_emulator}
      {:error, reason} ->
  ...
  defp update_active_buffer(%Core{active_buffer_type: :main} = emulator, updated_buffer) do
    %{emulator | main_screen_buffer: updated_buffer}
  end

  defp update_active_buffer(%Core{active_buffer_type: :alternate} = emulator, updated_buffer) do
