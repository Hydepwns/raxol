defmodule Raxol.Terminal.Escape.Parsers.CSIParserCached do
  ...
  def parse(data) do
    ...
    case ETSCacheManager.get_csi(cache_key) do
      ...
      :miss ->
        ...
        result = Telemetry.parse_csi(cache_key, fn ->
  ...
  defp extract_cache_key(data) do
    ...
    case Regex.run(~r/^([\?\d;]*[\x20-\x2F]*[\x40-\x7E])/, data, capture: :all_but_first) do
      [sequence] -> sequence
      _ -> data
    end
  end
  ...
  def warm_cache do
    common_sequences = [
      ...
      "38;5;16m", "38;5;231m", "48;5;16m", "48;5;231m",
    ...
    Logger.info("CSI parser cache warmed with #{length(common_sequences)} common sequences")
  end
end