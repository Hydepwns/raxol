  ...
  def handle_input(data, emulator, parser_state) do
    ...
    case data do
      ...
      <<27, rest_after_esc::binary>> ->
        Logger.debug("GroundState: ESC detected, transitioning to EscapeState.")
      ...
      <<142, rest_after_ss2::binary>> ->
        Logger.info("[Parser] SS2 (C1, 0x8E) received - will use G2 for next char only")
        {:continue, emulator, %{parser_state | single_shift: :ss2}, rest_after_ss2}

      # SS3 (C1 Control, 0x8F)
      <<143, rest_after_ss3::binary>> ->
        Logger.info("[Parser] SS3 (C1, 0x8F) received - will use G3 for next char only")
      ...
      # Unhandled case
      other ->
        Logger.warning("GroundState unhandled input: #{inspect(other)} with emulator: #{inspect(emulator)}")
