defmodule Raxol.Terminal.Buffer.Eraser do
  ...
  def clear_region(%ScreenBuffer{} = buffer, top, left, bottom, right, default_style) do
    ...
    if clamped_top > clamped_bottom or clamped_left > clamped_right do
      Logger.debug("[Eraser.clear_region] Invalid region (clamped_top > clamped_bottom or clamped_left > clamped_right), no-op.")
      buffer # No-op if region is invalid
    else
      # REMOVED condition: Log the exact clamped values for EVERY call that reaches here
      Logger.debug("[Eraser.clear_region PARAMS] top_arg: #{top}, left_arg: #{left}, bottom_arg: #{bottom}, right_arg: #{right} --- clamped_top: #{clamped_top}, clamped_left: #{clamped_left}, clamped_bottom: #{clamped_bottom}, clamped_right: #{clamped_right}")
      ...
      new_cells =
        ...
        |> Enum.map(fn {line, row_idx} ->
          condition_result = row_idx >= clamped_top and row_idx <= clamped_bottom

          # ADDED: Log row_idx and condition_result if we are in the call for LINE 2
          if top == 2 and bottom == 2 and left == 0 do # Check if this is the call originating from clear_line(2)
            Logger.debug("[Eraser.clear_region CALL FOR LINE 2] row_idx: #{row_idx}, condition_eval: (top=#{clamped_top}, bot=#{clamped_bottom}), condition_result: #{condition_result}")
          end
          ...
          if condition_result do
            ...
            Logger.debug("[Eraser.clear_region] Inside map for row #{row_idx}: effective_left=#{clamped_left}, effective_right=#{clamped_right}")
            ...
            Logger.debug("[Eraser.clear_region] row_idx: #{row_idx} - Original line chars: #{inspect(original_line_chars_map)}")
            Logger.debug("[Eraser.clear_region] row_idx: #{row_idx} - Splitting at left: #{clamped_left}, right_plus_1_for_drop: #{clamped_right + 1}")
            Logger.debug("[Eraser.clear_region] row_idx: #{row_idx} - Prefix chars: #{inspect(prefix_chars_map)} (count: #{length(prefix_cells)})")
            Logger.debug("[Eraser.clear_region] row_idx: #{row_idx} - Suffix chars: #{inspect(suffix_chars_map)} (count: #{length(suffix_cells)})" )
            Logger.debug("[Eraser.clear_region] row_idx: #{row_idx} - Blank segment to insert: #{inspect(blank_segment_chars_map)} (count: #{length(blank_row_segment)})")

            new_line_cells = prefix_cells ++ blank_row_segment ++ suffix_cells
            new_line_chars_map = Enum.map(new_line_cells, &(&1.char))
            Logger.debug("[Eraser.clear_region] row_idx: #{row_idx} - New line chars: #{inspect(new_line_chars_map)}")

            new_line_cells
          else
            line # Return unchanged line
          end
        end)
  ...
  Returns the updated buffer state.
  """
  @spec clear_line_from(ScreenBuffer.t(), integer(), integer(), TextFormatting.text_style()) :: ScreenBuffer.t()
  ...
  Returns the updated buffer state.
  """
  @spec clear_line_to(ScreenBuffer.t(), integer(), integer(), TextFormatting.text_style()) :: ScreenBuffer.t()
  ...
  Returns the updated buffer state.
  """
  @spec clear_line(ScreenBuffer.t(), integer(), TextFormatting.text_style()) :: ScreenBuffer.t()
  def clear_line(%ScreenBuffer{} = buffer, row, default_style) do
    Logger.debug("[Eraser.clear_line] CALLED with row: #{row}, buffer_height: #{buffer.height}")
    # Ensure row is valid
    if row >= 0 and row < buffer.height do
      Logger.debug("[Eraser.clear_line] Condition TRUE, calling clear_region for row: #{row}")
      clear_region(buffer, row, 0, row, buffer.width - 1, default_style)
    else
      Logger.debug("[Eraser.clear_line] Condition FALSE for row: #{row}, returning original buffer")
       buffer
    end
  end
  ...
  Returns the updated buffer state.
  """
  @spec clear_screen_from(ScreenBuffer.t(), integer(), integer(), TextFormatting.text_style()) :: ScreenBuffer.t()
  def clear_screen_from(%ScreenBuffer{} = buffer, row, col, default_style) do
    ...
    if eff_row < buffer.height - 1 do
      clear_region(buffer_step1, eff_row + 1, 0, buffer.height - 1, buffer.width - 1, default_style)
    else
      buffer_step1
    end
  end
  ...
  Returns the updated buffer state.
  """
  @spec clear_screen_to(ScreenBuffer.t(), integer(), integer(), TextFormatting.text_style()) :: ScreenBuffer.t()
  def clear_screen_to(%ScreenBuffer{} = buffer, row, col, default_style) do
    ...
    Logger.debug("[clear_screen_to] After clear_line_to, buffer_step1.width: #{buffer_step1.width}, eff_row: #{eff_row}")
    ...
    if eff_row > 0 do
      ...
      arg_right = buffer_step1.width - 1 # Assuming width is correct here
      Logger.debug(
      ...
      clear_region(buffer_step1, arg_top, arg_left, arg_bottom, arg_right, default_style) # Use explicit vars
    else
      buffer_step1
    end
  end
  ...
  Returns the updated buffer state.
  """
  @spec clear_screen(ScreenBuffer.t(), TextFormatting.text_style()) :: ScreenBuffer.t()
  def clear_screen(%ScreenBuffer{} = buffer, default_style) do
    clear_region(buffer, 0, 0, buffer.height - 1, buffer.width - 1, default_style)
  end
  ...
  def erase_in_display(%ScreenBuffer{} = buffer, {col, row}, type, default_style) do
    case type do
      :to_end ->
