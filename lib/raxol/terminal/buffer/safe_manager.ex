defmodule Raxol.Terminal.Buffer.SafeManager do
  ...
  This module wraps the standard buffer manager with error handling patterns
  including circuit breakers, retries, and graceful degradation.
  """
  ...
  def write(pid \\ __MODULE__, data, opts \\ []) do
    GenServer.call(pid, {:write, data, opts}, 10_000)
  catch
    :exit, {:timeout, _} ->
  ...
  def read(pid \\ __MODULE__, opts \\ []) do
    GenServer.call(pid, {:read, opts}, 5_000)
  catch
    :exit, {:timeout, _} ->
    ...
    try do
      ...
      cond do
        ...
        width > 10_000 or height > 10_000 ->
          {:reply, {:error, :dimensions_too_large}, state}

        true ->
          case perform_resize(state.manager_pid, width, height) do
            :ok ->
              # Update fallback buffer dimensions
              new_fallback = BufferImpl.new(width, height)
              {:reply, :ok, %{state | fallback_buffer: new_fallback}}
  ...
  def handle_info(
    ...
    new_state =
      with_error_handling :manager_restart do
        case Manager.start_link([]) do
          {:ok, new_pid} ->
            Process.monitor(new_pid)
            new_stats = Map.update(state.stats, :recoveries, 1, &(&1 + 1))
            %{state | manager_pid: new_pid, stats: new_stats}

          {:error, restart_reason} ->
            Logger.error(
              "Failed to restart manager: #{inspect(restart_reason)}"
            )
  ...
  defp perform_write(manager_pid, data, opts) do
    case GenServer.call(manager_pid, {:write, data, opts}, 5_000) do
      :ok -> {:ok, :written}
      error -> error
    end
  catch
  ...
  defp perform_read(manager_pid, opts) do
    GenServer.call(manager_pid, {:read, opts}, 5_000)
  catch
    :exit, {:timeout, _} -> {:error, :timeout}
    :exit, {:noproc, _} -> {:error, :manager_dead}
  end
