defmodule Raxol.Terminal.Buffer.UnifiedManager do
  ...
  def handle_call({:write_at, x, y, data}, _from, state) do
    case CellOperations.set_cell(state, x, y, Cell.new(data)) do
      {:ok, new_state} -> {:reply, {:ok, new_state}, new_state}
      {:error, reason} -> {:reply, {:error, reason}, state}
    end
  end
  ...
  @doc """
  Writes data at a specific position.
  """
  def write_at(pid \\ __MODULE__, x, y, data) do
    GenServer.call(pid, {:write_at, x, y, data})
  end
