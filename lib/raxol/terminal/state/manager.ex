defmodule Raxol.Terminal.State.Manager do
  ...
  def set_dimensions(%__MODULE__{} = state, rows, cols)
      when is_integer(rows) and rows > 0 and
             is_integer(cols) and cols > 0 do
  ...
  """
  def set_cursor_visibility(%__MODULE__{} = state, visible)
  ...
  """
  def set_cursor_style(%__MODULE__{} = state, style)
