defmodule Raxol.Terminal.Commands.CSIHandler.WindowHandlers do
  @moduledoc """
  ...
  def handle_window_maximize(emulator) do
    case Raxol.Terminal.Commands.WindowHandler.handle_t(emulator, [9]) do
  ...
  def handle_window_unmaximize(emulator) do
    case Raxol.Terminal.Commands.WindowHandler.handle_t(emulator, [10]) do
      {:ok, updated_emulator} -> updated_emulator
      {:error, _, emulator} -> emulator
    end
  end
  ...
  def handle_window_minimize(emulator) do
    case Raxol.Terminal.Commands.WindowHandler.handle_t(emulator, [2]) do
      {:ok, updated_emulator} -> updated_emulator
      {:error, _, emulator} -> emulator
    end
  end
  ...
  def handle_window_unminimize(emulator) do
    case Raxol.Terminal.Commands.WindowHandler.handle_t(emulator, [1]) do
      {:ok, updated_emulator} -> updated_emulator
      {:error, _, emulator} -> emulator
    end
  end
  ...
  def handle_window_raise(emulator) do
    case Raxol.Terminal.Commands.WindowHandler.handle_t(emulator, [5]) do
      {:ok, updated_emulator} -> updated_emulator
      {:error, _, emulator} -> emulator
    end
  end
  ...
  def handle_window_lower(emulator) do
    case Raxol.Terminal.Commands.WindowHandler.handle_t(emulator, [6]) do
      {:ok, updated_emulator} -> updated_emulator
      {:error, _, emulator} -> emulator
    end
  end
  ...
  def handle_window_title(emulator) do
    ...
    case Raxol.Terminal.Commands.WindowHandler.handle_t(emulator, [0, title]) do
      {:ok, updated_emulator} -> updated_emulator
      {:error, _, emulator} -> emulator
    end
  end
  ...
  def handle_window_icon_name(emulator) do
    ...
    case Raxol.Terminal.Commands.WindowHandler.handle_t(emulator, [
           8,
           icon_name
         ]) do
      {:ok, updated_emulator} -> updated_emulator
  ...
  def handle_window_size_report(emulator) do
    case Raxol.Terminal.Commands.WindowHandler.handle_t(emulator, [18]) do
      {:ok, updated_emulator} -> updated_emulator
      {:error, _, emulator} -> emulator
    end
  end
  ...
  def handle_window_size_pixels(emulator) do
    ...
    width_px =
      w * Raxol.Terminal.Commands.WindowHandler.default_char_width_px()

    height_px =
      h * Raxol.Terminal.Commands.WindowHandler.default_char_height_px()
