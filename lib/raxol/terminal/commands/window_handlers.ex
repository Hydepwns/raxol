defmodule Raxol.Terminal.Commands.WindowHandlers do
  ...
  defp handle_window_operation(emulator, op, params) do
    case op do
      ...
      11 ->
        handle_state_report(emulator)

      13 ->
        handle_size_report(emulator)

      14 ->
        handle_position_report(emulator)

      18 ->
        handle_screen_size_report(emulator)

      19 ->
        handle_screen_size_pixels_report(emulator)
  ...
  defp get_window_position(params) do
    case params do
      ...
      [x, y | _] when is_integer(x) and is_integer(y) ->
        Logger.warning(
          "Window move command received with negative position: x=#{x}, y=#{y}"
        )
      ...
      [x | _] when is_integer(x) ->
        Logger.warning(
          "Window move command received with negative x coordinate: #{x}"
        )
      ...
      [x, y | _] ->
        Logger.warning(
          "Window move command received with invalid parameter types: x=#{inspect(x)}, y=#{inspect(y)}"
        )
      ...
      _ ->
        Logger.warning(
          "Window move command received with insufficient parameters"
        )
  ...
  defp get_window_size(params) do
    case params do
      ...
      [width, height | _] when is_integer(width) and is_integer(height) ->
        Logger.warning(
          "Window resize command received with non-positive dimensions: width=#{width}, height=#{height}"
        )
      ...
      [width | _] when is_integer(width) and width > 0 ->
        Logger.warning(
          "Window resize command received with missing height parameter"
        )
      ...
      [width | _] when is_integer(width) ->
        Logger.warning(
          "Window resize command received with non-positive width: #{width}"
        )
      ...
      [width, height | _] ->
        Logger.warning(
          "Window resize command received with invalid parameter types: width=#{inspect(width)}, height=#{inspect(height)}"
        )
      ...
      _ ->
        Logger.warning(
          "Window resize command received with insufficient parameters"
        )
  ...
  defp handle_maximize(emulator) do
    %{
      ...
      | window_state: %{
        ...
        main_screen_buffer:
          ScreenBuffer.resize(emulator.main_screen_buffer, 100, 50),
        alternate_screen_buffer:
          ScreenBuffer.resize(emulator.alternate_screen_buffer, 100, 50)
    }
  end
  ...
  defp handle_size_report(emulator) do
    ...
    %{
      emulator
      | output_buffer: emulator.output_buffer <> "\e[8;#{height};#{width}t"
    }
  end
