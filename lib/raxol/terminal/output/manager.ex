defmodule Raxol.Terminal.Output.Manager do
  ...
  def flush_output(%__MODULE__{} = state) do
    ...
    new_state = %{state | output_buffer: "", buffer_size: 0}
    {output, new_state}
  end
  ...
  def clear_output_buffer(%__MODULE__{} = state) do
    %{state | output_buffer: "", buffer_size: 0}
  end
  ...
  """
  def enqueue_control_sequence(%__MODULE__{} = state, sequence)
  ...
  """
  def set_max_buffer_size(%__MODULE__{} = state, size)
