defmodule Raxol.Terminal.Extension.Manager do
  ...
  def load_extension(manager, extension) do
    with :ok <- validate_extension(extension),
      ...
      updated_extensions = Map.put(manager.extensions, extension.name, extension)
      updated_events = register_extension_events(manager.events, extension)
      updated_commands = register_extension_commands(manager.commands, extension)
  ...
  def unload_extension(manager, extension_name) do
    case Map.get(manager.extensions, extension_name) do
      ...
      extension ->
        ...
        updated_commands = unregister_extension_commands(manager.commands, extension)
  ...
  """
  @spec execute_command(t(), String.t(), [term()]) :: {:ok, term(), t()} | {:error, term()}
  ...
  def update_extension_config(manager, extension_name, config) do
    case Map.get(manager.extensions, extension_name) do
      nil -> {:error, :extension_not_found}
      extension ->
        updated_extension = %{extension | config: Map.merge(extension.config, config)}
        updated_extensions = Map.put(manager.extensions, extension_name, updated_extension)
  ...
  defp validate_extension(extension) do
    required_fields = [:name, :version, :description, :author, :events, :commands, :config, :state]
  ...
  defp unregister_extension_events(events, extension) do
    Enum.reduce(extension.events, events, fn event_name, acc ->
      Map.update(acc, event_name, [], &Enum.reject(&1, fn e -> e.name == extension.name end))
    end)
  end
