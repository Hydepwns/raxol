defmodule Raxol.Terminal.OutputManager do
  ...
  def enqueue_output(%__MODULE__{} = manager, output) do
    ...
    new_size = String.length(manager.output_buffer) + String.length(formatted_output)
    if new_size > manager.max_buffer_size do
      {:error, :buffer_overflow}
    else
      # Update metrics
      metrics = update_metric(manager.metrics, :processed_bytes, String.length(formatted_output))
      metrics = update_metric(metrics, :format_applications)

      # Update buffer
      {:ok, %{manager | output_buffer: manager.output_buffer <> formatted_output, metrics: metrics}}
    end
  end
  ...
  Returns {:ok, updated_manager} or {:error, :buffer_overflow}
  """
  @spec enqueue_control_sequence(t(), String.t()) :: {:ok, t()} | {:error, :buffer_overflow}
  def enqueue_control_sequence(%__MODULE__{} = manager, sequence) do
    # Check for buffer overflow
    new_size = String.length(manager.control_sequence_buffer) + String.length(sequence)
    if new_size > manager.max_buffer_size do
      {:error, :buffer_overflow}
    else
      # Update metrics
      metrics = update_metric(manager.metrics, :control_sequences)
      metrics = update_metric(metrics, :processed_bytes, String.length(sequence))

      # Update buffer
      {:ok, %{manager | control_sequence_buffer: manager.control_sequence_buffer <> sequence, metrics: metrics}}
    end
  end
  ...
  defp format_control_chars(text) do
    ...
    Regex.replace(pattern, text, fn char -> Map.get(@control_char_map, char, char) end)
  end
  ...
  """
  defp parse_ansi_sequence(sequence) when String.match?(sequence, ~r/^\e\[\d*[ABCDHFST]/), do: parse_cursor_sequence(sequence)
  defp parse_ansi_sequence(sequence) when String.match?(sequence, ~r/^\e\[\d*(;\d+)*m/), do: parse_sgr_sequence(sequence)
  defp parse_ansi_sequence(sequence) when String.match?(sequence, ~r/^\e\[\d*[JKL]/), do: parse_screen_sequence(sequence)
  defp parse_ansi_sequence(sequence) when String.match?(sequence, ~r/^\e\[\?(\d+)(h|l)/), do: parse_mode_sequence(sequence)
  defp parse_ansi_sequence(sequence) when String.match?(sequence, ~r/^\e\[(\d*)n/), do: parse_device_status_sequence(sequence)
  defp parse_ansi_sequence(sequence) when String.match?(sequence, ~r/^\e[\(\)][A-Z0-9]/), do: parse_charset_sequence(sequence)
  defp parse_ansi_sequence(sequence) when String.starts_with?(sequence, "\e]"), do: parse_osc_sequence(sequence)
  defp parse_ansi_sequence(sequence), do: sequence
