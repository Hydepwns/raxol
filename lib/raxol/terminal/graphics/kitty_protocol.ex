defmodule Raxol.Terminal.Graphics.KittyProtocol do
  ...
  # Protocol constants
  @chunk_size 4096
  @max_image_id 4294967295  # 2^32 - 1
  ...
  """
  @spec transmit_image(binary() | String.t(), map()) :: {:ok, binary()} | {:error, term()}
  def transmit_image(image_data, options \\ %{}) do
    with {:ok, validated_options} <- validate_transmission_options(options),
         {:ok, processed_data} <- process_image_data(image_data, validated_options),
         {:ok, image_id} <- generate_image_id(validated_options) do
      
      case Map.get(validated_options, :chunked, false) do
        true -> transmit_chunked_image(processed_data, image_id, validated_options)
        false -> transmit_single_image(processed_data, image_id, validated_options)
      end
    end
  end
  ...
  - `{:error, reason}` - Error with reason
  """
  @spec display_image(image_id(), display_options()) :: {:ok, binary()} | {:error, term()}
  ...
  def delete_image(image_id) when is_integer(image_id) and image_id > 0 and image_id <= @max_image_id do
  ...
  def create_animation(frames, options \\ %{}) when is_list(frames) do
    frame_delay = Map.get(options, :frame_delay, 100) # milliseconds
    loop_count = Map.get(options, :loop_count, -1) # -1 for infinite
    
    case transmit_animation_frames(frames, options) do
      {:ok, frame_ids} ->
        animation_commands = create_animation_commands(frame_ids, frame_delay, loop_count)
  ...
  def detect_support do
    case detect_terminal_type() do
      ...
      :unknown -> {:ok, :unsupported}  # Changed from {:error, :unknown}
      _ -> {:ok, :unsupported}
    end
  end
  ...
  defp validate_format(format) when format in [:rgb, :rgba, :png, :jpeg, :webp, :gif], do: :ok
  defp validate_format(_), do: {:error, :invalid_format}

  defp validate_display_option(key, value) 
    when key in [:x, :y, :width, :height, :x_offset, :y_offset, :columns, :rows] 
    and is_integer(value) and value >= 0 do
    {:ok, value}
  end

  defp validate_display_option(:z_index, value) when is_integer(value), do: {:ok, value}
  ...
  defp process_image_data(file_path, options) when is_binary(file_path) do
    ...
    case String.contains?(file_path, ["/", "\\", ".png", ".jpg", ".jpeg", ".gif", ".webp"]) do
      true ->
  ...
  defp transmit_chunked_image(data, image_id, options) do
    ...
    commands = Enum.with_index(chunks)
    |> Enum.map(fn {chunk, index} ->
      is_last_chunk = index == length(chunks) - 1
      control_data = build_transmission_control_data(image_id, options, not is_last_chunk)
  ...
  defp build_transmission_control_data(image_id, options, more_chunks) do
    ...
    chunk_param = case more_chunks do
      true -> ["m=1"]  # More chunks coming
      false -> ["m=0"] # Last chunk
    end
  ...
  defp detect_terminal_type do
    case {System.get_env("TERM"), System.get_env("TERM_PROGRAM"), 
          System.get_env("KITTY_WINDOW_ID"), System.get_env("WEZTERM_EXECUTABLE")} do
  ...
  defp check_iterm2_kitty_support do
    ...
    {:ok, :unsupported}  # Conservative default
  end

  defp transmit_animation_frames(frames, options) do
    # Transmit each frame and collect image IDs
    results = Enum.with_index(frames)
    |> Enum.map(fn {frame_data, index} ->
      frame_options = Map.put(options, :image_id, index + 1000) # Offset for animation frames
