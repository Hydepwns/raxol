defmodule Raxol.Terminal.Graphics.UnifiedGraphics do
  ...
  * **Fallback Mode** - ASCII art conversion for unsupported terminals
  ...
  def set_preferred_protocol(protocol, fallback_enabled \\ true) do
    GenServer.call(__MODULE__, {:set_preferred_protocol, protocol, fallback_enabled})
  end
  ...
  def update_graphics_properties(graphics_id, properties) do
    GenServer.call(__MODULE__, {:update_graphics_properties, graphics_id, properties})
  end
  ...
  """
  @spec display_images([binary() | {binary(), display_options()}], display_options()) :: 
    {:ok, [graphics_id()]} | {:error, term()}
  ...
  def convert_and_display(image_data, target_format, options \\ %{}) do
    GenServer.call(__MODULE__, {:convert_and_display, image_data, target_format, options})
  end
  ...
  def init(opts) do
    ...
    cache_pid = case cache_enabled do
      true ->
    ...
    Logger.info("UnifiedGraphics initialized with support: #{inspect(graphics_support)}")
    Logger.info("Image cache #{if cache_enabled, do: "enabled", else: "disabled"}")
    {:ok, state}
  end

  def handle_call({:display_image, image_data, options}, _from, state) do
    with {:ok, protocol} <- select_protocol(options, state),
         {:ok, processed_options} <- prepare_display_options(options, protocol, state),
         {:ok, processed_image} <- process_image_with_cache(image_data, processed_options, state),
         {:ok, graphics_id} <- create_graphics_context(processed_options, state),
         {:ok, command} <- render_with_protocol(protocol, processed_image.data, processed_options) do
      
      # Execute the display command (would normally write to terminal)
      Logger.debug("Graphics command: #{inspect(command)}")
      
      new_state = update_graphics_state_with_image_info(graphics_id, command, processed_image, state)
  ...
  def handle_call({:display_images, images, shared_options}, _from, state) do
    ...
    case Enum.find(results, fn result -> match?({:error, _}, result) end) do
      nil ->
        {final_state, graphics_ids} = Enum.reduce(results, {state, []}, fn
  ...
  def handle_call({:convert_and_display, image_data, target_format, options}, _from, state) do
    conversion_options = Map.merge(options, %{
      format: target_format,
      reprocess: true
    })
    
    case handle_call({:display_image, image_data, conversion_options}, nil, state) do
  ...
  def handle_call({:optimize_and_display, image_data, options}, _from, state) do
    ...
    with {:ok, optimized_variants} <- ImageProcessor.optimize_for_terminals(image_data, terminal_profiles, options) do
      # Select the best variant for current terminal
      best_variant = select_best_variant(optimized_variants, state.graphics_support)
      
      # Display the optimized variant
      case handle_call({:display_image, best_variant.data, options}, nil, state) do
        {:reply, {:ok, graphics_id}, new_state} ->
          # Store optimization info
          updated_state = store_optimization_info(graphics_id, optimized_variants, new_state)
      ...
      {:ok, :kitty} ->
        ...
        case KittyProtocol.create_animation(frames, options) do
          {:ok, commands} ->
            ...
            new_state = %{
              state | 
              graphics: Map.put(state.graphics, graphics_id, %{
                type: :animation,
                protocol: :kitty,
                commands: commands,
  ...
  def handle_call({:set_preferred_protocol, protocol, fallback_enabled}, _from, state) do
  ...
  def handle_call({:update_graphics_properties, graphics_id, properties}, _from, state) do
    case Map.get(state.graphics, graphics_id) do
      nil ->
      ...
      graphics_info ->
        ...
        updated_info = Map.merge(graphics_info, %{properties: properties, updated_at: System.system_time(:millisecond)})
        
        # Apply the property changes based on protocol
        case apply_property_updates(graphics_id, properties, graphics_info.protocol) do
          :ok ->
  ...
  defp select_protocol(options, state) do
    ...
    case {requested_protocol, is_protocol_supported?(requested_protocol, state)} do
      {:auto, _} -> {:ok, state.preferred_protocol}
      {protocol, true} -> {:ok, protocol}
  ...
  defp prepare_display_options(options, protocol, state) do
    ...
    constrained_options = apply_capability_constraints(protocol_options, capabilities)
  ...
  defp render_with_protocol(:iterm2, image_data, options) do
    ...
    command = "\033]1337;File=inline=1;width=#{width};height=#{height}:#{encoded_data}\007"
    {:ok, command}
  end
  ...
  defp build_protocol_capabilities_map(graphics_support) do
    %{}
    |> add_protocol_capabilities(:kitty, graphics_support.kitty_graphics, graphics_support.capabilities)
    |> add_protocol_capabilities(:iterm2, graphics_support.iterm2_graphics, graphics_support.capabilities) 
    |> add_protocol_capabilities(:sixel, graphics_support.sixel_graphics, graphics_support.capabilities)
  end
          ...
          {:error, _} ->
            ...
            case ImageProcessor.process_image(image_data, options) do
              {:ok, processed_image} ->
                ...
                ImageCache.put(cache_key, processed_image.data, processed_image.metadata)
                {:ok, processed_image}
  ...
  defp update_graphics_state_with_image_info(graphics_id, command, processed_image, state) do
    graphics_info = %{
      id: graphics_id,
      command: command,
  ...
  defp create_terminal_profiles(graphics_support) do
    ...
    case graphics_support.terminal_type do
      :kitty -> 
        [Map.put(base_profile, :max_colors, 16_777_216) | profiles]  # True color
      :iterm2 ->
  ...
  defp select_best_variant(optimized_variants, graphics_support) do
    ...
    preferred_order = case graphics_support.terminal_type do
  ...
  defp store_optimization_info(graphics_id, optimized_variants, state) do
    case Map.get(state.graphics, graphics_id) do
      nil -> state
      graphics_info ->
        updated_info = Map.put(graphics_info, :optimization_variants, 
  ...
  defp apply_kitty_property_updates(graphics_id, properties) do
    ...
    commands = Enum.reduce(properties, [], fn {property, value}, acc ->
  ...
  defp build_kitty_property_command(graphics_id, property, value) do
    case property do
      ...
      :position when is_map(value) and is_number(value.x) and is_number(value.y) ->
