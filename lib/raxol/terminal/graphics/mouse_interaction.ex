defmodule Raxol.Terminal.Graphics.MouseInteraction do
  ...
  @default_config %{
    double_click_timeout: 300,  # milliseconds
  ...
  @impl true
  def handle_call({:register_element, graphics_id, element_config}, _from, state) do
  ...
  def handle_call({:handle_mouse_event, mouse_event}, _from, state) do
    ...
    event = Map.put_new(mouse_event, :timestamp, System.system_time(:millisecond))
    
    # Find elements under the mouse cursor
    hit_elements = find_elements_at_position(state.interactive_elements, {event.x, event.y})
  ...
  def handle_call({:update_bounds, graphics_id, new_bounds}, _from, state) do
    case Map.get(state.interactive_elements, graphics_id) do
      ...
      element ->
        ...
        elements = Map.put(state.interactive_elements, graphics_id, updated_element)
        {:reply, :ok, %{state | interactive_elements: elements}}
    end
  end
  ...
  defp find_elements_at_position(elements, {x, y}) do
    ...
    |> Enum.sort_by(& &1.z_index, :desc)  # Higher z-index first
  end
  ...
  defp handle_click_event(event, element, state) do
    ...
    {is_double_click, updated_history} = check_double_click(event, state.click_history, state.config)
    ...
    case {is_double_click, Map.get(element.callbacks, :on_double_click)} do
      {true, callback} when is_function(callback, 1) ->
        try do
          ...
          error -> Logger.warning("Double-click callback error: #{inspect(error)}")
        end
      _ -> :ok
    end
  ...
  defp handle_move_event(_event, hit_elements, state) do
    ...
    new_hover_state = case current_element do
      nil -> nil
      element -> element
    end
  ...
  defp check_double_click(event, history, config) do
    ...
    is_double = case new_history do
      [current, previous | _] ->
        time_diff = current.timestamp - previous.timestamp
        pos_diff = abs(current.x - previous.x) + abs(current.y - previous.y)
        
        time_diff <= config.double_click_timeout and pos_diff <= config.drag_threshold
      _ ->
        false
    end
