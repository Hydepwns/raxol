defmodule Raxol.Terminal.Graphics.ImageCache do
  ...
  * **Distributed Cache** - Shared cache across multiple processes (future)
  ...
  @default_config %{
    ...
    disk_cache_dir: nil,         # Will use system temp dir
  ...
  * Cache key string
  ...
  def generate_key(image_data, options \\ %{}) do
    ...
    combined_hash = :crypto.hash(:sha256, <<data_hash::binary, options_hash::binary>>)
  ...
  * `:ok` - Preloading completed
  ...
  def handle_cast({:preload, image_specs}, state) do
    final_state = Enum.reduce(image_specs, state, fn {key, data, metadata}, acc_state ->
  ...
  defp put_entry(key, data, metadata, state) do
    ...
    new_access_order = [key | List.delete(state_after_eviction.access_order, key)]
    new_memory_usage = state_after_eviction.memory_usage + data_size
    
    new_state = %{state_after_eviction |
      entries: new_entries,
  ...
  defp evict_lru_entries(state, target_memory, target_entries) do
    ...
    keys_to_evict = determine_eviction_candidates(state, target_memory, target_entries)
    
    final_state = Enum.reduce(keys_to_evict, state, fn key, acc_state ->
      remove_entry(key, acc_state)
    end)
    
    eviction_count = length(keys_to_evict)
    updated_stats = Map.update(final_state.stats, :evictions, eviction_count, & &1 + eviction_count)
  ...
  defp determine_eviction_candidates(state, target_memory, target_entries) do
    ...
    {_final_memory, _final_entries, evict_keys} = 
      Enum.reduce_while(candidates, {state.memory_usage, map_size(state.entries), []}, 
        fn key, {memory, entries, evict_list} ->
          case Map.get(state.entries, key) do
            nil -> {:cont, {memory, entries, evict_list}}
            entry ->
  ...
  defp calculate_cache_stats(state) do
    ...
    hit_rate = case total_requests > 0 do
  ...
  defp process_batch_operations(operations, state) do
    ...
    |> then(fn {final_state, results} -> {final_state, Enum.reverse(results)} end)
  end

  defp handle_get_operation(key, state) do
    # Similar to handle_call({:get, key}, ...) but returns tuple format for batch
    case Map.get(state.entries, key) do
      nil -> {update_stats(state, :miss), {:error, :not_found}}
      entry ->
        case is_expired?(entry, state.config.ttl) do
          true -> 
  ...
  defp perform_cleanup(state) do
    ...
    expired_keys = Enum.filter(Map.keys(state.entries), fn key ->
      case Map.get(state.entries, key) do
        nil -> false
  ...
  defp get_from_disk_cache(_key, %{enable_disk_cache: false}), do: {:error, :not_found}
  defp get_from_disk_cache(_key, _config), do: {:error, :not_found}
  
  defp write_to_disk_cache(_key, _data, _metadata, %{enable_disk_cache: false}), do: :ok
