defmodule Raxol.Terminal.Metrics.Manager do
  ...
  """
  def record_characters(%__MODULE__{} = manager, count)
  ...
  """
  def record_command(%__MODULE__{} = manager, command)
    ...
    usage_stats =
      Map.put(manager.usage_stats, :command_frequency, command_frequency)
  ...
  """
  def update_performance_metrics(%__MODULE__{} = manager, metrics)
  ...
  def update_usage_stats(%__MODULE__{} = manager, stats) when is_map(stats) do
    ...
    %{manager | usage_stats: usage_stats, last_update: DateTime.utc_now()}
  end

  @doc """
  Records feature usage.
  """
  def record_feature_usage(%__MODULE__{} = manager, feature)
      when is_binary(feature) do
    feature_usage =
      Map.update(
  ...
  """
  def set_custom_metric(%__MODULE__{} = manager, key, value)
      when is_binary(key) do
    custom_metrics = Map.put(manager.custom_metrics, key, value)
    %{manager | custom_metrics: custom_metrics, last_update: DateTime.utc_now()}
  end
