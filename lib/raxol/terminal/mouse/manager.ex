defmodule Raxol.Terminal.Mouse.Manager do
  ...
  """
  def set_mode(%__MODULE__{} = manager, mode) when mode in [:normal, :button_event, :any_event, :highlight_tracking] do
    %{manager | mode: mode}
  end
  ...
  """
  def set_button_state(%__MODULE__{} = manager, state) when state in [:none, :left, :middle, :right, :wheel_up, :wheel_down] do
  ...
  """
  def set_position(%__MODULE__{} = manager, {x, y} = position) when is_integer(x) and is_integer(y) do
