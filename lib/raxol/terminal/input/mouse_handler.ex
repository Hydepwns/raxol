defmodule Raxol.Terminal.Input.MouseHandler do
  ...
  def enable_mouse_tracking(mode) do
    case mode do
      :x10 -> "\e[?9h"                    # X10 compatibility mode
      :x11 -> "\e[?1000h"                  # Normal tracking mode
      :button_event -> "\e[?1002h"         # Button event tracking
      :any_event -> "\e[?1003h"            # Any event tracking
      :sgr -> "\e[?1006h"                  # SGR extended mode
      :urxvt -> "\e[?1015h"                # URXVT extended mode
  ...
  defp parse_urxvt_event(sequence) do
    case Regex.run(@urxvt_pattern, sequence) do
      [_, button_str, x_str, y_str] ->
        ...
        event = %{
          type: :press,  # URXVT doesn't distinguish press/release
          button: button,
          x: x - 1,
          y: y - 1,
          modifiers: modifiers,
  ...
  defp parse_x10_event(sequence) do
    case Regex.run(@x10_pattern, sequence) do
      [_, button_byte, x_byte, y_byte] ->
        ...
        x = x_raw - 33  # 32 offset + 1-based to 0-based
        y = y_raw - 33
  ...
  defp decode_sgr_button(code) do
    ...
    if code in [64, 65] do
      ...
      base_button = rem(code, 4)  # Only 4 basic buttons: 0,1,2,3
      modifier_bits = div(code, 4)
      
      button = case base_button do
        0 -> :left
        1 -> :middle  
        2 -> :right
        3 -> :release
  ...
  defp decode_x10_button(code) do
    ...
    type = cond do
      ...
      motion -> :move
      band(code, 64) == 64 -> :scroll  # Wheel events
      true -> :press
    end
    
    # Handle wheel events specially
    button = if type == :scroll do
  ...
  defp update_button_state(state, %{type: :press, button: button, x: x, y: y} = _event) 
       when not is_nil(button) do
    %{state | 
      pressed_buttons: MapSet.put(state.pressed_buttons, button),
      drag_start: {x, y}  # Set drag_start when button is pressed
    }
  end
  ...
  defp detect_multi_click(state, %{type: :press, x: x, y: y, timestamp: event_time} = _event) do
    now = event_time
      ...
      case state.last_click_time do
        ...
        last_time when now - last_time <= state.double_click_threshold ->
          ...
          case state.last_position do
            {last_x, last_y} when abs(x - last_x) <= 3 and abs(y - last_y) <= 3 ->
  ...
  defp generate_actions(state, event) do
    ...
    actions = if state.click_count == 2 do
      [{:double_click, event} | actions]
    else
      actions
    end
    
    actions = if state.click_count == 3 do
      [{:triple_click, event} | actions]
    else
      actions
    end
    
    actions = if state.drag_start && event.type == :move do
      [{:drag, %{start: state.drag_start, current: {event.x, event.y}}} | actions]
    else
      actions
    end
    
    actions = if MapSet.size(state.pressed_buttons) > 1 do
  ...
  def set_mouse_mode(mode, enable \\ true) do
    sequences = case mode do
      :click_only -> [9]           # X10 compatibility
      :click_drag -> [1000]        # Normal tracking
      :button_events -> [1002]     # Button event tracking  
      :all_events -> [1003]        # Any event tracking
      :sgr_extended -> [1006]      # SGR extended
      :urxvt_extended -> [1015]    # URXVT extended
      :focus_events -> [1004]      # Focus in/out events
      _ -> []
    end
