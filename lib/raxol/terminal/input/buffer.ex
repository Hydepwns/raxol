defmodule Raxol.Terminal.Input.Buffer do
  ...
  def new(max_size) when is_integer(max_size) and max_size > 0 do
    %{
      ...
      current_size: 0,
      callback: nil,
      input_buffer: ""  # String buffer for partial sequences
    }
  end
  ...
  defp parse_input_sequence(state, input) do
    ...
    case parse_events_recursive(combined_input, []) do
      {:ok, events, remaining} ->
        ...
        new_state = if remaining != "", do: %{state | input_buffer: remaining}, else: %{state | input_buffer: ""}
        {:ok, events, new_state}
      {:buffer, remaining} ->
  ...
  defp parse_next_event(input) do
    cond do
      ...
      String.match?(input, ~r/^\e\[(\d+);(\d+);(\d+);(\d+)M/) ->
        [_, button_code, action_code, x, y] = Regex.run(~r/^\e\[(\d+);(\d+);(\d+);(\d+)M/, input)
        ...
        sequence_length = String.length("\e[#{button_code};#{action_code};#{x};#{y}M")
      ...
      String.match?(input, ~r/^\e\[(\d+);(\d+);(\d+)([A-Z])/) ->
        [_, param1, param2, param3, key] = Regex.run(~r/^\e\[(\d+);(\d+);(\d+)([A-Z])/, input)
      ...
      String.match?(input, ~r/^\e\[(\d+);(\d+)([A-Z])/) ->
        [_, modifier_code, key_code, key] = Regex.run(~r/^\e\[(\d+);(\d+)([A-Z])/, input)
      ...
      String.length(input) >= 1 and not String.match?(String.first(input), ~r/[\x00-\x1F]/) ->
  ...
  defp parse_modifiers(params) do
    modifiers = Enum.reduce(params, [], fn param, acc ->
      case param do
        "1" -> acc ++ [:shift]
        "2" -> acc ++ [:shift]
        "3" -> acc ++ [:alt]
