defmodule Raxol.Agent.SemanticTree do
  ...
  defp maybe_add_focused(%{id: id} = node, focused_id) when not is_nil(focused_id) do
    Map.put(node, :focused, id == focused_id)
  end
  defp maybe_add_focused(node, _), do: node

  defp transform_children(%{children: children} = node, focused_id) when is_list(children) do
