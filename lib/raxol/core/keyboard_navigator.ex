defmodule Raxol.Core.KeyboardNavigator do
  ...
  * `height` - Height of the component
  ...
  def register_component_position(component_id, x, y, width, height) do
    ...
    updated_map = Map.put(spatial_map, component_id, %{
      id: component_id,
      x: x,
      y: y,
      width: width,
  ...
  * `handler` - Function to call when shortcut is pressed
  ...
  defp find_closest_in_direction(current_pos, direction, spatial_map) do
    ...
    candidates = 
      ...
      |> Map.values()
      |> Enum.filter(fn pos -> 
        pos.id != current_pos.id && is_in_direction?(current_pos, pos, direction)
      end)
    
    # Find the closest one
    if length(candidates) > 0 do
      candidates
      |> Enum.sort_by(fn pos -> distance_in_direction(current_pos, pos, direction) end)
      |> hd()
      |> Map.get(:id)
    else
      nil
