defmodule Raxol.Core.I18n.Server do
  ...
  def init(config) do
    ...
    state = %__MODULE__{
      config: config_map,
      event_manager: Map.get(config_map, :event_manager, Raxol.Core.Events.Manager),
      accessibility_module: Map.get(config_map, :accessibility_module, Raxol.Core.Accessibility),
  ...
  def handle_call({:init_i18n, config}, _from, state) do
    ...
    new_state = %{state |
      ...
      accessibility_module: Map.get(config_map, :accessibility_module, state.accessibility_module),
      current_locale: Map.get(config_map, :default_locale, state.current_locale),
  ...
  def handle_call({:add_translations, locale, translations}, _from, state) do
    updated_translations = Map.update(
      state.translations,
      locale,
      translations,
  ...
  defp load_translations(state, locale) do
    ...
    default_translations = case locale do
      "fr" ->
        %{
    ...
    updated_translations = Map.put(state.translations, locale, default_translations)
