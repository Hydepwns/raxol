defmodule Raxol.Core.UXRefinement.Server do
  ...
  """
  def register_accessibility_metadata(server \\ __MODULE__, component_id, metadata) do
    GenServer.call(server, {:register_accessibility_metadata, component_id, metadata})
  end
  ...
  @impl GenServer
  def handle_call({:register_component_hint, component_id, hint_info}, _from, state) do
  ...
  @impl GenServer
  def handle_call({:register_accessibility_metadata, component_id, metadata}, _from, state) do
    new_state = if MapSet.member?(state.features, :accessibility) do
  ...
  def handle_call({:get_accessibility_metadata, component_id}, _from, state) do
    metadata = if MapSet.member?(state.features, :accessibility) do
  ...
  defp do_enable_feature(:focus_management, _opts, user_prefs, state) do
    ...
    new_state = %{state | features: MapSet.put(state.features, :focus_management)}
    {:ok, new_state}
  end
  ...
  defp do_enable_feature(:keyboard_navigation, _opts, user_prefs, state) do
    ...
    new_state = %{state | features: MapSet.put(state.features, :keyboard_navigation)}
    {:ok, new_state}
  end
  ...
  defp do_enable_feature(:keyboard_shortcuts, _opts, user_prefs, state) do
    ...
    state = ensure_feature_enabled(:events, user_prefs, state)
    new_state = %{state | features: MapSet.put(state.features, :keyboard_shortcuts)}
    {:ok, new_state}
  end
  ...
  defp do_disable_feature(:keyboard_shortcuts, state) do
    ...
    new_state = %{state | features: MapSet.delete(state.features, :keyboard_shortcuts)}
    {:ok, new_state}
  end
  ...
  defp do_disable_feature(:events, state) do
    if MapSet.member?(state.features, :accessibility) ||
      ...
      {{:error, "Cannot disable events while accessibility or keyboard shortcuts are enabled"}, state}
    else
  ...
  defp do_register_component_hint(component_id, hint_info, state) when is_binary(hint_info) do
    do_register_component_hint(component_id, %{basic: hint_info}, state)
  end
  
  defp do_register_component_hint(component_id, hint_info, state) when is_map(hint_info) do
  ...
  defp handle_accessibility_focus_change(old_focus, new_focus, user_prefs, state) do
    if MapSet.member?(state.features, :accessibility) do
      ...
      announcement = if is_nil(old_focus) do
        "Focus set to #{label}"
      else
        old_label = Map.get(state.metadata, old_focus, %{}) |> Map.get(:label, old_focus)
        "Focus moved from #{old_label} to #{label}"
      end
      
      accessibility_module().announce(announcement, [priority: :low], user_prefs)
    end
