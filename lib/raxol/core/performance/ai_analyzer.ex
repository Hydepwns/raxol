defmodule Raxol.Core.Performance.AIAnalyzer do
  ...
  alias Raxol.Core.Performance.{Analyzer, AIIntegration}
  ...
  def analyze(metrics, options \\ %{}) do
    ...
    case AIIntegration.analyze(ai_data, options) do
      {:ok, analysis} ->
        analysis
        
      {:error, reason} ->
        Logger.warning("AI analysis failed: #{inspect(reason)}, falling back to mock analysis")
