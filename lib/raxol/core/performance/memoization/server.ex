defmodule Raxol.Core.Performance.Memoization.Server do
  ...
  def init(opts) do
    ...
    state = %{
      ...
      max_entries_per_process: Keyword.get(opts, :max_entries_per_process, 1000),
  ...
  def handle_call(:stats, _from, state) do
    ...
    stats = %{
      hits: state.hits,
      misses: state.misses,
      hit_rate: if state.hits + state.misses > 0 do
  ...
  def handle_info(:cleanup, state) do
    ...
    cache = if state.ttl != :infinity do
      state.cache
