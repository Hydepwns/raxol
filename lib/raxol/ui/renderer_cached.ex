defmodule Raxol.UI.RendererCached do
  ...
  defp render_individual_cached(elements, theme) do
    ...
    |> Enum.flat_map(fn element ->
      element_theme = ThemeResolverCached.resolve_element_theme_with_inheritance(
        element,
        theme
      )
  ...
  defp render_tree_recursive(%{visible: false}, _theme, _parent_style, _depth), do: []
  
  defp render_tree_recursive(element, theme, parent_style, depth) when depth > 50 do
  ...
  defp render_tree_recursive(element, theme, parent_style, depth) do
    ...
    element_cells = if cacheable_element?(element) do
    ...
    children_cells = case Map.get(element, :children) do
      nil -> []
      children when is_list(children) ->
        # Use cached style processing
        element_style = StyleProcessorCached.flatten_merged_style(
          parent_style,
          element,
          theme
        )
  ...
  defp cacheable_element?(%{type: type}) when type in [:text, :box, :panel, :button], do: true
  defp cacheable_element?(%{type: :input, value: value}) when is_binary(value), do: true
  ...
  defp calculate_visible_range(items, %{height: height, item_height: item_height}) do
