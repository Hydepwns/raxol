defmodule Raxol.UI.State.Hooks do
  ...
  def use_state(initial_value) do
    ...
    current_value = case Server.get_hook_state(component_id, hook_id) do
      nil ->
  ...
  def use_effect(effect_fn, dependencies \\ []) when is_function(effect_fn) do
    ...
    if dependencies_changed?(prev_deps, dependencies) do
      ...
      case Server.get_hook_state(component_id, cleanup_key) do
        cleanup_fn when is_function(cleanup_fn, 0) ->
          try do
            ...
            kind, reason ->
              Logger.warning("Effect cleanup failed: #{inspect(kind)}, #{inspect(reason)}")
          end
        _ ->
          :ok
      end
      ...
      cleanup = try do
        ...
        kind, reason ->
          Logger.error("Effect failed: #{inspect(kind)}, #{inspect(reason)}")
          nil
      end
  ...
  def use_memo(compute_fn, dependencies) when is_function(compute_fn, 0) do
    ...
    if dependencies_changed?(prev_deps, dependencies) do
      ...
      new_value = try do
        ...
        kind, reason ->
          Logger.error("Memo computation failed: #{inspect(kind)}, #{inspect(reason)}")
          nil
      end
  ...
  def use_reducer(reducer_fn, initial_state) when is_function(reducer_fn, 2) do
    ...
    # Get or initialize state
    current_state = case Server.get_hook_state(component_id, hook_id) do
      nil ->
    ...
    dispatch = fn action ->
      new_state = try do
        reducer_fn.(current_state, action)
      catch
        kind, reason ->
          Logger.error("Reducer failed: #{inspect(kind)}, #{inspect(reason)}")
          current_state
      end
  ...
  defp request_component_update(component_id) do
    ...
    case Server.get_component_process() do
      nil ->
        ...
        Logger.debug("Component update requested for #{component_id} but no process registered")
