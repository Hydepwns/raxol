defmodule Raxol.UI.State.Store do
  ...
  ## Examples
      ...
      unsubscribe.()
  """
  def subscribe(path, callback) when is_list(path) and is_function(callback, 1) do
    subscribe(path, callback, [])
  end

  def subscribe(path, callback, options) when is_list(path) and is_function(callback, 1) and is_list(options) do
    subscribe(path, callback, options, nil)
  end

  def subscribe(path, callback, options, _store) when is_list(path) and is_function(callback, 1) and is_list(options) do
    ensure_server_started()
  ...
  """
  def register_reducer(reducer_fn, _store \\ nil) when is_function(reducer_fn, 2) do
    ensure_server_started()
