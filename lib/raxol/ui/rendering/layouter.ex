defmodule Raxol.UI.Rendering.Layouter do
  ...
  defp do_layout_node_and_children(node_content, diff_for_this_node)
    ...
    unless Map.has_key?(node_content, :children) do
      Raxol.Core.Runtime.Log.warning_with_context(
        "Layout Engine: Map node missing :children key: #{inspect(node_content)}",
        %{}
      )
      ...
      processed_children =
        case diff_for_this_node do
          {:replace, _} ->
            Enum.map(
              Map.get(node_with_own_layout, :children, []),
          ...
          {:update_children, child_changes_list} ->
            children = Map.get(node_with_own_layout, :children, [])
            map_child_changes_to_new_children_list(children, child_changes_list)

          _unknown_diff ->
            Raxol.Core.Runtime.Log.warning_with_context(
              "Layout Engine: Unhandled diff for node, doing full child relayout: #{inspect(diff_for_this_node)} on node #{inspect(Map.get(node_with_own_layout, :type, :unknown))}",
              %{}
            )
  ...
  defp do_layout_node_and_children(node_content, _diff_for_this_node)
       when is_list(node_content) do
    Raxol.Core.Runtime.Log.warning_with_context(
      "Layout Engine: Encountered list node (likely []): #{inspect(node_content)}. Returning as-is.",
      %{}
    )
  ...
  defp dummy_layout_for_node(node_content, diff_type) do
    %{
      x: 0,
      y: 0,
      width: 10,
