defmodule Raxol.UI.Rendering.ComponentCache do
  ...
  def render_elements_cached(elements, theme) when is_list(elements) do
    ...
    cache_keys = Enum.map(elements, fn element ->
      element_theme = ThemeResolver.resolve_element_theme_with_inheritance(element, theme)
  ...
  defp hash_element(element) when is_map(element) do
    ...
    relevant_keys = [:type, :x, :y, :width, :height, :text, :visible, 
                     :label, :value, :children, :style, :class, :id]
