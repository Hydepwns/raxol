defmodule Raxol.UI.Components.VirtualScrolling do
  ...
  def handle_call({:scroll_to_position, position, options}, _from, state) do
    ...
    new_scroll_state =
      build_scroll_state(state.scroll_state, clamped_position, smooth)
  ...
  defp calculate_visible_range(state) do
    ...
    {start_index, end_index} =
      calculate_range_by_height_type(state, scroll_top, viewport_height)
  ...
  defp recalculate_total_height(state) do
    total_height =
      calculate_total_height_by_type(state.config.item_height, state)
  ...
  defp calculate_position_for_index(
  ...
  defp maybe_trigger_infinite_scroll(
         %{infinite_scroll: %{enabled: false}} = state
       ),
       do: state

  defp maybe_trigger_infinite_scroll(
         %{infinite_scroll: %{has_more: false}} = state
       ),
       do: state

  defp maybe_trigger_infinite_scroll(
         %{infinite_scroll: %{loading: true}} = state
       ),
       do: state
  ...
  defp maybe_trigger_infinite_scroll(state) do
    ...
    check_and_trigger_infinite_load(
      state,
      scroll_position,
      max_scroll,
      threshold
  ...
  defp check_and_trigger_infinite_load(
         state,
         scroll_position,
         max_scroll,
         threshold
  ...
  defp calculate_performance_stats(
         %{performance_monitor: %{enabled: false}} = _state
       ) do
  ...
  defp calculate_performance_stats(state) do
    ...
    cache_hit_rate =
      calculate_cache_hit_rate(state.performance_monitor, cache_total)
    ...
    %{
      ...
      render_time_ms: elapsed / max(1, state.performance_monitor.render_count),
      total_items: state.config.item_count,
  ...
  defp calculate_content_height(length, base) when length > 50,
    do: round(base * 1.5)
  ...
  defp announce_scroll_if_enabled(
         %{config: %{accessibility_enabled: true}} = state,
         index
       ) do
  ...
  defp perform_search_by_config(
         %{search_config: %{search_entire_dataset: true}} = state,
         query,
         search_fn,
         loaded_matches
       ) do
