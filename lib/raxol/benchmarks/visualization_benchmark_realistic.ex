  ...
  def run_benchmark do
    ...
    chart_results =
      Enum.map(chart_sizes, fn size ->
        ...
        IO.puts("| #{String.pad_trailing(Integer.to_string(size), 6)} | #{String.pad_trailing("#{Float.round(first_ms, 2)}ms", 12)} | #{String.pad_trailing("#{Float.round(second_ms, 2)}ms", 13)} | #{String.pad_trailing("#{Float.round(speedup, 1)}x", 8)} |")
    ...
    treemap_results =
      Enum.map(treemap_sizes, fn size ->
        ...
        first_ms = first_time / 1000
        second_ms = second_time / 1000
        IO.puts("| #{String.pad_trailing(Integer.to_string(size), 6)} | #{String.pad_trailing(Integer.to_string(node_count), 6)} | #{String.pad_trailing("#{Float.round(first_ms, 2)}ms", 12)} | #{String.pad_trailing("#{Float.round(second_ms, 2)}ms", 13)} | #{String.pad_trailing("#{Float.round(speedup, 1)}x", 8)} |")
    ...
    avg_chart_speedup = Enum.sum(Enum.map(chart_results, & &1.speedup)) / length(chart_results)
    avg_treemap_speedup = Enum.sum(Enum.map(treemap_results, & &1.speedup)) / length(treemap_results)
    ...
    treemap_time_ratio = largest_treemap.first_render_ms / smallest_treemap.first_render_ms
    treemap_efficiency = treemap_size_ratio / treemap_time_ratio

    IO.puts("\nScaling Efficiency:")
    IO.puts("Chart: #{Float.round(chart_efficiency, 2)} (higher is better, 1.0 means linear scaling)")
    IO.puts("TreeMap: #{Float.round(treemap_efficiency, 2)} (higher is better, 1.0 means linear scaling)")

    IO.puts("\nConclusion:")
    conclusion = case {chart_efficiency, treemap_efficiency} do
  ...
  defp render_chart_content(data, size, bounds, state) do
    ...
    case Map.get(state, :layout_cache, %{}) |> Map.get(cache_key) do
      nil ->
        ...
        # Large dataset: ~500ms
        base_time = 30
        factor = :math.log(size) / :math.log(10)  # log10(size)
