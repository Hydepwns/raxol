defmodule Raxol.Benchmarks.VisualizationBenchmarkSimple do
  ...
  def run_simple_benchmark do
    ...
    {first_chart_time, _} = :timer.tc(fn ->
      render_chart_content(chart_data, %{title: "Test Chart"}, bounds, plugin_state)
    end)
    ...
    {second_chart_time, _} = :timer.tc(fn ->
      render_chart_content(chart_data, %{title: "Test Chart"}, bounds, updated_state)
    end)
    ...
    # First render (should miss cache)
    {first_treemap_time, _} = :timer.tc(fn ->
      render_treemap_content(treemap_data, %{title: "Test TreeMap"}, bounds, plugin_state)
    end)
    ...
    {second_treemap_time, _} = :timer.tc(fn ->
      render_treemap_content(treemap_data, %{title: "Test TreeMap"}, bounds, updated_state)
    end)
  ...
  defp render_chart_content(data, opts, bounds, state) do
    ...
    case Map.get(state, :layout_cache, %{}) |> Map.get(cache_key) do
      nil ->
        ...
        Process.sleep(50) # Simulate calculation time
        "chart_cells"
  ...
  defp render_treemap_content(data, opts, bounds, state) do
    ...
    case Map.get(state, :layout_cache, %{}) |> Map.get(cache_key) do
      nil ->
        # No cache hit - would normally calculate layout and draw
        # For testing, just simulate the work
        Process.sleep(100) # Simulate calculation time
        "treemap_cells"
