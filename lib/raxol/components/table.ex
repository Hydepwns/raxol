defmodule Raxol.Components.Table do
  ...
  * `:footer` - Function that renders a footer row or rows
  ...
  def render(data, columns, opts \\ []) do
    ...
    selected_style = Keyword.get(opts, :selected_style, %{bg: :blue, fg: :white})
    ...
    select_key = Keyword.get(opts, :select_key, 
    ...
    View.panel([id: id, style: table_style], fn ->
      View.column([], fn ->
        ...
        # Render table body with optional max height constraint
        body_props = if max_height do
          [style: %{height: max_height, overflow: :scroll}]
        else
          []
        end
        
        View.panel(body_props, fn ->
          View.column([], fn ->
            # Render each row
            sorted_data
            |> Enum.with_index()
            |> Enum.map(fn {row_data, index} ->
              ...
              is_selected = selected && Map.get(row_data, select_key) == selected
              
              # Determine row style based on zebra striping and selection
              row_final_style = cond do
                is_selected -> 
              ...
              row_click_handler = if on_select do
              ...
              row_props = if row_click_handler do
  ...
  * `:total_items` - Total number of items (for display only)
  ...
  defp render_header(columns, style, sort_by, sort_dir, on_sort) do
    View.row([style: style], fn ->
      ...
      |> Enum.map(fn column ->
        ...
        sort_indicator = if is_sorted do
          case sort_dir do
            :asc -> " ↑"
            :desc -> " ↓"
            _ -> ""
        ...
        # Create click handler for column header if sortable
        header_click = if column.sortable && on_sort do
          fn ->
        ...
        header_props = 
          if Map.has_key?(column, :width) do
            Keyword.put(header_props, :style, Map.put(Keyword.get(header_props, :style), :width, column.width))
          else
            constraints = []
            ...
            if constraints != [] do
              Keyword.put(header_props, :style, Map.merge(Keyword.get(header_props, :style), Map.new(constraints)))
            else
              header_props
            end
          end
        
        # Add click handler if column is sortable
        header_props = 
          if header_click do
            Keyword.put(header_props, :on_click, header_click)
          else
            header_props
          end
        
        # Render the header cell  
        View.panel(header_props, fn ->
          View.text("#{column.label}#{sort_indicator}")
        end)
      end)
    end)
  end
  ...
  defp render_cell(row_data, column, row_index) do
    ...
    cell_props = 
      if Map.has_key?(column, :width) do
        Keyword.put(cell_props, :style, Map.put(cell_style, :width, column.width))
      else
        constraints = []
        ...
        if constraints != [] do
          Keyword.put(cell_props, :style, Map.merge(cell_style, Map.new(constraints)))
        else
          cell_props
        end
      end
    ...
    # Render the cell
    View.panel(cell_props, fn ->
      View.text(display_value)
    end)
  end
