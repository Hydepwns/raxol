defmodule Raxol.Components.Input.MultiLineInput.RenderHelper do
  ...
  def render_line(line_index, line, state) do
    ...
    Raxol.View.Elements.row [] do
      # Combine line number and content, filtering out nil line number
  ...
  def render_line_with_cursor(line, state) do
    ...
    [
      # Use label macro
      Raxol.View.Elements.label(content: before_cursor, style: [color: state.style.text_color]),
      Raxol.View.Elements.label(content: "│", style: [color: state.style.cursor_color]),
      Raxol.View.Elements.label(content: after_cursor, style: [color: state.style.text_color])
    ]
  end
