defmodule Raxol.Components.Progress do
  ...
  def bar(value, opts \\ []) do
    ...
    View.row([id: id, style: style], fn ->
      if filled_width > 0 do
        View.text([style: filled_style], filled_portion)
      end
      
      if empty_width > 0 do
        View.text([style: empty_style], empty_portion)
      end
    end)
  end
      ...
      :above ->
        View.column([id: id], fn ->
          View.row([style: %{justify: :space_between}], fn ->
            View.text([style: label_style], label)
            if percentage_text, do: View.text([style: percentage_style], percentage_text)
          end)
          bar(value, opts)
        end)
  ...
  * `:type` - Type of spinner animation (default: :dots)
  ...
  def spinner(message \\ nil, frame, opts \\ []) do
    ...
    View.row([id: id, style: style], fn ->
      View.text([style: spinner_style], current_frame)
      
      if message do
        View.text([style: message_style], " #{message}")
      end
    end)
  end
  ...
  * `:segment_size` - Size of the animated segment (default: 5)
  ...
  def indeterminate(frame, opts \\ []) do
    ...
    View.row([id: id, style: style], fn ->
      if left_width > 0 do
        View.text([style: background_style], String.duplicate(" ", left_width))
      end
      
      View.text([style: bar_style], String.duplicate(" ", bar_width))
      
      if right_width > 0 do
        View.text([style: background_style], String.duplicate(" ", right_width))
      end
    end)
  end
  ...
  * `:percentage_style` - Style for the percentage text
  ...
  def circular(value, opts \\ []) do
    ...
    View.row([id: id], fn ->
      View.text([style: style], progress_char)
      if show_percentage do
        View.text([style: percentage_style], " #{percentage_text}")
      end
    end)
  end
end 