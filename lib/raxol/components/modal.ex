...
defmodule Raxol.Components.Modal do
  ...
  def update(msg, state) do
    ...
    case msg do
      ...
      _ ->
        Logger.warning(
          "Modal #{state.id} received unknown message: #{inspect(msg)}"
        )
  ...
  def render(state, %{} = _props) do
    if state.visible do
      ...
      modal_box_id = if state.id, do: "#{state.id}-box", else: nil
  ...
  defp render_form_content(state) do
    ...
    field_elements =
      ...
      |> Enum.map(fn {field, index} ->
        ...
        field_full_id =
          if state.id, do: "#{state.id}.#{field.id}", else: field.id
