defmodule Raxol.Components.Modal do
  ...
  * `:on_escape` - Function to call when Escape key is pressed (default: nil)
  ...
  def render(title, content_fn, actions_fn, opts \\ []) do
    ...
    style = Keyword.get(opts, :style, %{border: true})
    title_style = Keyword.get(opts, :title_style, %{fg: :white, bold: true})
    content_style = Keyword.get(opts, :content_style, %{})
    actions_style = Keyword.get(opts, :actions_style, %{})
    show_backdrop = Keyword.get(opts, :backdrop, true)
    backdrop_style = Keyword.get(opts, :backdrop_style, %{bg: :black, transparent: 0.7})
    centered = Keyword.get(opts, :centered, true)
    on_escape = Keyword.get(opts, :on_escape, nil)
    
    # Default height for the content area
    content_height = 
      if height == :auto do
        nil  # Let the content determine the height
      else
        height - 4  # Account for borders, title, and actions area
      end
