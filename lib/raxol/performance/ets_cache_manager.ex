defmodule Raxol.Performance.ETSCacheManager do
  ...
  def get_csi(sequence) do
    case :ets.lookup(@csi_parser_cache, sequence) do
      [{^sequence, result, _timestamp}] ->
        ...
        :ets.update_element(@csi_parser_cache, sequence, {3, System.monotonic_time()})
        {:ok, result}
      [] ->
        :miss
    end
  ...
  def get_font_metrics(key) do
    case :ets.lookup(@font_metrics_cache, key) do
      [{^key, result, _timestamp}] ->
        :ets.update_element(@font_metrics_cache, key, {3, System.monotonic_time()})
        {:ok, result}
      [] ->
        :miss
    end
  ...
  defp constraints_compatible?(cached, target) when is_list(cached) and is_list(target) do
  ...
  def init(_opts) do
    ...
    :ets.new(@csi_parser_cache, [:set, :public, :named_table, read_concurrency: true])
    :ets.new(@cell_cache, [:set, :public, :named_table, read_concurrency: true, write_concurrency: true])
    :ets.new(@style_cache, [:set, :public, :named_table, read_concurrency: true])
    :ets.new(@buffer_cache, [:set, :public, :named_table, read_concurrency: true, write_concurrency: true])
    :ets.new(@layout_cache, [:set, :public, :named_table, read_concurrency: true])
    :ets.new(@font_metrics_cache, [:set, :public, :named_table, read_concurrency: true, write_concurrency: true])
