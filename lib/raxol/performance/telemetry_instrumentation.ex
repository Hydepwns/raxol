defmodule Raxol.Performance.TelemetryInstrumentation do
  ...
  """
  def instrument(event_name, metadata \\ %{}, fun) when is_atom(event_name) and is_function(fun, 0) do
  ...
  def render_start(component_type, props_hash) do
    start_span(:ui_component_render, %{component: component_type, props_hash: props_hash})
  end
  ...
  def layout_calculation(tree_hash, constraints, fun) do
    instrument(:ui_layout_calculate, %{tree_hash: tree_hash, constraints: inspect(constraints)}, fun)
  end
  
  def style_resolution(theme_id, component_type, fun) do
    instrument(:ui_style_resolve, %{theme_id: theme_id, component: component_type}, fun)
  end
  ...
  def parse_csi(sequence, fun) do
    instrument(:terminal_parse, %{
      sequence: String.slice(sequence, 0, 20),  # Truncate for telemetry
      operation: :parse_csi
    }, fun)
  end
  ...
  defp handle_performance_event(event, measurements, metadata, _config) do
    if measurements.duration > 100 do  # Log operations over 100μs
      require Logger
      Logger.debug("""
  ...
  defp handle_cache_event([:raxol, :cache, type], measurements, metadata, _config) do
  ...
  defp handle_slow_operation(event, measurements, metadata, %{threshold: threshold}) do
