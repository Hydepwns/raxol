defmodule Raxol.Plugins.NotificationPlugin do
    ...
    cond do
      String.starts_with?(input, "/notify ") ->
        ...
        case parse_notification_command(input) do
          {:ok, type, message} ->
            show_notification(plugin, type, message)
          {:error, reason} ->
            {:error, reason}
        end
  ...
  defp parse_notification_command(command) do
    case String.slice(command, 0..7//1) do
      "notify: " ->
        message = String.slice(command, 8..-1//1)
        {:ok, message}
      _ ->
  ...
  defp parse_config_command(command) do
    case String.slice(command, 0..13//1) do
      "notification: " ->
        config = String.slice(command, 14..-1//1)
