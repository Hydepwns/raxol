defmodule Raxol.Plugins.PluginDependency do
  ...
  def check_dependencies(plugin, loaded_plugins) when is_map(plugin) and is_list(loaded_plugins) do
    ...
    Enum.reduce_while(dependencies, {:ok, []}, fn dependency, {:ok, missing} ->
      ...
      case find_plugin(loaded_plugins, plugin_name) do
        ...
        loaded_plugin ->
          case check_version_compatibility(loaded_plugin.version, version_constraint) do
            :ok ->
              {:cont, {:ok, missing}}
            {:error, reason} ->
              if optional do
                ...
                {:halt, {:error, "Version mismatch for '#{plugin_name}': #{reason}"}}
              end
          end
      end
    end)
  ...
  def check_api_compatibility(plugin_api_version, manager_api_version) do
    ...
    if plugin_major == manager_major do
      ...
      {:error, "API version mismatch: plugin requires API version #{plugin_api_version}, but manager provides #{manager_api_version}"}
    end
  end
  ...
  defp detect_cycles(graph) do
    ...
    case Enum.find(Map.keys(graph), fn node ->
      ...
      nil -> {:ok, graph}
      cycle_start -> {:error, find_cycle_path(graph, cycle_start, cycle_start, [])}
    end
  end
    ...
    case Regex.run(~r/^([<>=]+)\s*([0-9.]+)$/, constraint) do
      [_, op, version_str] ->
        case compare_versions(version, version_str) do
          ...
          :eq when op in [">=", "<=", "="] -> :ok
          :gt when op in ["<", "<="] -> :ok
          _ -> {:error, "Version #{version} does not satisfy constraint #{constraint}"}
        end
      _ ->
  ...
  defp compare_version_parts([a | rest_a], [a | rest_b]), do: compare_version_parts(rest_a, rest_b)
