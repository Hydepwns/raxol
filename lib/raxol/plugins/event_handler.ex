defmodule Raxol.Plugins.EventHandler do
  ...
  """
  @spec handle_input(PluginManager.t(), binary()) :: {:ok, PluginManager.t()} | {:error, any()}
  def handle_input(%PluginManager{} = manager, input) do
    dispatch_event(manager, :handle_input, [input], {:ok, manager}, &handle_simple_update/4)
  end

  @doc """
  Dispatches a 'resize' event to all enabled plugins implementing `handle_resize/3`.
  """
  @spec handle_resize(PluginManager.t(), integer(), integer()) :: {:ok, PluginManager.t()} | {:error, any()}
  def handle_resize(%PluginManager{} = manager, width, height) do
    dispatch_event(manager, :handle_resize, [width, height], {:ok, manager}, &handle_simple_update/4)
  end
  ...
  """
  @spec handle_mouse_legacy(PluginManager.t(), tuple(), map()) :: {:ok, PluginManager.t()} | {:error, any()}
  def handle_mouse_legacy(%PluginManager{} = manager, event, emulator_state) do
    dispatch_event(manager, :handle_mouse, [event, emulator_state], {:ok, manager}, &handle_simple_update/4)
  end
  ...
  """
  @spec handle_output(PluginManager.t(), binary()) :: {:ok, PluginManager.t(), binary()} | {:error, any()}
  def handle_output(%PluginManager{} = manager, output) do
    # Initial accumulator includes the output
    initial_acc = {:ok, manager, output}
    dispatch_event(manager, :handle_output, [output], initial_acc, &handle_output_update/4)
  end
  ...
  def handle_mouse_event(%PluginManager{} = manager, event, rendered_cells) do
    ...
    dispatch_event(manager, :handle_mouse, [event, rendered_cells], initial_acc, &handle_mouse_update/4)
  end
  ...
  @spec handle_key_event(PluginManager.t(), map(), map()) ::
          {:ok, PluginManager.t(), list(any()), :propagate | :halt} | {:error, any()}
  def handle_key_event(%PluginManager{} = manager, event, rendered_cells) do
    ...
    dispatch_event(manager, :handle_input, [event, rendered_cells], initial_acc, &handle_key_update/4)
  end
  ...
  @type handler_payload :: accumulator() | {:error, term()} # Extracted type
  @type handler_result :: {:cont | :halt, handler_payload()} # Simplified line 82
  # @type result_handler_fun :: (accumulator(), map(), callback_name(), any()) -> handler_result() # Commented out

  @spec dispatch_event(PluginManager.t(), callback_name(), event_args(), accumulator(), fun()) ::
          accumulator() | {:error, term()} # Use fun() instead
  defp dispatch_event(manager, callback_name, event_args, initial_accumulator, handle_result_fun) do
    required_arity = length(event_args) + 1 # +1 for the plugin state itself

    Enum.reduce_while(manager.plugins, initial_accumulator, fn {_plugin_name, plugin},
                                                             acc ->
      # Ensure accumulator is not an error before proceeding
      case acc do
        {:error, _reason} -> {:halt, acc} # Stop if previous step resulted in error
        _ -> # Proceed if accumulator is ok
  ...
  defp extract_manager_from_acc({:ok, manager, _, _, _}), do: manager # Add more patterns if needed
  defp extract_manager_from_acc(_), do: nil # Or raise an error
  ...
  @spec handle_simple_update(accumulator(), map(), callback_name(), term()) :: handler_result
  defp handle_simple_update({:ok, acc_manager}, plugin, _callback_name, plugin_result) do
    case plugin_result do
      ...
      {:error, reason} ->
        Logger.error("Plugin #{plugin.name} failed during event handling: #{inspect(reason)}")
        # Halt the reduce_while loop on the first plugin error
        {:halt, {:error, reason}}

      _other ->
        # Log a warning for unexpected return values but continue
        Logger.warning("Plugin #{plugin.name} returned unexpected value: #{inspect(_other)}. Ignoring.")
        {:cont, {:ok, acc_manager}}
    end
  end
  # Handle cases where acc might already be an error
  defp handle_simple_update(error_acc = {:error, _}, _plugin, _callback_name, _plugin_result), do: {:halt, error_acc}

  # Handles results for handle_output, accumulating transformed output.
  @spec handle_output_update(accumulator(), map(), callback_name(), term()) :: handler_result
  defp handle_output_update({:ok, acc_manager, acc_output}, plugin, _callback_name, plugin_result) do
    case plugin_result do
      ...
      {:ok, updated_plugin, transformed_output} when is_binary(transformed_output) ->
        new_manager_state = %{
            acc_manager
            | plugins: Map.put(acc_manager.plugins, plugin.name, updated_plugin)
          }
      ...
      {:error, reason} ->
        Logger.error("Plugin #{plugin.name} failed during output handling: #{inspect(reason)}")
        {:halt, {:error, reason}}

      _other ->
        Logger.warning("Plugin #{plugin.name} returned unexpected value from handle_output: #{inspect(_other)}. Ignoring.")
        {:cont, {:ok, acc_manager, acc_output}}
    end
  end
  defp handle_output_update(error_acc = {:error, _}, _plugin, _callback_name, _plugin_result), do: {:halt, error_acc}

  # Handles results for handle_mouse_event, managing :halt propagation.
  @spec handle_mouse_update(accumulator(), map(), callback_name(), term()) :: handler_result
  defp handle_mouse_update({:ok, acc_manager, _propagation_state}, plugin, _callback_name, plugin_result) do
    case plugin_result do
      # Plugin handled, continue propagation
      {:ok, updated_plugin_state, :propagate} ->
        new_manager_state = %{
            acc_manager
            | plugins: Map.put(acc_manager.plugins, plugin.name, updated_plugin_state)
          }
      ...
      _other ->
        Logger.warning("Invalid return from #{plugin.name}.handle_mouse/3: #{inspect(_other)}. Propagating.")
        # Continue propagating if plugin returns invalid value
        {:cont, {:ok, acc_manager, :propagate}}
    end
  end
  defp handle_mouse_update(error_acc = {:error, _}, _plugin, _callback_name, _plugin_result), do: {:halt, error_acc}

  # Handles results for handle_key_event, managing commands and propagation.
  @spec handle_key_update(accumulator(), map(), callback_name(), term()) :: handler_result
  defp handle_key_update({:ok, acc_manager, acc_commands, _propagation_state}, plugin, _callback_name, plugin_result) do
    case plugin_result do
      ...
      {:error, reason} ->
        Logger.error("Error from plugin #{plugin.name} in handle_input (key event): #{inspect(reason)}")
        {:halt, {:error, reason}}

      _other ->
        Logger.warning("Invalid return from #{plugin.name}.handle_input/2 (key event): #{inspect(_other)}. Propagating.")
        # Continue, assuming propagate
        {:cont, {:ok, acc_manager, acc_commands, :propagate}}
    end
  end
  defp handle_key_update(error_acc = {:error, _}, _plugin, _callback_name, _plugin_result), do: {:halt, error_acc}
