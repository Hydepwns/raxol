defmodule Raxol.Cloud.EdgeComputing.EdgeServer do
  @moduledoc """
  ...
  def handle_call({:cache_put, key, value, opts}, _from, state) do
    ...
    state =
      maybe_evict_cache_items(
        state.cache.size + data_size > state.cache.max_size,
        state,
        data_size
      )
    ...
    case Map.get(state.cache.items, key) do
      ...
      item ->
        ...
        handle_cache_item_ttl(
          item.ttl &&
            DateTime.diff(DateTime.utc_now(), item.created_at) > item.ttl,
          key,
          item,
          state
        )
  ...
  def handle_call(:process_pending, _from, state) do
    {processed, remaining} =
      process_queue_operations(state.queue.operations, state.config)
  ...
  def handle_call({:update_config, config}, _from, state) do
    ...
    updated_cache =
      update_cache_max_size_if_needed(
        config[:offline_cache_size],
        state.cache,
        config[:offline_cache_size]
      )
