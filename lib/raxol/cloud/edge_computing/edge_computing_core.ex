defmodule Raxol.Cloud.EdgeComputing.Core do
  ...
  def init(opts \\ []) do
    ...
    # Store state
    Raxol.Cloud.EdgeComputing.EdgeServer.update_sync_state(state)
  ...
  def get_state do
    Raxol.Cloud.EdgeComputing.EdgeServer.get_sync_state() || State.new()
  end
  ...
  defp handle_state_update(result) do
    case {is_map(result), is_map(result) && Map.has_key?(result, :mode)} do
      {true, true} ->
        ...
        Raxol.Cloud.EdgeComputing.EdgeServer.update_sync_state(result)
        result
