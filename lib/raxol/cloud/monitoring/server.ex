defmodule Raxol.Cloud.Monitoring.Server do
  ...
  def set_prometheus_metric(metric_name, value, tags) do
    GenServer.cast(__MODULE__, {:set_prometheus_metric, metric_name, value, tags})
  end
  ...
  def handle_call({:init_monitoring, opts}, _from, state) do
    ...
    config_updates = Keyword.take(opts, [
      :metrics_interval,
  ...
  def handle_cast({:record_metric, name, value, opts}, state) do
    if state.active do
      ...
      # Update metrics data
      metrics_data = Map.update(state.metrics.data, name, [metric], &([metric | &1] |> Enum.take(1000)))
      
      # Add to buffer for batch processing
      buffer = [metric | state.metrics.buffer]
      
      # Check if we need to flush
      {buffer, last_flush} = if length(buffer) >= state.config.metrics_batch_size do
      ...
      updated_state = check_alert_threshold(name, value, %{state | metrics: updated_metrics})
  ...
  def handle_cast({:record_metrics, metrics}, state) do
    updated_state = Enum.reduce(metrics, state, fn {name, value, opts}, acc_state ->
      {:noreply, new_state} = handle_cast({:record_metric, name, value, opts}, acc_state)
      new_state
    end)
    {:noreply, updated_state}
  end
  ...
  def handle_call({:run_health_check, opts}, _from, state) do
    if state.active do
      ...
      check_results = Enum.map(components, fn component ->
        {component, check_component_health(component, timeout)}
      end) |> Enum.into(%{})
      
      # Determine overall status
      overall_status = if Enum.all?(check_results, fn {_, status} -> status == :healthy end) do
        :healthy
      else
        :unhealthy
      end
      
      updated_health = %{state.health |
        status: overall_status,
        last_check: DateTime.utc_now(),
        components: check_results,
        check_results: [check_results | state.health.check_results] |> Enum.take(100)
      }
      
      updated_state = %{state | health: updated_health}
      
      # Trigger alert if unhealthy
      updated_state = if overall_status == :unhealthy do
        {:noreply, alert_state} = handle_cast(
  ...
  def handle_cast({:set_prometheus_metric, metric_name, value, tags}, state) do
    updated_prometheus = Map.put(state.backends.prometheus_metrics, metric_name, {value, tags})
    updated_backends = %{state.backends | prometheus_metrics: updated_prometheus}
  ...
  def handle_info(:run_health_check, state) do
    if state.active do
      # Run health check
      {:reply, _result, new_state} = handle_call({:run_health_check, []}, nil, state)
      
      # Schedule next check
      Process.send_after(self(), :run_health_check, state.config.health_check_interval)
  ...
  defp collect_system_metrics(state) do
    ...
    GenServer.cast(self(), {:record_metric, "memory.total", memory[:total], [source: :system]})
    GenServer.cast(self(), {:record_metric, "memory.processes", memory[:processes], [source: :system]})
    
    # Process metrics
    GenServer.cast(self(), {:record_metric, "process.count", length(:erlang.processes()), [source: :system]})
    ...
    GenServer.cast(self(), {:record_metric, "runtime.ratio", runtime_ratio, [source: :system]})
    
    # GC metrics
    gc_count = :erlang.statistics(:garbage_collection) |> elem(0)
    GenServer.cast(self(), {:record_metric, "gc.count", gc_count, [source: :system]})
  ...
  defp determine_severity(value, threshold) when value > threshold * 2.0, do: :critical
  defp determine_severity(value, threshold) when value > threshold * 1.5, do: :error  
  defp determine_severity(value, threshold) when value > threshold * 1.2, do: :warning
