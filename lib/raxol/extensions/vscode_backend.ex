defmodule Raxol.Extensions.VSCodeBackend do
  ...
  alias Raxol.AI.{ContentGeneration, PerformanceOptimization, ServiceAdapter}
  ...
  def handle_cast({:handle_message, message}, state) do
    case handle_extension_message(message, state) do
      {:ok, new_state} -> {:noreply, new_state}
      {:error, reason} ->
        Logger.error("[VSCodeBackend] Error handling message: #{inspect(reason)}")
        {:noreply, state}
    end
  end
  ...
  def handle_performance_analysis(code, component_name, metrics \\ %{}) do
    case PerformanceOptimization.get_ai_optimization_analysis(component_name, code, metrics) do
      {:ok, analysis} ->
        {:ok, %{
  ...
  def handle_component_analysis(file_path) do
    with {:ok, content} <- File.read(file_path),
      ...
      {:error, reason} -> {:error, "Failed to analyze component: #{inspect(reason)}"}
    end
  end
  ...
  defp listen_for_messages do
    case IO.read(:stdio, :line) do
      :eof -> 
        ...
        :ok
      {:error, reason} ->
        Logger.error("[VSCodeBackend] Error reading from stdin: #{inspect(reason)}")
        :error
      data ->
        case Jason.decode(String.trim(data)) do
          ...
          {:error, _} ->
            Logger.warning("[VSCodeBackend] Invalid JSON received: #{inspect(data)}")
        end
        listen_for_messages()
    end
  end

  defp handle_extension_message(%{"type" => "request", "id" => request_id} = message, state) do
    # Store the request
  ...
  defp process_request(%{"action" => "analyze_performance", "payload" => payload}) do
  ...
  defp generate_component_suggestions(ast) do
    ...
    suggestions = if complexity > 10 do
      ["Consider breaking down complex functions" | suggestions]
    else
      suggestions
    end
    
    suggestions = if length(functions) > 20 do
