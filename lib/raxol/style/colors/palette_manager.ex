defmodule Raxol.Style.Colors.PaletteManager do
  ...
  * `:saturation_adjust` - Adjustment to saturation for each step (default: 0.05)
  ...
  def generate_scale(base_color, steps \\ 9, opts \\ []) do
    ...
    # Calculate step size
    step_size = (max_lightness - min_lightness) / (steps - 1)
    
    # Generate colors
    scale = for i <- 0..(steps - 1) do
      # Calculate lightness for this step (start from lightest)
      lightness = max_lightness - (i * step_size)
      
      # Calculate saturation adjustment (parabolic curve, peaked in middle)
      saturation_factor = 1.0 + 
        (saturation_adjust * (1.0 - :math.pow(2.0 * (i / (steps - 1)) - 1.0, 2)))
      
      # Generate the color
      Utilities.adjust_color(base_color, %{
        lightness: lightness,
