defmodule Raxol.Style.Colors.System.Server do
  ...
  def init(opts) do
    ...
    accessibility_options = Keyword.get(opts, :accessibility_options, %{
  ...
  def handle_call({:init_system, opts}, _from, state) do
    ...
    high_contrast = case state.accessibility_options do
      nil -> 
        Keyword.get(opts, :high_contrast, false)
      accessibility_options ->
        Keyword.get(opts, :high_contrast, accessibility_options[:high_contrast])
    end
    ...
    updated_state = %{state | 
      ...
      current_theme_name: initial_theme_id,
      high_contrast: high_contrast,
      color_cache: %{}  # Clear cache on reinit
    }
    
    # Dispatch theme change event
    EventManager.dispatch({:theme_changed, %{
      theme: initial_theme,
      high_contrast: high_contrast
    }})
  ...
  def handle_call({:get_color, color_name, variant}, _from, state) do
    ...
    color = case Map.get(state.color_cache, cache_key) do
      nil ->
        # Calculate color
        calculated_color = if state.high_contrast do
        ...
        final_color = case calculated_color do
          %Color{} = c -> c
  ...
  def handle_call({:apply_theme, theme_name, opts}, _from, state) do
    ...
    updated_state = %{state | 
      ...
      high_contrast: high_contrast,
      color_cache: %{}  # Clear cache on theme change
    }
  ...
  def handle_call({:get_ui_color, ui_role}, _from, state) do
    color = case Map.fetch(state.current_theme.ui_mappings || %{}, ui_role) do
  ...
  def handle_call(:get_all_ui_colors, _from, state) do
    colors = if state.current_theme do
      (state.current_theme.ui_mappings || %{})
      |> Enum.map(fn {role, color_name} ->
        color_atom = if is_atom(color_name), do: color_name, else: String.to_atom(color_name)
  ...
  def handle_call({:set_accessibility_options, options}, _from, state) do
    ...
    if options[:high_contrast] != state.high_contrast do
      updated_state = %{updated_state | 
        ...
        color_cache: %{}  # Clear cache on high contrast change
      }
  ...
  @impl true
  def handle_cast({:handle_high_contrast, {:accessibility_high_contrast, enabled}}, state) do
    updated_state = %{state | 
  ...
  defp get_standard_color(theme, color_name, variant) do
    val = Map.get(theme.variants || %{}, {color_name, variant}) ||
          Map.get(theme.colors, color_name) ||
          Map.get(theme.variants || %{}, {to_string(color_name), to_string(variant)}) ||
  ...
  defp get_high_contrast_variant(theme, color_name, variant) do
    Map.get(theme.variants || %{}, {color_name, variant, :high_contrast}) ||
      Map.get(theme.variants || %{}, {to_string(color_name), to_string(variant), "high_contrast"})
  end
