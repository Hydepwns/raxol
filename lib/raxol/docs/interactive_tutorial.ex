defmodule Raxol.Docs.InteractiveTutorial do
  ...
  * Integration with documentation
  ...
  def init do
    ...
    state = Enum.reduce(built_in_tutorials(), state, fn tutorial, acc ->
  ...
  def list_tutorials do
    with_state(fn state ->
      ...
      |> Enum.map(fn tutorial ->
        ...
        progress = Map.get(state.progress, tutorial.id, %{
          completed: false,
          completed_steps: [],
          last_step: nil
        })
  ...
  def next_step do
    with_state(fn state ->
      if state.current_tutorial && state.current_step do
        ...
        current_index = Enum.find_index(tutorial.steps, fn step -> 
        ...
        if current_index < length(tutorial.steps) - 1 do
          ...
          # Update progress
          progress = Map.get(state.progress, state.current_tutorial, %{
            completed: false,
            completed_steps: [],
            last_step: nil
          })
          ...
          updated_progress = %{progress |
            completed_steps: [state.current_step | progress.completed_steps] |> Enum.uniq(),
          ...
          updated_state = %{state |
            ...
            progress: Map.put(state.progress, state.current_tutorial, updated_progress),
            history: [{:step_change, state.current_tutorial, next_step.id} | state.history]
          }
          ...
          updated_state = %{state |
            progress: Map.put(state.progress, state.current_tutorial, updated_progress),
            history: [{:tutorial_complete, state.current_tutorial} | state.history]
          }
      ...
      if state.current_tutorial && state.current_step do
        ...
        if current_index > 0 do
          ...
          updated_state = %{state |
            ...
            progress: Map.put(state.progress, state.current_tutorial, updated_progress),
            history: [{:step_change, state.current_tutorial, prev_step.id} | state.history]
          }
      ...
      if state.current_tutorial && state.current_step do
        ...
        if step.validation do
          ...
          if result == :ok do
            ...
            updated_state = %{state |
              progress: Map.put(state.progress, state.current_tutorial, updated_progress),
              history: [{:exercise_completed, state.current_tutorial, state.current_step} | state.history]
            }
      ...
      if state.current_tutorial && state.current_step do
        ...
        if step.hints && length(step.hints) > index do
          ...
          if UXRefinement.feature_enabled?(:ai_content_generation) do
            ...
            case ContentGeneration.generate(:hint, "Generate a hint for the current exercise", context: context) do
      ...
      %Tutorial{
        ...
        steps: [
          ...
          %Step{
            ...
            """,
            exercise: "Initialize the Raxol UX Refinement system and enable the focus_management and keyboard_navigation features.",
            validation: fn code ->
              cond do
                ...
                not String.contains?(code, "enable_feature(:keyboard_navigation)") ->
          ...
          %Step{
            ...
            """,
            example_code: nil,
            exercise: "Create a panel with a title, some text, and a button as shown in the example",
            validation: nil,
            hints: [],
