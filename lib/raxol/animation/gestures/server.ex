defmodule Raxol.Animation.Gestures.Server do
  @moduledoc """
  ...
  def handle_call({:register_handler, pid, gesture_type, handler}, _from, state) do
    ...
    updated_state = update_process_state(state, pid, fn gesture_state ->
      handlers = Map.update(
  ...
  def handle_call({:touch_move, pid, position, time}, _from, state) do
    updated_state = update_process_state(state, pid, fn gesture_state ->
      if gesture_state.active do
        ...
        %{gesture_state |
          touch_current: position,
          touch_history: [position | gesture_state.touch_history] |> Enum.take(10),
          current_time: time,
          velocity: velocity,
  ...
  def handle_call({:touch_up, pid, position, time}, _from, state) do
    updated_state = update_process_state(state, pid, fn gesture_state ->
      if gesture_state.active do
        ...
        call_handlers_async(final_gesture_type, gesture_data, gesture_state.handlers)
        
        # Create animation if applicable
        animations = create_animation_if_applicable(
          final_gesture_type, 
          gesture_data, 
  ...
  def handle_call({:update_animations, pid, delta_time}, _from, state) do
    delta_time = delta_time || 16.67 # ~60fps
    
    updated_state = update_process_state(state, pid, fn gesture_state ->
      # Update physics worlds for all animations
      updated_animations = Enum.map(gesture_state.active_animations, fn animation ->
        updated_world = PhysicsEngine.step(animation.world, delta_time / 1000.0)
        %{animation | world: updated_world}
      end)
      
      # Remove completed animations
      active_animations = Enum.reject(updated_animations, &animation_completed?/1)
  ...
  defp detect_gesture_type(gesture_state, event_type) do
    ...
    case event_type do
      ...
      :up ->
        ...
        distance = calculate_distance(gesture_state.touch_start, gesture_state.touch_current)
        velocity_magnitude = Vector.magnitude(gesture_state.velocity)
        
        case {velocity_magnitude > 200, duration > 500, distance > 10, distance > 5} do
          {true, _, true, _} -> :swipe
  ...
  defp create_animation_if_applicable(gesture_type, gesture_data, current_animations) do
  ...
  defp create_swipe_animation(gesture_data, animations) do
    ...
    world = PhysicsEngine.add_object(world, "swipe_element", %{
  ...
  defp create_drag_animation(gesture_data, animations) do
    ...
    world = PhysicsEngine.add_object(world, "drag_element", %{
      position: gesture_data.end_position,
      velocity: %Vector{x: gesture_data.velocity.x * 0.3, y: gesture_data.velocity.y * 0.3},
      mass: 1.2,
      friction: 0.9
    })
