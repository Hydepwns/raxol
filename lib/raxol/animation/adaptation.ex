defmodule Raxol.Animation.Adaptation do
  ...
  """
    def re_adapt_animations_if_needed(user_preferences_pid, active_animations \\ nil) do
    active_animations = active_animations || StateManager.get_active_animations()

    Raxol.Core.Runtime.Log.debug("[Animation] Re-adapting #{map_size(active_animations)} active animations")
    ...
    completed_due_to_disable = Enum.flat_map(active_animations, fn {element_id, element_animations} ->
      Enum.flat_map(element_animations, fn {animation_name, instance} ->
        animation = instance.animation

        # Re-adapt the animation
        adapted_animation = AnimAccessibility.adapt_animation(
          animation,
          reduce_motion?,
        ...
        if AnimAccessibility.disabled?(adapted_animation) do
          ...
          updated_instance = Map.put(updated_instance, :pending_completion, true)
          ...
          _updated_instance = Map.delete(updated_instance, :pending_completion)
          # Not completed
          []
        end

                # Update the instance in the state manager
        StateManager.put_active_animation(element_id, animation_name, updated_instance)
        []
      end)
    end)
  ...
  def should_apply_cognitive_accessibility? do
    Raxol.Core.UserPreferences.get([:accessibility, :cognitive_accessibility]) || false
  end
end
