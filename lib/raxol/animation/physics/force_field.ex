defmodule Raxol.Animation.Physics.ForceField do
  ...
  * `:strength` - Strength of the field (default: 1.0)
  ...
  def directional_field(opts \\ []) do
    %__MODULE__{
      type: :directional,
      direction: Keyword.get(opts, :direction, %Vector{x: 0, y: 1, z: 0}) |> Vector.normalize(),
      strength: Keyword.get(opts, :strength, 1.0),
      radius: :infinity
    }
  end
  ...
  * `:properties` - Additional properties for the function (default: %{})
  ...
  defp calculate_point_force(%__MODULE__{type: :point} = field, object) do
    ...
    if field.radius != :infinity and distance > field.radius do
      ...
      force_magnitude = case field.falloff do
        :none -> field.strength
        :linear -> field.strength * (1 - distance / field.radius)
        :quadratic -> field.strength * (1 - (distance / field.radius) * (distance / field.radius))
      end
  ...
  defp calculate_directional_force(%__MODULE__{type: :directional} = field, _object) do
  ...
  defp calculate_vortex_force(%__MODULE__{type: :vortex} = field, object) do
    ...
    if field.radius != :infinity and distance > field.radius do
      ...
      axis_projection = Vector.scale(field.direction, Vector.dot(to_object, field.direction))
