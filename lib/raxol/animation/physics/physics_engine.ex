defmodule Raxol.Animation.Physics.PhysicsEngine do
  ...
  * Force fields
  ...
  @type world_state :: %{
    ...
    last_update: integer() # System time in milliseconds
  }
  ...
  def new_world(opts) do
    ...
    |> Map.put(:boundaries, Map.merge(world.boundaries, Keyword.get(opts, :boundaries, %{})))
  end
  ...
  def update(world, delta_time \\ nil) do
    ...
    # Update all objects
    updated_objects = world.objects
    |> Enum.map(fn {id, object} -> {id, update_object(object, world, delta_time)} end)
    |> Map.new()
  ...
  """
  def spring_force(world, object_id_1, object_id_2, spring_constant, rest_length) do
  ...
  def create_particle_system(world, position, count, opts \\ []) do
    ...
    Enum.reduce(1..count, world, fn i, acc_world ->
      ...
      # Create the particle
      particle = new_object("particle_#{world.iteration}_#{i}", 
        position: position,
        velocity: velocity,
        mass: size * 0.1,
        properties: %{
  ...
  defp update_object(object, world, delta_time) do
    ...
    # Apply force fields
    all_forces = Enum.reduce(world.force_fields, all_forces, fn field, forces ->
      field_force = ForceField.calculate_force(field, object)
      [field_force | forces]
    end)
    
    # Calculate net force
    net_force = Enum.reduce(all_forces, %Vector{x: 0, y: 0, z: 0}, &Vector.add/2)
    ...
    velocity_after_damping = if damping > 0 do
    ...
    velocity = Vector.add(velocity_after_damping, Vector.scale(acceleration, delta_time))
    ...
    properties = if Map.has_key?(object.properties, :lifetime) do
      Map.update!(object.properties, :lifetime, fn lifetime -> lifetime - delta_time end)
    else
      object.properties
    end
    ...
    %{object | 
      ...
      forces: [], # Clear forces after applying
      ...
      Enum.reduce(object_list, acc_objects, fn obj2, inner_acc ->
        if obj1.id != obj2.id do
          ...
          if check_collision(obj1_updated, obj2_updated) do
            ...
            {obj1_after, obj2_after} = resolve_collision(obj1_updated, obj2_updated)
            inner_acc
  ...
  defp resolve_collision(obj1, obj2) do
    ...
    if velocity_along_normal > 0 do
      ...
      obj1_velocity = Vector.subtract(obj1.velocity, Vector.scale(impulse, 1 / obj1.mass))
      obj2_velocity = Vector.add(obj2.velocity, Vector.scale(impulse, 1 / obj2.mass))
  ...
  defp apply_boundaries(objects, boundaries) do
    Enum.map(objects, fn {id, obj} ->
      ...
      # Apply X boundaries
      updated_obj = if boundaries.min_x != nil and obj.position.x < boundaries.min_x do
        %{updated_obj | 
          position: %{updated_obj.position | x: boundaries.min_x},
          velocity: %{updated_obj.velocity | x: -updated_obj.velocity.x * 0.8}
        }
      else
        updated_obj
      end
      
      updated_obj = if boundaries.max_x != nil and obj.position.x > boundaries.max_x do
        %{updated_obj | 
      ...
      # Apply Y boundaries
      updated_obj = if boundaries.min_y != nil and obj.position.y < boundaries.min_y do
        %{updated_obj | 
          position: %{updated_obj.position | y: boundaries.min_y},
          velocity: %{updated_obj.velocity | y: -updated_obj.velocity.y * 0.8}
        }
      else
        updated_obj
      end
      
      updated_obj = if boundaries.max_y != nil and obj.position.y > boundaries.max_y do
        %{updated_obj | 
      ...
      # Apply Z boundaries
      updated_obj = if boundaries.min_z != nil and obj.position.z < boundaries.min_z do
        %{updated_obj | 
          position: %{updated_obj.position | z: boundaries.min_z},
          velocity: %{updated_obj.velocity | z: -updated_obj.velocity.z * 0.8}
        }
      else
        updated_obj
      end
      
      updated_obj = if boundaries.max_z != nil and obj.position.z > boundaries.max_z do
        %{updated_obj | 
