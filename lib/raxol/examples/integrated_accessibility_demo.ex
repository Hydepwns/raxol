defmodule Raxol.Examples.IntegratedAccessibilityDemo do
    ...
    case state.active_section do
      :animation ->
        if state.sample_animation do
          AnimationFramework.stop_animation(
            :demo_animation,
            :demo_animation_target
          )

          %{state | sample_animation: nil}
        else
          _animation =
            create_demo_animation(state.animation_speed, state.reduced_motion)

          AnimationFramework.start_animation(
            :demo_animation,
            :demo_animation_target
          )
  ...
  defp create_demo_animation(speed, reduced_motion) do
    ...
    final_duration =
      if reduced_motion, do: div(base_duration, 4), else: base_duration
  ...
  defp update_animation(state) do
    case AnimationFramework.get_current_value(
           state.sample_animation,
           :demo_animation_target
         ) do
      {:not_found, _done} ->
        %{state | sample_animation: nil}
  ...
  defp render_navigation(state, _direction) do
    ...
    |> Enum.each(fn {section, index} ->
      ...
      displayed_name =
        if is_focused, do: "> #{section_name} <", else: "  #{section_name}  "

      Terminal.print(displayed_name,
        color: color,
        bold: is_active || is_focused
      )
    end)
  ...
  defp render_welcome_section(state) do
    ...
    Terminal.println(
      "#{I18n.t("demo.reduced_motion")}: #{state.reduced_motion}"
    )

    Terminal.println("#{I18n.t("demo.theme")}: #{state.theme}")

    Terminal.println(
      "#{I18n.t("demo.language")}: #{I18n.t("locales.#{state.locale}")}"
    )
  ...
  defp render_color_system_section(state) do
    Terminal.println("#{I18n.t("section.color_system")}",
      color: ColorSystem.get_color(:primary),
      bold: true
    )
    ...
    high_contrast_color =
      if state.high_contrast,
        do: ColorSystem.get_color(:success),
        else: ColorSystem.get_color(:foreground)
  ...
  defp render_animation_section(state) do
    Terminal.println("#{I18n.t("section.animation")}",
      color: ColorSystem.get_color(:primary),
      bold: true
    )
    ...
    if state.loading_progress > 0 do
      ...
      progress_bar =
        String.duplicate("█", completed_chars) <>
          String.duplicate("░", remaining_chars)
      ...
      if state.loading_progress >= 100 do
        Terminal.println("\n#{I18n.t("animation.completed")}",
          color: ColorSystem.get_color(:success)
        )
      end
    end
  end
  ...
  defp render_i18n_section(state) do
    Terminal.println("#{I18n.t("section.internationalization")}",
      color: ColorSystem.get_color(:primary),
      bold: true
    )
    ...
    rtl_label =
      if is_rtl, do: "RTL (Right-to-Left)", else: "LTR (Left-to-Right)"
  ...
  defp render_preferences_section(state) do
    Terminal.println("#{I18n.t("section.user_preferences")}",
      color: ColorSystem.get_color(:primary),
      bold: true
    )
    ...
    save_color =
      if state.preferences_saved,
        do: ColorSystem.get_color(:success),
        else: ColorSystem.get_color(:foreground)

    Terminal.println("#{I18n.t("demo.save_preferences")} (Space)",
      color: save_color
    )
    ...
    if state.preferences_saved do
      ...
      Terminal.println("✓ #{I18n.t("demo.preferences_saved")}",
        color: ColorSystem.get_color(:success)
      )
    end
  end
  ...
  defp render_shortcuts_section(state) do
    Terminal.println("#{I18n.t("section.keyboard_shortcuts")}",
      color: ColorSystem.get_color(:primary),
      bold: true
    )
