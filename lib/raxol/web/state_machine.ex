defmodule Raxol.Web.StateMachine do
  ...
  defmacro deftransition(from_state => to_states) when is_list(to_states) do
    quote do
      Enum.each(unquote(to_states), fn to_state ->
        @transitions Map.put(@transitions || %{}, {unquote(from_state), to_state}, %{
  ...
  defmacro deftransition(from_state => to_state) do
    quote do
      @transitions Map.put(@transitions || %{}, {unquote(from_state), unquote(to_state)}, %{
        from: unquote(from_state),
        to: unquote(to_state),
        defined_at: __ENV__
      })
  ...
  defmacro defhandler(state_name, do: block) do
    quote do
      defmodule Module.concat(__MODULE__, "Handler#{unquote(state_name) |> Atom.to_string() |> Macro.camelize()}") do
      ...
      @state_handlers Map.put(@state_handlers || %{}, unquote(state_name), 
        Module.concat(__MODULE__, "Handler#{unquote(state_name) |> Atom.to_string() |> Macro.camelize()}"))
    end
  end
  ...
  defmacro deftransition_guard(name, guard_expr) do
    quote do
      @transition_guards Map.put(@transition_guards || %{}, unquote(name), unquote(guard_expr))
  ...
  defmacro __using__(opts \\ []) do
    quote location: :keep do
      ...
      Module.register_attribute(__MODULE__, :states, accumulate: false, persist: true)
      Module.register_attribute(__MODULE__, :transitions, accumulate: false, persist: true)
      Module.register_attribute(__MODULE__, :state_handlers, accumulate: false, persist: true)
      Module.register_attribute(__MODULE__, :transition_guards, accumulate: false, persist: true)
  ...
  defmacro __before_compile__(env) do
    ...
    quote location: :keep do
      ...
      def transition(machine_pid_or_session_id, to_state, data \\ %{}) do
        GenServer.call(resolve_pid(machine_pid_or_session_id), {:transition, to_state, data})
      end
      ...
      def update_state_data(machine_pid_or_session_id, data) do
        GenServer.cast(resolve_pid(machine_pid_or_session_id), {:update_state_data, data})
      end
      
      def get_transition_history(machine_pid_or_session_id) do
        GenServer.call(resolve_pid(machine_pid_or_session_id), :transition_history)
      end
      
      def force_state(machine_pid_or_session_id, state, reason \\ :forced) do
        GenServer.call(resolve_pid(machine_pid_or_session_id), {:force_state, state, reason})
      end
      ...
      def init({session_id, initial_state, opts}) do
        ...
        # Enter initial state
        {:ok, machine} = enter_state(machine, initial_state, %{reason: :initialization})
      ...
      def handle_call({:transition, to_state, data}, _from, machine) do
        case validate_transition(machine.current_state, to_state) do
          ...
          {:error, reason} ->
            Logger.warning("Invalid transition from #{machine.current_state} to #{to_state}: #{reason}")
      ...
      def handle_call({:force_state, state, reason}, _from, machine) do
        ...
        # Enter new state
        {:ok, new_machine} = enter_state(machine, state, %{reason: reason, forced: true})
        ...
        case Map.get(machine.handlers, machine.current_state) do
          ...
          handler_module ->
            case handler_module.timeout(timer_name, machine) do
              ...
              {:transition, to_state} -> 
                case perform_transition(machine, to_state, %{reason: :timeout, timer: timer_name}) do
                  {:ok, new_machine} -> {:noreply, new_machine}
      ...
      defp perform_transition(machine, to_state, data) do
        Logger.debug("Transitioning from #{machine.current_state} to #{to_state}")
      ...
      defp enter_state(machine, state, data) do
        case Map.get(machine.handlers, state) do
          nil -> 
            {:ok, machine}
          handler_module ->
            case handler_module.enter(%{machine | state_data: Map.merge(machine.state_data, data)}) do
      ...
      defp exit_state(machine, data) do
        case Map.get(machine.handlers, machine.current_state) do
          nil -> 
            {:ok, machine}
          handler_module ->
            case handler_module.exit(%{machine | state_data: Map.merge(machine.state_data, data)}) do
              {:ok, new_machine} -> {:ok, new_machine}
              {:error, reason} -> {:error, reason}
              _ -> {:ok, machine}
            end
      ...
      defp notify_session_bridge(machine) do
        case SessionBridge.get_session_state(machine.session_id) do
          {:ok, _session_state} ->
            ...
            SessionBridge.update_session_state(machine.session_id, changes, :state_machine)
          {:error, _reason} ->
      ...
      defp resolve_pid(session_id) when is_binary(session_id), do: via_tuple(session_id)
  ...
  defp validate_state_machine!(states, transitions, env) do
    ...
    if not Enum.empty?(undefined_states) do
      raise CompileError,
        file: env.file,
        line: env.line,
        description: "State machine validation failed: undefined states #{inspect(undefined_states)}"
    end
    
    # Check for unreachable states
    reachable_states = transitions
    |> Map.values()
    |> Enum.map(& &1.to)
    |> Enum.uniq()
    
    unreachable_states = defined_states -- (reachable_states ++ [List.first(defined_states)])
    
    if not Enum.empty?(unreachable_states) do
      IO.warn("Warning: unreachable states detected: #{inspect(unreachable_states)}",
  ...
  defmodule StateMachineBehaviour do
    ...
    @callback transition(pid() | String.t(), atom(), map()) :: {:ok, atom()} | {:error, atom()}
  ...
  defmodule StateHandler do
    ...
    @callback timeout(atom(), map()) :: {:ok, map()} | {:transition, atom()} | {:error, term()}
  end
end