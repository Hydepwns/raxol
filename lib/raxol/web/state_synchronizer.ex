defmodule Raxol.Web.StateSynchronizer do
  ...
  alias Raxol.Web.{PersistentStore, SessionBridge}
  ...
  """
  @spec apply_change(String.t(), operation(), user_id()) :: :ok | {:error, term()}
  ...
  def handle_remote_change(session_id, operation, from_user) do
    GenServer.cast(via_tuple(session_id), {:remote_change, operation, from_user})
  end
  ...
  def update_user_state(session_id, user_id, user_state) do
    GenServer.cast(via_tuple(session_id), {:update_user_state, user_id, user_state})
  end
  ...
  def merge_changes(current_state, changes, from_interface) do
    ...
    enhanced_changes = Map.put(changes, :_metadata, %{
    ...
    merged_state = Map.merge(current_state, enhanced_changes, fn
      key, old_value, new_value when key in [:cursor_position, :buffer_content] ->
        # For these keys, prefer newer timestamp
        old_timestamp = get_in(old_value, [:_metadata, :timestamp]) || DateTime.from_unix!(0)
  ...
  def handle_cast({:subscribe, subscriber_pid}, state) do
    Logger.debug("Subscribing #{inspect(subscriber_pid)} to session #{state.session_id}")
  ...
  def handle_cast({:unsubscribe, subscriber_pid}, state) do
    Logger.debug("Unsubscribing #{inspect(subscriber_pid)} from session #{state.session_id}")
  ...
  def handle_cast({:apply_change, operation, user_id}, state) do
    Logger.debug("Applying change from user #{user_id} in session #{state.session_id}")
    
    # Add vector clock information
    enhanced_operation = enhance_operation(operation, user_id, state.vector_clock)
  ...
  def handle_cast({:remote_change, operation, from_user}, state) do
    ...
    conflicts = detect_conflicts(operation, state.pending_operations, state.vector_clock)
    ...
    if Enum.empty?(conflicts) do
      ...
      # Update vector clock
      new_vector_clock = merge_vector_clocks(state.vector_clock, operation.vector_clock)
      
      {:noreply, %{state | vector_clock: new_vector_clock, last_sync: DateTime.utc_now()}}
    else
      ...
      new_vector_clock = merge_vector_clocks(state.vector_clock, operation.vector_clock)
      new_pending = remove_resolved_operations(state.pending_operations, conflicts)
  ...
  def handle_cast({:update_user_state, user_id, user_state}, state) do
    ...
    # Update user state
    enhanced_user_state = Map.merge(user_state, %{
      user_id: user_id,
  ...
  def handle_call(:get_sync_stats, _from, state) do
    stats = %{
      ...
      conflicts_resolved: 0  # Would be tracked in a full implementation
    }
  ...
  def handle_info({:DOWN, _ref, :process, pid, _reason}, state) do
    ...
    new_user_states = Enum.reject(state.user_states, fn {_user_id, user_state} ->
      Map.get(user_state, :pid) == pid
    end) |> Map.new()
    
    {:noreply, %{state | subscribers: new_subscribers, user_states: new_user_states}}
  end
  ...
  defp broadcast_presence_update(session_id, user_id, user_state) do
    ...
    PubSub.broadcast(Raxol.PubSub, topic, {:presence_update, user_id, user_state})
  end
  ...
  defp operations_conflict?(op1, op2) do
    ...
    case {op1.type, op2.type} do
      {:insert, :insert} -> 
        abs(op1.position - op2.position) < 5  # Arbitrary threshold
      {:delete, :delete} ->
        ranges_overlap?(op1.position, op1.position + op1.length, 
                       op2.position, op2.position + op2.length)
      {:insert, :delete} ->
        op1.position >= op2.position && op1.position <= op2.position + op2.length
      {:delete, :insert} ->
        op2.position >= op1.position && op2.position <= op1.position + op1.length
      _ ->
        false
    end
  end
  ...
  defp resolve_conflicts(operation, conflicts) do
    ...
    latest_conflict = Enum.max_by(conflicts, fn conflict_op -> 
  ...
  defp transform_operation(operation, conflicting_operation) do
    ...
    case {operation.type, conflicting_operation.type} do
      {:insert, :insert} when conflicting_operation.position <= operation.position ->
        %{operation | position: operation.position + String.length(conflicting_operation.content)}
      {:insert, :delete} when conflicting_operation.position < operation.position ->
        %{operation | position: max(0, operation.position - conflicting_operation.length)}
      _ ->
        operation
    end
  end
  ...
  defp remove_resolved_operations(pending_operations, conflicts) do
    ...
    Enum.reject(pending_operations, fn op -> MapSet.member?(conflict_ids, op.operation_id) end)
  end
