defmodule Raxol.Web.PersistentStore do
  ...
  def init(config) do
    ...
    cleanup_timer = Process.send_after(self(), :cleanup_ets, config.ets_cleanup_interval)
    sync_timer = Process.send_after(self(), :sync_dets, config.dets_sync_interval)
  ...
  @impl GenServer
  def handle_call({:store_session, session_id, session_state, opts}, _from, state) do
    ...
    result = case tier do
      :ets -> store_in_ets(session_id, enhanced_state, state.ets_table)
      :dets -> store_in_dets(session_id, enhanced_state, state.dets_table, state.config)
      :database -> store_in_database(session_id, enhanced_state, state.config)
      :all -> store_in_all_tiers(session_id, enhanced_state, state)
    end
  ...
  def handle_call({:get_session, session_id}, _from, state) do
    ...
    result = case get_from_ets(session_id, state.ets_table) do
      {:ok, session_state} -> 
        {:ok, session_state}
      {:error, :not_found} ->
        case get_from_dets(session_id, state.dets_table) do
          {:ok, session_state} ->
            ...
            spawn(fn -> promote_to_ets(session_id, session_state, state.ets_table) end)
            {:ok, session_state}
          {:error, :not_found} ->
            case get_from_database(session_id) do
              {:ok, session_state} ->
                ...
                spawn(fn -> 
                  promote_to_dets(session_id, session_state, state.dets_table, state.config)
  ...
  def handle_call({:update_session, session_id, changes}, _from, state) do
    ...
    case handle_call({:get_session, session_id}, nil, state) do
      {:reply, {:ok, current_state}, _} ->
        ...
        # Store updated state
        result = handle_call({:store_session, session_id, updated_state, []}, nil, state)
  ...
  def handle_call({:persist_all_tiers, session_id}, _from, state) do
    case get_from_ets(session_id, state.ets_table) do
      {:ok, session_state} ->
        ...
        :ok = store_in_dets(session_id, session_state, state.dets_table, state.config)
  ...
  def handle_call(:get_stats, _from, state) do
    ...
    memory_usage = Keyword.get(ets_info, :memory, 0) * :erlang.system_info(:wordsize)
  ...
  def handle_info(:cleanup_ets, state) do
    ...
    cleanup_timer = Process.send_after(self(), :cleanup_ets, state.config.ets_cleanup_interval)
  ...
  def handle_info(:sync_dets, state) do
    ...
    # Schedule next sync
    sync_timer = Process.send_after(self(), :sync_dets, state.config.dets_sync_interval)
  ...
  defp store_in_dets(session_id, session_state, dets_table, config) do
    ...
    final_state = if map_size(enhanced_state) > config.compression_threshold do
  ...
  defp store_in_all_tiers(session_id, session_state, state) do
    with :ok <- store_in_ets(session_id, session_state, state.ets_table),
         :ok <- store_in_dets(session_id, session_state, state.dets_table, state.config) do
  ...
  defp do_promote_session(session_id, target_tier, state) do
    case target_tier do
      :ets -> 
        case get_from_dets(session_id, state.dets_table) do
          {:ok, session_state} -> promote_to_ets(session_id, session_state, state.ets_table)
          _ -> :ok
        end
      :dets ->
        case get_from_database(session_id) do
          {:ok, session_state} -> promote_to_dets(session_id, session_state, state.dets_table, state.config)
          _ -> :ok
        end
    end
  end
  ...
  defp do_demote_session(session_id, target_tier, state) do
    ...
    case target_tier do
      :dets -> 
        case get_from_ets(session_id, state.ets_table) do
          {:ok, session_state} -> 
            store_in_dets(session_id, session_state, state.dets_table, state.config)
            :ets.delete(state.ets_table, session_id)
          _ -> :ok
        end
      :database ->
  ...
  defp cleanup_expired_sessions(ets_table) do
    ...
    :ets.foldl(fn {session_id, session_state}, _acc ->
  ...
  defp cleanup_lru_sessions(ets_table, max_entries) do
    ...
    if current_size > max_entries do
      ...
      sessions = :ets.foldl(fn {session_id, session_state}, acc ->
        last_access = get_in(session_state, [:metadata, :last_access]) || DateTime.utc_now()
