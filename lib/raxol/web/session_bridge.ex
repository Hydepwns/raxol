defmodule Raxol.Web.SessionBridge do
  ...
  """
  @spec update_session_state(String.t(), map(), interface()) :: :ok | {:error, term()}
  def update_session_state(session_id, changes, from_interface) do
    GenServer.cast(via_tuple(session_id), {:update_state, changes, from_interface})
  end
  ...
  """
  @spec list_active_sessions() :: [%{session_id: String.t(), interface: interface(), clients: integer()}]
  ...
  def handle_call({:migrate_to_web, web_pid}, _from, state) do
    ...
    session_state = case state.terminal_pid do
      nil -> 
  ...
  def handle_call({:migrate_to_terminal, terminal_pid}, _from, state) do
    ...
    broadcast_to_web_clients(state.web_pids, {:migrated_to_terminal, terminal_pid})
  ...
  def handle_cast({:update_state, changes, from_interface}, state) do
    Logger.debug("Updating session #{state.session_id} state from #{from_interface}")
    
    # Get current state
    {:ok, current_state} = PersistentStore.get_session(state.session_id)
    
    # Merge changes with conflict resolution
    new_state = StateSynchronizer.merge_changes(current_state, changes, from_interface)
    ...
    case from_interface do
      :terminal -> broadcast_to_web_clients(state.web_pids, {:state_update, changes})
      :web -> if state.terminal_pid, do: send(state.terminal_pid, {:state_update, changes})
    end
  ...
  def handle_info({:DOWN, _ref, :process, pid, _reason}, state) do
    Logger.info("Process #{inspect(pid)} disconnected from session #{state.session_id}")
    ...
    cond do
      ...
      MapSet.member?(state.web_pids, pid) ->
        ...
        new_interface = if MapSet.size(new_web_pids) == 0 and state.terminal_pid, do: :terminal, else: state.current_interface
        ...
        if MapSet.size(new_web_pids) == 0 and is_nil(state.terminal_pid) do
          Process.send_after(self(), :cleanup_session, 30_000) # 30 second grace period
        end
