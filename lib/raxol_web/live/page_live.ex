defmodule RaxolWeb.PageLive do
  ...
  def handle_event("toggle_high_contrast", _params, socket) do
    new_value = if socket.assigns.high_contrast == "true", do: "false", else: "true"
    {:noreply, 
      socket
  ...
  def handle_event("toggle_reduced_motion", _params, socket) do
    new_value = if socket.assigns.reduced_motion == "true", do: "false", else: "true"
    {:noreply, 
      socket
  ...
  def handle_event("toggle_font_size", _params, socket) do
    new_value = case socket.assigns.font_size do
      "normal" -> "large"
      "large" -> "larger"
      "larger" -> "normal"
    end
