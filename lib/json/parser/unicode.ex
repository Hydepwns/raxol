defmodule JSON.Parser.Unicode do
  ...
  """
  def parse(<<?\\, ?u, json::binary>>),
    do: parse_escaped_unicode_codepoint(json, 0, 0)
  ...
  defp parse_escaped_unicode_codepoint(
         <<?d, hex::utf8, f1, f2, ?\\, ?u, ?d, hex2::utf8, s1, s2,
           json::binary>>,
         _,
         0
       )
  ...
  defp parse_escaped_unicode_codepoint(json, acc, chars_parsed)
       when 4 === chars_parsed do
    {:ok, <<acc::utf8>>, json}
  end

  defp parse_escaped_unicode_codepoint(
         <<hex::utf8, json::binary>>,
         acc,
         chars_parsed
       )
       when hex in ~c"0123456789" do
  ...
  defp parse_escaped_unicode_codepoint(
       ...
       when hex in ~c"abcdef" do
    parse_escaped_unicode_codepoint(
      json,
      16 * acc + 10 + hex - ?a,
      chars_parsed + 1
    )
  ...
  defp parse_escaped_unicode_codepoint(<<>>, _, _),
    do: {:error, :unexpected_end_of_buffer}

  defp parse_escaped_unicode_codepoint(json, _, _),
    do: {:error, {:unexpected_token, json}}
end
