defmodule Mix.Tasks.Raxol.Memory.Stability do
  ...
  def run(args) do
    ...
    Mix.shell().info("====================")
    Mix.shell().info("Duration: #{config.duration}s (#{Float.round(config.duration / 60, 1)} minutes)")
  ...
  defp run_stability_test(config) do
    ...
    scenario_task = Task.async(fn -> run_scenario(config.scenario, config.duration) end)
    ...
    memory_measurements = Task.await(monitoring_task, (config.duration + 60) * 1000)
  ...
  defp run_scenario("log_streaming", duration), do: simulate_log_streaming(duration)
  defp run_scenario("interactive_shell", duration), do: simulate_interactive_shell(duration)
  ...
  defp simulate_vim_session(duration) do
    ...
    |> Stream.scan({buffer, operations_count}, fn operation, {current_buffer, count} ->
    ...
    |> List.last()
    |> case do
      {final_buffer, final_count} -> %{buffer: final_buffer, operations: final_count, type: "vim_session"}
      _ -> %{operations: 0, type: "vim_session"}
    end
  end
  ...
  defp simulate_log_streaming(duration) do
    ...
    |> List.last()
    |> case do
      {final_buffer, final_count} -> %{buffer: final_buffer, lines: final_count, type: "log_streaming"}
      _ -> %{lines: 0, type: "log_streaming"}
    end
  end
  ...
  defp simulate_interactive_shell(duration) do
    ...
    |> List.last()
    |> case do
      {final_buffer, final_count} -> %{buffer: final_buffer, commands: final_count, type: "interactive_shell"}
      _ -> %{commands: 0, type: "interactive_shell"}
    end
  end
  ...
  defp vim_syntax_highlight(buffer) do
    ...
    highlight_sequences = [
      "\e[34mdef\e[0m",      # Blue for keywords
      "\e[32m'string'\e[0m", # Green for strings
      "\e[33m# comment\e[0m" # Yellow for comments
    ]
  ...
  defp monitor_memory_loop(config, start_time, measurements) do
    ...
    if elapsed < config.duration do
      ...
      if config.with_profiling do
        Mix.shell().info("Memory at #{Float.round(elapsed, 1)}s: #{format_memory(memory_info.total)}")
      end
  ...
  defp analyze_results(results, config) do
    ...
    Mix.shell().info("\nMemory Analysis:")
    Mix.shell().info("  Initial: #{format_memory(results.initial_memory.total)}")
    ...
    Mix.shell().info("  Growth rate: #{Float.round(memory_analysis.growth_rate_mb_per_hour, 2)} MB/hour")
  ...
  defp analyze_memory_trend(measurements) do
    if length(measurements) < 2 do
      ...
      growth_rate_mb_per_hour = if elapsed_hours > 0 do
  ...
  defp detect_memory_leak(results, config) do
    growth_mb = (results.final_memory.total - results.initial_memory.total) / 1_000_000
