...
- **`raxol`: four benchmark harnesses masquerading as library modules.** `Raxol.UI.{ComponentTest, IntegrationTest, PerformanceTest, VisualTest}` (1,518 lines) drove `benchee`, a `only: [:dev, :test]` dependency, so every entry point raised `UndefinedFunctionError` for anyone who installed the package. The remaining benchmark and memory trees are now excluded from the published tarball rather than shipped broken.
