...
`Raxol.Application` passes the MCP supervisor an explicit authorizer, resolved by `Raxol.Application.resolve_mcp_authorizer/2` from `config :raxol, :mcp_authorizer` and `:mcp_read_authorizer` (each a 3-arity `(tool_name, arguments, context) -> :allow | {:ask, prompt} | {:deny, reason}` fun, or a `{module, function}` tuple naming one; a malformed value, or a tuple naming a function that does not exist, raises at boot rather than falling back). The tuple form exists because `sys.config` cannot hold a fun, so in a release the fun form only works from `runtime.exs`. `:mcp_allowed_tools` and `:mcp_allowed_read_methods` bind in EVERY environment; the environment decides only what happens when nothing is configured, which is `Raxol.MCP.Authorizer.allow_all/0` outside production and, in production, an empty tool allowlist plus a read allowlist of the three listing methods (`tools/list`, `resources/list`, `prompts/list`, so `resources/read`, the subscribe pair, `prompts/get` and `completion/complete` stay denied). Scoping those allowlists to production was a defect rather than a simplification: naming one in dev produced allow-all, while `mcp_authorizer_source/0`, which only checks whether the key is present, reported `:configured` and satisfied the SSE boot gate, so the one restricting setting opened the network gate and restricted nothing. `Raxol.MCP.Deployment.production?/0` is now read at runtime, which also means the SSE gate no longer bites in a dev session; `config :raxol_mcp, require_authorization: true` asks for the production posture there. The production default must not be `allow_all`, and a non-nil default must not open the network by itself: `Raxol.MCP.Server.authorization_configured?/1` is therefore **not** `authorizer != nil` but `authorizer != nil and authorizer_source == :configured`, and the SSE transport's boot gate (`Raxol.MCP.Deployment.enforce_authorization!/2`) reads that. `:authorizer_source` separates an operator's policy from raxol's fallback: naming `:mcp_authorizer` or `:mcp_allowed_tools` is what makes a network transport bootable (`mcp_allowed_tools: []` counts), the two read keys deliberately do not, and a framework fallback never does however strict it is. It defaults to `:default` in both `Raxol.MCP.Server` and `Raxol.MCP.Supervisor`, so an embedder who forgets the option fails closed; `Raxol.Application.mcp_supervisor_opts/0` passes `:configured` when an operator named a key. The empty tool allowlist is also tighter than the `nil` it replaced, since `Authorizer.decide/4` treats nil as allow. The three headless tools that change something (`raxol_start`, `raxol_send_key`, `raxol_stop`) are annotated sensitive and are refused at `tools/call` when no authorizer is configured; the three read tools are not annotated. The runtime half of that needs the tool registry reachable to classify a tool, and fails closed when it is not. `Raxol.Headless.McpTools.inject_into_tidewave/1` substitutes gated closures into BOTH halves of Tidewave's record (the dispatch map and the advertised entries' own `:callback`), so neither route bypasses the authorizer; it denies when the server is down and bounds its wait so one slow tool cannot hold every client. That gates raxol's tools only: Tidewave's own `project_eval` evaluates Elixir in the running node, so whether that endpoint is reachable is an endpoint-level decision (auth, bind address).
...
CLI banner carries the version plus the build commit (`raxol
<version>+<short-sha>`) so a stale binary is visible at a glance.
