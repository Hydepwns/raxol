...
`Raxol.Application` passes the MCP supervisor an explicit authorizer, resolved by `Raxol.Application.resolve_mcp_authorizer/2` from `config :raxol, :mcp_authorizer` and `:mcp_read_authorizer` (each a 3-arity `(tool_name, arguments, context) -> :allow | {:ask, prompt} | {:deny, reason}` fun; a malformed value raises at boot rather than falling back). Unconfigured, the default is `Raxol.MCP.Authorizer.allow_all/0` outside production and `nil` in production. That asymmetry is load-bearing: `Raxol.MCP.Server.authorization_configured?/1` is `authorizer != nil` and the SSE transport's boot gate (`Raxol.MCP.Deployment.enforce_authorization!/2`) reads exactly that, so defaulting production to `allow_all` would satisfy the gate and let a network transport serve every tool unguarded. The three headless tools that change something (`raxol_start`, `raxol_send_key`, `raxol_stop`) are annotated sensitive and are refused at `tools/call` when no authorizer is configured; the three read tools are not gated. `Raxol.Headless.McpTools.inject_into_tidewave/0` writes into Tidewave's own dispatch map and so bypasses the server and every gate on it; Tidewave is `only: :dev` and nothing calls that function, so it must not be wired into a shipped startup path.
