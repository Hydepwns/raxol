...
{status, result} = Raxol.Cloud.init(
...
{status, result} = Raxol.Cloud.register(
...
{status, result} = Raxol.Cloud.deploy(
...
{status, result} = Raxol.Cloud.scale(
...
{status, result} = Raxol.Cloud.connect(
  service: :storage,
  provider: :aws,
  region: "us-west-2"
)
