...
Raxol v1.4.1 introduces a unified configuration system based on TOML files. Enhanced in v2.0 with UnifiedConfigManager using BaseManager pattern. The `Raxol.Config` module provides centralized configuration management with environment-specific overrides, runtime updates, and validation.
