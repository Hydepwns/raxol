...
class StateInspector {
    ...
    static createOrShow(context) {
        ...
        const panel = vscode.window.createWebviewPanel('raxolStateInspector', 'Raxol State Inspector', column || vscode.ViewColumn.Three, {
            enableScripts: true,
    ...
    async exportStateHistory() {
        try {
            ...
            vscode.window.showErrorMessage(`Failed to export state history: ${error.message}`);
        }
    }
    /**
    ...
    updateContent(filter) {
        ...
        const filteredHistory = filter
            ? this.stateHistory.filter(entry => JSON.stringify(entry.state).toLowerCase().includes(filter.toLowerCase()))
            : this.stateHistory;
     ...
     */
    getWebviewContent(stateHistory, filter) {
     ...
     */
    renderHistoryList(stateHistory) {
