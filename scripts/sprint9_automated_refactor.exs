...
defmodule Sprint9AutomatedRefactor do
  ...
  defp refactor_cond_statements do
    ...
    # Analyze each file
    files_with_cond
    |> Enum.take(5)  # Process first 5 as examples
    |> Enum.each(fn file ->
  ...
  defp analyze_cond_in_file(file) do
    ...
    if length(cond_blocks) > 0 do
      ...
      Enum.each(cond_blocks, fn {start_line, conditions} ->
        IO.puts("    Line #{start_line}: cond with #{length(conditions)} conditions")
  ...
  defp find_cond_blocks(lines) do
    lines
    |> Enum.with_index(1)
    |> Enum.reduce({[], nil, []}, fn {line, idx}, {blocks, current_start, conditions} ->
      cond do
    ...
    Enum.each(conditions, fn condition ->
      cond do
        String.match?(condition, ~r/^is_(\w+)\((\w+)\)/) ->
          ...
          IO.puts("        def function(#{var}) when is_#{type}(#{var}), do: ...")
        ...
        String.match?(condition, ~r/^(\w+)\s+in\s+(.+)/) ->
          ...
          IO.puts("        def function(#{var}) when #{var} in #{range}, do: ...")
  ...
  defp check_if_scattered(functions) do
    ...
    Enum.any?(grouped, fn {_type, funcs} ->
      if length(funcs) > 1 do
        ...
        |> Enum.any?(fn {a, b} -> b - a > 50 end)  # Allow up to 50 lines between clauses
      else
        false
      end
    end)
