    ...
    |> Enum.reduce([], fn {line, line_num}, acc ->
      cond do
        ...
        String.match?(line, ~r/^\s*if\s+[\w\.]+\s+do$/) ->
          ["Line #{line_num}: Simple if/else can be replaced with pattern matching" | acc]
        
        # If with is_* guard
        String.match?(line, ~r/^\s*if\s+is_(atom|binary|list|map|nil|integer)\(/) ->
          ["Line #{line_num}: Type check if can be replaced with guard clause" | acc]
  ...
  defp find_cond_patterns(lines) do
    ...
    |> Enum.reduce([], fn {line, line_num}, acc ->
      if String.match?(line, ~r/^\s*cond\s+do/) do
        ["Line #{line_num}: Cond statement can be replaced with pattern matching functions" | acc]
      else
        acc
      end
    end)
  ...
  defp find_nested_conditions(lines) do
    ...
    |> Enum.reduce({[], 0}, fn {line, line_num}, {acc, depth} ->
      ...
      if new_depth > 2 && String.match?(line, ~r/\b(if|case|cond)\b/) do
        {["Line #{line_num}: Deeply nested condition (depth: #{new_depth})" | acc], new_depth}
      else
        {acc, new_depth}
      end
    end)
