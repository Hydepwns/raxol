defmodule Raxol.LiveView.TerminalBridge do
  ...
  @type theme :: :nord | :dracula | :solarized_dark | :solarized_light | :monokai | :default
  @type html_opts :: [
          theme: theme(),
          css_prefix: String.t(),
  ...
  def buffer_to_html(buffer, opts \\ []) do
    ...
    theme_class = if theme != :default, do: " #{css_prefix}-theme-#{theme}", else: ""
  ...
  @spec render_line_with_diff(Buffer.line(), Buffer.line(), non_neg_integer(), String.t()) ::
          String.t()
  ...
  @spec render_cell(Buffer.cell(), boolean(), atom(), String.t(), boolean()) :: String.t()
  defp render_cell(cell, is_cursor, cursor_style, css_prefix, use_inline) do
    char = escape_html(cell.char)
    style = cell.style

    cursor_class = if is_cursor, do: " #{css_prefix}-cursor #{css_prefix}-cursor-#{cursor_style}", else: ""
  ...
  def style_to_classes(style, css_prefix \\ "raxol") do
    ...
    # Text attributes
    classes = if Map.get(style, :bold), do: classes ++ ["#{css_prefix}-bold"], else: classes
    classes = if Map.get(style, :italic), do: classes ++ ["#{css_prefix}-italic"], else: classes

    classes =
      if Map.get(style, :underline), do: classes ++ ["#{css_prefix}-underline"], else: classes

    classes = if Map.get(style, :reverse), do: classes ++ ["#{css_prefix}-reverse"], else: classes
  ...
  def style_to_inline(style) do
    ...
    # Text attributes
    styles = if Map.get(style, :bold), do: styles ++ ["font-weight: bold"], else: styles
    styles = if Map.get(style, :italic), do: styles ++ ["font-style: italic"], else: styles
    ...
    styles =
      case Map.get(style, :fg_color) do
        ...
        n when is_integer(n) ->
          ...
          styles ++ ["color: rgb(#{elem(rgb, 0)}, #{elem(rgb, 1)}, #{elem(rgb, 2)})"]
  ...
  @spec color_256_to_rgb(non_neg_integer()) :: {non_neg_integer(), non_neg_integer(), non_neg_integer()}
